...
// makeDeliveryBody serializes an outbox.Entry into a v1 wire envelope suitable
// for delivery to unmarshalDelivery, via the package-public outbox.MarshalEnvelope
// (the only path to obtain wire bytes after SAFEID-UPSTREAM-FUNNEL-HARD-01 sealed
// kernel/outbox.wireMessage as package-private). RabbitMQ only accepts relay
// envelope format; legacy PascalCase Entry JSON is no longer supported.
//
// Callers must pass an Entry with safe ID-shaped fields; MarshalEnvelope's
// producer-side ParseSafeID fail-fast rejects unsafe IDs at marshal time. To
// produce attack-vector wire bytes for decoder rejection tests, build the JSON
// directly with []byte literal templating — the seal forbids in-Go construction
// of the wire envelope outside kernel/outbox.
func makeDeliveryBody(t *testing.T, entry outbox.Entry) []byte {
	t.Helper()
	// Entries are constructed via outbox.NewEntry, which validates a non-empty
	// payload, so no nil-payload default is needed.
	b, err := outbox.MarshalEnvelope(entry)
	require.NoError(t, err)
	return b
}

// mustNewEntry constructs a sealed outbox.Entry via the producer constructor,
// failing the test on construction error. eventType and payload are positional;
// remaining fields (ID, topic, metadata, timestamps) are passed as options.
func mustNewEntry(t *testing.T, eventType string, payload []byte, opts ...outbox.EntryOption) outbox.Entry {
	t.Helper()
...
func setupTopic(topic, group string) outbox.Subscription {
	return outbox.Subscription{Topic: topic, ConsumerGroup: group, CellID: group}
}
...
func TestSubscriber_Setup_EmptyDLX_ReturnsError(t *testing.T) {
	conn, _ := newTestConnection(t)
	sub := NewSubscriber(clock.Real(), conn, SubscriberConfig{DLXExchange: ""})
...
func TestSubscriber_Subscribe_ProcessesDelivery(t *testing.T) {
	...
	entry := mustNewEntry(t, "test.created", []byte(`{"key":"value"}`), outbox.WithID("evt-001"))
...
func TestUnmarshalDelivery(t *testing.T) {
	t.Run("wire_message_envelope", func(t *testing.T) {
		entry := mustNewEntry(t, "test.created", []byte(`{"x":1}`), outbox.WithID("entry-uuid-001"))
...
func TestSubscriber_Subscribe_HandlerError_NackWithRequeue(t *testing.T) {
	...
	entry := mustNewEntry(t, "test.failed", []byte(`{}`), outbox.WithID("evt-002"))
	entryBytes := makeDeliveryBody(t, entry)
...
func TestSubscriber_Subscribe_AfterClose(t *testing.T) {
	...
	err := sub.Subscribe(context.Background(), outbox.Subscription{Topic: "test.topic", CellID: "test-cell"},
		entryToSubHandler(func(_ context.Context, _ outbox.Entry) outbox.HandleResult {
			return outbox.Ack()
		}))
	assert.Error(t, err)
...
func TestSubscriber_DeliveryChannelClosed_TriggersReconnect(t *testing.T) {
	...
	go func() {
		subscribeDone <- sub.Subscribe(ctx, outbox.Subscription{Topic: "test.topic", CellID: "test-cell"}, entryToSubHandler(handler))
	}()
	...
	entry := mustNewEntry(t, "test.reconnected", []byte(`{}`), outbox.WithID("reconnect-001"))
	entryBytes := makeDeliveryBody(t, entry)
...
func TestSubscriber_ReconnectLoop_CtxCancelledDuringWait(t *testing.T) {
	...
	sub := NewSubscriber(clock.Real(), c, SubscriberConfig{
		QueueName:   "test-queue",
		DLXExchange: "test.dlx",
	})
...
func TestSubscriber_Subscribe_ClosedDuringReconnect(t *testing.T) {
	...
	subscribeDone := make(chan error, 1)
	go func() {
		subscribeDone <- sub.Subscribe(context.Background(), outbox.Subscription{Topic: "test.topic", CellID: "test-cell"},
			entryToSubHandler(func(_ context.Context, _ outbox.Entry) outbox.HandleResult {
				return outbox.Ack()
			}))
	}()
...
func TestSubscriber_ProcessDelivery_CtxCancelled_NackWithRequeue(t *testing.T) {
	...
	entry := mustNewEntry(t, "test.cancel", []byte(`{}`), outbox.WithID("evt-ctx-cancel"))
	entryBytes := makeDeliveryBody(t, entry)
...
func TestConsumerBase_WrapWithClaimer_Success_ReturnsReceipt(t *testing.T) {
	...
	handlerCalled := false
	handler := cb.Wrap(outbox.Subscription{Topic: "test.topic", ConsumerGroup: "test-group", CellID: "test-group"},
		func(_ context.Context, e outbox.Entry) outbox.HandleResult {
			handlerCalled = true
			return outbox.Ack()
		})

	entry := newIDEntry(t, "evt-claimer-001")
...
func TestConsumerBase_WrapWithClaimer_ClaimDone_SkipsHandler(t *testing.T) {
	...
	res, settlement := handler(context.Background(), newIDEntry(t, "evt-done"))
...
func TestConsumerBase_WrapWithClaimer_ClaimBusy_Requeues(t *testing.T) {
	...
	res, _ := handler(context.Background(), newIDEntry(t, "evt-busy"))
	assert.False(t, handlerCalled)
...
func TestConsumerBase_WrapWithClaimer_Reject_ThreadsReceipt(t *testing.T) {
	...
	res, settlement := handler(context.Background(), newIDEntry(t, "evt-reject"))
...
func TestConsumerBase_WrapWithClaimer_ExplicitReject_FirstRoundNoRetry(t *testing.T) {
	...
	res, settlement := handler(context.Background(), newIDEntry(t, "evt-reject-direct"))
	assert.Equal(t, outbox.DispositionReject, res.Disposition)
...
func TestConsumerBase_WrapWithClaimer_ClaimError_DefaultFailClosed_LocalRetryThenSuccess(t *testing.T) {
	...
	res, settlement := handler(context.Background(), newIDEntry(t, "evt-retry-ok"))
...
func TestConsumerBase_WrapWithClaimer_ClaimError_DefaultFailClosed_HasBackoff(t *testing.T) {
	...
	start := time.Now()
	res, _ := handler(context.Background(), newIDEntry(t, "evt-claim-err"))
...
func TestConsumerBase_WrapWithClaimer_ClaimError_DefaultFailClosed_CtxCancel(t *testing.T) {
	...
	start := time.Now()
	res, _ := handler(ctx, newIDEntry(t, "evt-claim-ctx"))
	elapsed := time.Since(start)
...
func TestConsumerBase_WrapWithClaimer_ClaimError_DefaultFailClosed_RetryCount1(t *testing.T) {
	...
	start := time.Now()
	res, _ := handler(context.Background(), newIDEntry(t, "evt-retry1"))
	elapsed := time.Since(start)
...
func TestConsumerBase_WrapWithClaimer_ClaimRetryConfig_Independent(t *testing.T) {
	...
	start := time.Now()
	res, _ := handler(context.Background(), newIDEntry(t, "evt-independent"))
	elapsed := time.Since(start)
...
func TestConsumerBase_MaxRetryDelay_Caps_ClaimBackoff(t *testing.T) {
	...
	start := time.Now()
	_, _ = handler(context.Background(), newIDEntry(t, "evt-cap"))
	elapsed := time.Since(start)
...
func TestConsumerBase_NegativeClaimRetryBaseDelay_NoPanic(t *testing.T) {
	...
	res, _ := handler(context.Background(), newIDEntry(t, "evt-neg-delay"))
	assert.Equal(t, outbox.DispositionRequeue, res.Disposition)
}
...
func TestConsumerBase_NegativeMaxRetryDelay_NoPanic(t *testing.T) {
	...
	res, _ := handler(context.Background(), newIDEntry(t, "evt-neg-cap"))
	assert.Equal(t, outbox.DispositionRequeue, res.Disposition)
}
...
func TestProcessDelivery_NilReceipt_NoPanic(t *testing.T) {
	...
	entry := mustNewEntry(t, "test.nil", []byte(`{}`), outbox.WithID("evt-nil-receipt"))
	entryBytes := makeDeliveryBody(t, entry)
...
func TestProcessDelivery_PassesThroughContextWithoutRestore(t *testing.T) {
	...
	obsCtx := ctxkeys.WithTraceID(
		ctxkeys.WithCorrelationID(
			ctxkeys.WithRequestID(context.Background(), "req-sub-1"),
			"corr-sub-1"),
		"trace-sub-1")
	entry, entryErr := outbox.NewEntry(clock.Real(), obsCtx, "test.restore", []byte(`{}`),
		outbox.WithID("evt-ctx-restore"))
...
func TestProcessDelivery_DoesNotRestoreObservabilityContext(t *testing.T) {
	...
	obsCtx := ctxkeys.WithTraceID(
		ctxkeys.WithCorrelationID(
			ctxkeys.WithRequestID(context.Background(), "req-log-1"),
			"corr-log-1"),
		"trace-log-1")
	entry, entryErr := outbox.NewEntry(clock.Real(), obsCtx, "test.restore", []byte(`{}`),
		outbox.WithID("evt-no-restore"))
	require.NoError(t, entryErr)
	entryBytes := makeDeliveryBody(t, entry)
...
func TestConsumerBase_WrapWithClaimer_ClaimBusy_HasBackoff(t *testing.T) {
	...
	start := time.Now()
	res, _ := handler(context.Background(), newIDEntry(t, "evt-busy-backoff"))
	elapsed := time.Since(start)
...
func TestConsumerBase_WrapWithClaimer_ClaimError_FailClosed(t *testing.T) {
	...
	res, _ := handler(context.Background(), newIDEntry(t, "evt-fail-closed"))
...
func TestConsumerBase_WrapWithClaimer_ClaimError_FailOpen_Explicit(t *testing.T) {
	...
	res, settlement := handler(context.Background(), newIDEntry(t, "evt-fail-open-explicit"))
...
func TestConsumerBase_RetryLoop_CtxCancelledAfterFinalAttempt_Requeues(t *testing.T) {
	...
	res, _ := handler(ctx, newIDEntry(t, "evt-ctx-final"))
	assert.Equal(t, outbox.DispositionRequeue, res.Disposition,
...
func TestProcessDelivery_HandlerError_Logged(t *testing.T) {
	...
	entry := mustNewEntry(t, "test.ackwitherr", []byte(`{}`), outbox.WithID("evt-ack-with-err"))
	entryBytes := makeDeliveryBody(t, entry)
...
func TestConsumerBase_WrapWithClaimer_TransientError_ThenSuccess(t *testing.T) {
	...
	entry := newIDEntry(t, "evt-transient-ok")
	res, settlement := handler(context.Background(), entry)
...
func TestConsumerBase_WrapWithClaimer_ExplicitReject_NoRetry(t *testing.T) {
	...
	entry := newIDEntry(t, "evt-explicit-reject")
	res, settlement := handler(context.Background(), entry)
...
func TestConnection_ErrorChain_DoesNotLeakCredentials(t *testing.T) {
	...
	_, err := NewConnection(clock.Real(), Config{URL: url}, WithDialFunc(func(u string) (AMQPConnection, error) {
...
func TestConsumerBase_RetryExhaustion(t *testing.T) {
	...
	callCount := 0
	wrappedHandler := cb.Wrap(outbox.Subscription{Topic: "test.retry.unit", ConsumerGroup: "test-group", CellID: "test-group"},
		func(_ context.Context, _ outbox.Entry) outbox.HandleResult {
			callCount++
