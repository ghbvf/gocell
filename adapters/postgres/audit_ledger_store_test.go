...
func TestAuditLedgerStore_HashFormatConstraint(t *testing.T) {
	...
	insertRow := func(ns string, seq int64, prevHash, hash string) error {
		...
		// trace_id is supplied as '' (empty) — it is NOT NULL no-DEFAULT after
		// migration 046 but is excluded from the HMAC chain (observability only).
		_, err := p.DB().Exec(ctx,
...
// ---------------------------------------------------------------------------
// TestAuditLedgerStore_TraceID_RoundTripAndFilter (migration 046)
// ---------------------------------------------------------------------------
