-- Migration 047: create (namespace, trace_id) index on audit_entries CONCURRENTLY.
-- Split from 046 because CREATE INDEX CONCURRENTLY cannot run inside a transaction
-- (goose wraps transactional migrations in BEGIN/COMMIT). The column itself is
-- added in 046 (transactional, safe to roll back atomically); this migration adds
...
--
-- ref: 046_audit_entries_trace_id.sql — column definition
