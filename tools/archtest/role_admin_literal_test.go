...
//
// Forbids duplicate const declarations of the form
//
...
// Any other file re-declaring the same constant string is a drift risk:
// the names will diverge silently when the role value changes.
//
// Detection uses AST-level const scanning so string literals inside
// comments, function bodies with non-const "admin" values, and log-field
