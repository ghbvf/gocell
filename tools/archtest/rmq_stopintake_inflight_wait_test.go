...
func TestRMQStopIntakeInflightWait01_StopIntakeWaitsForInflight(t *testing.T) {
	...
	// Look for an inflight-wait sentinel in the function body. Accept either:
	//   - a call ending in `.localWg.Wait()` (per-run waitgroup)
	//   - a call to `run.waitInflight(...)` / `r.waitInflight(...)` helper
	//   - a call to a wgDone() helper on a subscriptionRun
	// The sentinel proves StopIntake settles in-flight processDelivery goroutines.
	var found bool
	ast.Inspect(stopIntake.Body, func(n ast.Node) bool {
		...
		case "waitInflight", "waitDrained", "wgDone":
			found = true
			return false
		}
		return true
