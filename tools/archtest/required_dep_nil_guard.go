...
// # Register status
//
// Not registered in StandardCellRules: A2 fires only on services carrying a
// gocell:"required" field plus the validateRequired() generated by the
// requireddepsgen codegen funnel (A1); an external repo not using that codegen
// has no required-tagged services → scan is vacuous, no external safety signal.
