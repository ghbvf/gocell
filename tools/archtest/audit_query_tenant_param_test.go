...
// tenant from ctx; only Query takes the typed param — enrolling the whole Store
// would need five carve-outs, see ADR 202606071300 §threat-matrix). The audit
// TENANT axis instead rests on Query's typed positional tenant.TenantID param
// (param[1], after ctx): on the mem backend it is the SOLE tenant isolation, and
// on PG it is the app-layer half of the FORCE-RLS dual-layer.
//
...
//
// The scanner asserts that the `Query` method of every enrolled audit-read
// interface carries tenant.TenantID at param[1] (immediately after ctx), using
// go/types object identity (alias-proof).
//
...
//
//   - Only the method literally named "Query" is checked (the audit read API). A
//     future second tenant-scoped read method would need adding to the name set —
//     guarded by the anti-vacuity check (zero Query methods scanned fails the test).
...
// auditQueryReadIfaces are the audit-read interfaces whose Query method must carry
// a typed tenant.TenantID at param[1]. Looked up via Scope.Lookup in the enrolled
// ledger package; absent names resolve to nil and are skipped.
	...
	for _, ifaceName := range ifaceNames {
		...
		for i := 0; i < iface.NumExplicitMethods(); i++ {
			...
			diags = append(diags, Diagnostic{
				Line: pos.Line,
				Message: fmt.Sprintf(
					"AUDIT-QUERY-TENANT-PARAM-01: %s.Query must carry tenant.TenantID at param[1] (after ctx). "+
...
// it runs the SAME scanner against the RED/GREEN fixture and asserts FakeBadQueryStore
// (Query without the tenant param) is flagged while FakeGoodQueryStore is not.
// Expects exactly 1 diagnostic.
func TestAuditQueryTenantParam01_ScannerCatchesViolation(t *testing.T) {
	...
	if len(diags) != 1 {
		t.Fatalf("expected exactly 1 diagnostic (FakeBadQueryStore.Query missing tenant param), got %d: %v", len(diags), diags)
	}
}
