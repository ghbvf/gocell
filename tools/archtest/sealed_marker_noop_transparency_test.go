...
func TestSealedMarkerNoopTransparency01(t *testing.T) {
	...
	// internalCellWriter / internalCellEmitter (kernel/outbox). Assert we found
	// at least the three pre-existing ones so a load regression cannot make
	// this archtest vacuously pass.
	if found < 3 {
		t.Fatalf("SEALED-MARKER-NOOP-TRANSPARENCY-01: discovered only %d internalCell* struct types under "+
			"kernel/...; expected ≥3 (auto-discovery likely broken)", found)
	}
}
