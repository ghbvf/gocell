...
func (c *enrolledCursor) add(e outbox.Entry) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.next++
	c.pos[e.ID()] = c.next
...
func (c *enrolledCursor) Position(e outbox.Entry) (int64, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	if p, ok := c.pos[e.ID()]; ok {
		return p, nil
	}
...
func (unenrolledCursor) Position(_ outbox.Entry) (int64, error) {
	return 1, nil
}
