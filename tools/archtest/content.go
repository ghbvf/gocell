...
// ContentContext is the non-Go counterpart of the scanner's per-file payload
// for YAML / JSON / Markdown / SQL etc. Plain bytes only — no AST, no FileSet.
type ContentContext = scanner.ContentContext

// loadContentFiles is the pure testing-free reader: validates suffixes, walks
// the scope, returns a slice of [ContentContext]. It is unexported on purpose —
// it is the in-package reader for non-test helpers that return ([]X, error);
// test code that wants fail-loud ergonomics uses [EachContentFile] instead.
//
