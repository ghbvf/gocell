...
// call-bearing statement. The scan uses Production (ModeWorkspace) and filters
// by relPath so satellite go.work modules stay covered. Generated assembly
// mains are covered by the A3 generated segment (marker-derived, Hard
// self-covering) and MUST NOT appear here. This residual list is the Go-language
// permanent Medium ceiling: codegen cannot reach hand-written mains, so their
// seal is hand-maintained (A1's Hard ban on bare handler construction outside
// logging/ is the backstop). Tracked as a deliberate won't-do at gh #1424
// (analogous to #851 / #893 holder-seal ceilings).
var slogHandwrittenEntryPoints = []struct {
	pkgPattern string // package pattern used by reverse-coverage allowlist
...
func TestSlogHandlerSealedFunnel_A3_DetectsViolation(t *testing.T) {
	...
	_ = Run(t, Production(TypedOpts{Tests: false}),
		func(p *Pass) []Diagnostic {
			if p.TypesInfo == nil {
				return nil
			}
			for _, f := range p.Files {
				if filepath.ToSlash(p.Rel(f)) != "cmd/corebundle/main.go" {
					continue
				}
				if !isGocellAssemblyEntryMain(f) {
					continue
