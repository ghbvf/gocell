...
// calls GetByIDInTenant but uses the WRONG comparison operator (>) instead of (!=).
...
type stubService struct {
	userRepo interface {
		GetByIDInTenant(ctx context.Context, tenantID string, id string) (stubUser, error)
	}
}
...
// AuthzEpoch and calls GetByIDInTenant but uses > instead of != — the epoch-inequality
...
func (s *stubService) enforceSessionState(ctx context.Context, claims kauth.Claims) (kauth.Claims, error) {
	user, err := s.userRepo.GetByIDInTenant(ctx, claims.TenantID, claims.Subject)
	if err != nil {
