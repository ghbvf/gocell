...
//
//	ASSEMBLY-CELLMODULE-TYPE-04         When cmd/{id}/modules_gen.go exists,
//	                                    cmd/{id}/ same package must have a
//	                                    top-level type CellModule declaration
//	                                    (interface or struct, either is valid).
//	                                    Without CellModule defined in the
//	                                    package, modules_gen.go cannot compile,
//	                                    but the compile error gives no
//	                                    actionable guidance. This rule provides
//	                                    fail-fast scaffolding guidance instead.
//
...
//   - CellModule type must be present in cmd/{id}/ same package so that the
//     generated modules_gen.go compiles; ASSEMBLY-CELLMODULE-TYPE-04 provides
//     actionable scaffold guidance when the type is absent.
...
//
// MAINTENANCE NOTE: 添加新 platform cell 时**必须**同步更新此列表，否则
// ASSEMBLY-MODULES-SWITCH-FORBIDDEN-02 对新 cell ID 的 switch-case 检测失效。
// Future: 可改为 filepath.Glob("cells/*/cell.yaml") 动态读，但 archtest 故意
// 避免 metadata 包依赖（保持 archtest 自包含）。
