...
//     type but file outside kernel/reconcile/; the path gate must still flag it.
//   - control_plane_wrong_receiver_type_violates: RED — method of otherClock
//     (not controlPlaneClock) in kernel/reconcile/ is NOT exempt.
//   - control_plane_no_marker_violates: RED (repurposed) — free function in
//     kernel/reconcile/ (no receiver) calling time.NewTicker is flagged.
...
func TestProdClockInjectionFixtures(t *testing.T) {
	...
	dirs := []string{
		...
		"control_plane_wrong_path_violates",          // RED: controlPlaneClock method outside kernel/reconcile/
		"control_plane_wrong_receiver_type_violates", // RED: wrong receiver type in kernel/reconcile/
		"control_plane_no_marker_violates",           // RED: free function in kernel/reconcile/ (no receiver)
