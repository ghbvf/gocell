...
- 新 backlog 条目：全程 CLI / 技能创建，`gh issue create --label backlog --label pri-pX --label area-XX --label type-XX --body-file <填好的 .github/project-template/backlog.md>`（pri/area/type 必须显式贴）；Status/Estimate/Wave 在 Project v2 设；真源 = GitHub Issues + Project v2 #3，模版 + schema 全在 `.github/project-template/`（索引见其 `README.md`，治理见 `PROJECT.md`）
...
corecells/        — 平台 Cell 实现（accesscore / auditcore / configcore），每个 Cell 下含 slices/
...
archtest CI 入口是 `.github/workflows/archtest-nightly.yml`（schedule cron + workflow_dispatch，24-shard matrix）；PR / push CI 不跑完整 archtest suite（全 628+ tests），但 4 类核心 invariant 通过 `hack/verify-archtest-invariants.sh` 在 `governance.yml` 中 PR-time 执行（~30-60s 单进程，共享 SharedResolver）；完整 archtest 由 `archtest-nightly.yml` 兜底。本地反馈走开发者显式触发：`make verify` 一键全跑（含 archtest）或 `bash hack/verify-archtest.sh` 直跑；`hack/githooks/pre-push` 因 CPU/RSS 预算不跑 archtest（PR #887 round-2 撤回，详见 ADR §"pre-push archtest 撤回"）。`hack/verify-archtest.sh` 三种执行模式：`SHARD_TARGET=N` → 单 shard（CI matrix）；`SHARD_COUNT=1` → 单进程流式（local default）；`SHARD_COUNT>1` 无 `SHARD_TARGET` → 并行 fan-out。`go test ./tools/archtest/...` 行为不变。archtest 的 production scan 要求 GOWORK 处于激活状态（ModeWorkspace）；一旦卫星模块存在，禁止以 `GOWORK=off` 运行 archtest，否则卫星模块会从扫描范围中静默消失（追踪 gh #1590）。详见 ADR `docs/architecture/202605120000-adr-archtest-process-isolation.md` §Amendment 2026-05-23-pr-time-to-nightly + §Amendment 2026-05-28（K=16→24）。
