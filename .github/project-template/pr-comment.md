...
> - **kind→{phase,verdict,round} 派生规则单源 = `pr-meta.sh` 的 `derive_facts`（selftest golden-lock）**：phase 按 kind（ship/fix；ci=check；oos=review；pr-review 用 `--phase`）；verdict ship/fix/oos 固定、ci 由 `ci.failedChecks` 空否派生、pr-review 用 `--verdict`（**pr-review 的 phase/verdict 由 caller 显式传入承载 review 判断，非自动派生——与 ci verdict 从 facts 自动派生相区别**）；round ship=0 / fix=roundBase+1 / pr-review·ci·oos=roundBase（carry）。**不在本文档或任何 skill 重述此映射**（重述=漂移面，本 issue #1774 即为消除它）。
