...
#
# CORECELLS-CROSS-CELL-IMPORT-BAN-01 (TestCORECELLS_CROSS_CELL_IMPORT_BAN_01
# [+_FixtureMetaTest]) IS in this PR-time set: it guards the ADR PR-10a D1 premise
# "a corecells cell cannot import a sibling cell's Authorizer/package" (Soft→Medium,
# #1981). A new cross-cell Go import is a real architectural regression that must
# fail at PR-merge, not sit a day until nightly. Cheap here: a filesystem walk +
# go/parser ImportsOnly scan over corecells-cell-owned files (NO whole-tree
# packages.Load), same cost class as the *-FUNNEL-01 import scans above; it reuses
# the shared resolver's whole-tree warm-up and adds negligible wall-clock. The
# FixtureMetaTest is the anti-vacuity proof and runs against a t.TempDir() synthetic
# tree only.
#
