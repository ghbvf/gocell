...
| `verify-archtest-invariants.sh` | merged PR-time gate for PROD-CLOCK-INJECTION-01 + KERNEL-CLOCK-LEAF-FALLBACK + PROD-CLOCK-INJECTION-FIXTURES + PROD-DURATION-CONST-01 + PROD-DURATION-CONST-FIXTURES + TEST-TIME-LITERAL-01 + TEST-SLEEP-DISCIPLINE-01 + TEST-TIME-LITERAL-FIXTURES + PANIC-REGISTERED-01 + PANIC-REGISTERED-SCANNER-FIXTURES; runs 4 tests (~33s) via a single shared-resolver `go test` invocation |
