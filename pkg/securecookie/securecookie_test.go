...
func newTestSecureCookie(t *testing.T, hashKey, blockKey []byte, clk Clock, maxAge int) *SecureCookie {
	t.Helper()
	sc, err := New(Config{
		HashKey:  hashKey,
		BlockKey: blockKey,
...
func TestSecureCookie_SignOnly_RoundTrip(t *testing.T) {
	...
	sc := newTestSecureCookie(t, hashKey, nil, newFixedClock(), testMaxAge)
...
func TestSecureCookie_Encrypted_RoundTrip(t *testing.T) {
	...
	sc := newTestSecureCookie(t, hashKey, blockKey, newFixedClock(), testMaxAge)
...
func TestSecureCookie_Expired(t *testing.T) {
	hashKey := generateKey(t, 32)
	clk := newFixedClock()
	sc := newTestSecureCookie(t, hashKey, nil, clk, 1)
...
func TestSecureCookie_HashKeyTooShort(t *testing.T) {
	_, err := New(Config{HashKey: []byte("short"), Clock: newFixedClock(), MaxAge: testMaxAge})
	assert.ErrorIs(t, err, ErrHashKeyTooShort)
}

func TestSecureCookie_NilClock_ReturnsError(t *testing.T) {
	hashKey := generateKey(t, 32)
	_, err := New(Config{HashKey: hashKey, Clock: nil, MaxAge: testMaxAge})
...
func TestSecureCookie_TypedNilClock_ReturnsError(t *testing.T) {
	...
	_, err := New(Config{HashKey: hashKey, Clock: typedNil, MaxAge: testMaxAge})
	assert.ErrorIs(t, err, ErrClockRequired)
}

func TestSecureCookie_MaxAgeRequired(t *testing.T) {
	hashKey := generateKey(t, 32)
	_, err := New(Config{HashKey: hashKey, Clock: newFixedClock()})
	assert.ErrorIs(t, err, ErrMaxAgeRequired)
}

func TestSecureCookie_InvalidBlockKeyLength(t *testing.T) {
	hashKey := generateKey(t, 32)
	badBlockKey := generateKey(t, 15)
	_, err := New(Config{HashKey: hashKey, BlockKey: badBlockKey, Clock: newFixedClock(), MaxAge: testMaxAge})
	assert.Error(t, err)
	assert.True(t, strings.Contains(err.Error(), "blockKey"), "error should mention blockKey")
	assert.NotContains(t, err.Error(), "15")
}
...
func TestSecureCookie_Decode_MaliciousInput(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.True(t, errors.As(err, &ec), "decode input errors should use errcode")
			assert.Equal(t, errcode.KindInvalid, ec.Kind)
		})
	}
}

func TestSecureCookie_New_DeepCopyHashKey(t *testing.T) {
	hashKey := generateKey(t, 32)
	sc := newTestSecureCookie(t, hashKey, nil, newFixedClock(), testMaxAge)

	// Mutate original hashKey — should not affect sc.
	hashKey[0] ^= 0xFF
