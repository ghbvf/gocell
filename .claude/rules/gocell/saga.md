...
| SAGA-GLOBALREADER-CONFORMANCE-ENROLL-01 | 每个满足 `kernel/saga/journal.GlobalReader`（全局有序扫描窄接口，#1609 PR-02：`LoadSince`/`HeadSeq` + `Event.GlobalSeq`，不并入 `JournalCore`）的生产具名类型，其包下 `_test.go` 必须有 `sagajournaltest.RunGlobalReaderConformance` 调用且在 factory 闭包内构造该 impl（全局序单调 + 分页 + 边界 + 终态入序）；今日唯一实现 = `MemJournal`，PG GlobalReader deferred PR-PG (#1630) 落地后自动纳入。`SAGA-JOURNAL-CONFORMANCE-ENROLLMENT-01` 的读侧兄弟规则 | Medium（`types.Implements` 类型感知扫描 + `_test.go` 调用点 impl-level 解析；Hard 路径 = codegen golden 枚举实现，与 Journal enrollment **共用 gh #1003**，不另开） |
| SAGA-TAILER-CHECKPOINT-ADVANCER-CALLER-01（#1609 PR-04） | saga 投影 checkpoint advance（`projection.OwnerCheckpointStore.AdvanceIfOwner`）唯一生产 callsite = `runtime/saga/tailer.(*Tailer).commitEvent`（FuncLit 下降归属 enclosing FuncDecl）。caller-allowlist 漏斗，对齐 `RECONCILE-REQUEUE-ENQUEUE-CALLER-01` 范式 + RED fixture（`internal/sagataileradvancerfixture`）+ anti-vacuity。注：Tailer 持窄 `OwnerCheckpointStore`（无 `SaveOffset`），故「禁绕过 fencing 直写 offset」是 type-system Hard（方法不在依赖接口） | 下游 **Hard**（caller-allowlist，go/types `ResolveMethodCall`，alias 无效）+ 上游 **Medium**（Go 可见性天花板——`AdvanceIfOwner` 是导出接口方法，无法编译期强制单一 caller；Hard 化追踪 **gh #1612**，archtest godoc 点名） |
| SAGA-OWNER-CHECKPOINT-CONFORMANCE-ENROLL-01（#1609 PR-04） | 每个满足 `kernel/projection.OwnerCheckpointStore`（窄 fenced-checkpoint 接口 `LoadOffset`/`AdvanceIfOwner`，#1609 PR-04 D5(b)）的生产具名类型，其包下 `_test.go` 必须有 `projectiontest.RunOwnerCheckpointConformance` 调用（cold claim / same-owner re-advance / new-leader claim-ahead / stale-owner reject）；今日唯一实现 = `MemOwnerCheckpointStore`，PG impl deferred PR-PG 落地后自动纳入。RED fixture（`internal/sagaownercheckpointenrollfixture`）+ anti-vacuity | Medium（`types.Implements` 类型感知扫描 + `_test.go` 调用点 impl-level 解析；Hard 路径 = codegen golden 枚举实现，与 Journal enrollment **共用 gh #1003**） |
...
- EPIC #1609 投影源 ADR（**设计已立项，能力待 PR-02..06 落地**）：`docs/architecture/202606051200-1609-adr-saga-journal-projection-source.md`（`saga_events` model-a 投影源 / 独立 `Tailer` / `journal.GlobalReader` + `global_seq`）；三条新 enforcement——`PROJECTION-EVENT-CARRIER-TYPED-01`（PR-01）/ `SAGA-TAILER-CHECKPOINT-ADVANCER-CALLER-01`（PR-04，落 `saga_invariants_test.go`，受 `SAGA-INVARIANTS-FILE-CONSOLIDATED-01` 约束）/ `SAGA-GLOBALREADER-CONFORMANCE-ENROLL-01`（PR-02）——随各落地 PR 增量入上方 Archtest Invariants 表
