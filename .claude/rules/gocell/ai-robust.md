...
- **Medium**：保留；若有低成本升 Hard 的路径，开 gh issue 跟踪（`gh issue create --label backlog`）
- **Soft**：
  - 新引入 → 直接 reject，要求改 ≥ Medium
  - 既有 Soft 的补丁 → 优先讨论"升级到 Hard/Medium"，而非在 Soft 层打补丁
  - 允许暂留时，必须同步开 gh issue 跟踪升级（`gh issue create --label backlog`，不能 silent carryover）
...
Soft 上游 + Hard 下游不算闭环 funnel，按 Soft 处理。允许 Medium 上游（archtest caller allowlist）+ Hard 下游的过渡形态，但**必须同步开 gh issue 跟踪显式 Hard 化任务**（`gh issue create --label backlog`），并在 funnel 自身的 godoc / 测试注释中点名该 issue 号，让审查者能直接追到升级路径。
