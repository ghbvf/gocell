...
- `pkg/validation.IsNilInterface` 是 runtime/ 层唯一的 typed-nil 反射 helper（`ERROR-FIRST-TYPED-NIL-01` archtest 锁定识别 pattern），不得新建并行 helper。kernel/ 层因不依赖 pkg/，保留同义 file-local helper（`kernel/outbox.isNilEmitterDependency` / `kernel/clock.MustHaveClock` 内联反射 / `kernel/cell.IsNilHookObserver`）作为已知架构残留，不计入散点。
