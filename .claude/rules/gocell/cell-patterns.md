...
| 档 | 适用 | 位置 |
|----|------|------|
| A | 单 slice 自用 | `corecells/{cell}/slices/{slice}/` |
| B | 同 cell 多 slice 共享 | `corecells/{cell}/internal/dto/` 或 `internal/domain/` |
| C | 跨 cell wire 类型 | 禁止手写共享 Go 包，使用 contract schema 或 generated contract |

禁止把跨 cell 事件 DTO 放到 `pkg/`、`corecells/{cell}/events/`、
