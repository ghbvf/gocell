...
- `cellmodules/*` 任意文件（cellmodules Composition Root 层：`cellmodules/<cell>/module.go` 将平台 cell 绑定到 adapter，#1085 COMPOSITION-MODULE-API-01）
...
`kernel/governance` ADV-05 在 `gocell validate` 阶段对每个 `kind: event` 的 contract 建议（advisory）：
- 若 `lifecycle: active` 且订阅者集合（`endpoints.subscribers`，由 slice contractUsages[subscribe] + actorSubscribers 派生）为空，**SeverityWarning（advisory，不阻断 CI）** —— ADV-05 自 M3-RULE-ENGINE 起从 error 改为 warning，修正 advisory 系列错分（dead event 是需关注项，非阻断项）。finding 锚定 `lifecycle` 字段（`endpoints.subscribers` 是派生字段，不可手写）
...
- 订阅者可以是 cell（slice 声明 contractUsages[role=subscribe]）或 external actor（actors.yaml 注册 + contract.yaml actorSubscribers 引用）

典型修复路径（订阅者派生自下列，不直接编辑 `endpoints.subscribers`）：
- 真死事件 → `lifecycle: deprecated`
- 设计中尚未连线 → `lifecycle: draft`（待真实 producer/consumer 落地后转 active）
- 内部 fan-out 但还没 consumer → 添加订阅 slice（`contractUsages[role=subscribe]`）
- 对外 fan-out 给 SIEM/外部平台 → actors.yaml 注册 actor + contract.yaml `actorSubscribers` 引用 actor ID
