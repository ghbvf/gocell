...
- **typed function funnel for metrics instrument construction** — Two-ring funnel:
  - **Inner ring**: `adapters/prometheus/internal/promwrap.{NewCounter,NewCounterVec,NewGauge,NewGaugeFunc,NewGaugeVec,NewHistogramVec}` and `adapters/otel/internal/otelwrap.{Float64Counter,Float64Gauge,Float64Histogram}` are the sole sanctioned entries for Prometheus client and OTel meter instrument construction. **Hard upstream** via Go `internal/` compile-time closure (external imports inexpressible at type system level). **Hard downstream** via archtest `METRICS-GAUGEVEC-FUNNEL-01`: prom side prefix predicate `^New(Counter|Gauge|Histogram|Summary|Untyped)(Vec|Func)?$` resolved via `*types.Info.Uses` against `bannedPromPkg`; OTel side method-resolution against `bannedOtelMethods` set covering all synchronous Float64*/Int64* variants. RED fixture: 19 violations covering full ban surface including Int64 defensive bans. Surface drift locked by `TestMetricsFunnel_SymbolSentinel` (exact export-set equality on the two wrap packages).
  - **Outer ring**: `adapters/prometheus.{RegisterOrReuseCounter,NewCounter,NewCounterVec,NewGauge,NewGaugeFunc}` are public passthrough wrappers exposed to adapter-external callers that cannot reach internal/promwrap due to Go `internal/` closure. **Medium upstream** via archtest `METRICS-ADAPTERPROM-CALLER-ALLOWLIST-01` — caller-by-file hand-maintained allowlist (Go has no friend-package; functions cannot be sealed; this is the highest tier reachable for function-level funnels in the Go type system, parallels `panicregister.Approved` archtest-bound ceiling). **Hard downstream** via callee form-uniqueness on the five symbols. Surface drift locked by extending `TestMetricsFunnel_SymbolSentinel` to assert no new `New*`/`Register*` prefix exports appear in adapters/prometheus beyond the five funnel symbols. Long-term Hard upgrade path for outer ring: adapter-external callers migrate to kernel `metrics.Provider` so the outer ring collapses to inner ring only (Hard+Hard); tracked in the funnel's own archtest godoc per §"Funnel 双向锁评级".
  - Callee resolver walks both `*types.Info.Uses` and `*types.Info.Selections`; reverse blind-spot self-tests (BS-1 reflect, BS-3 function-value capture, BS-A1/BS-A2 outer-ring reflect/method-value) assert these forms are absent in production. Observable meter variants (`Int64ObservableUpDownCounter`, etc.) are intentionally excluded: callback registration pattern cannot be cleanly funneled synchronously, and their callsites in `adapters/otel/*.go` are legitimate implementation sites.
