...
**Registrar 共享（Option 3 #1152）**：cell attribution 的 resolver 与 adapter 的 grpc.Server delegation 是同一个 `ServiceRegistrar`——composition root 先 `runtimegrpc.NewServiceRegistrar()`，把 `reg.CellIDForMethod` 传 `interceptor.Deps.CellResolver`、把 `reg` 传 `adaptersgrpc.Config.Registrar`（必填，`New` 内 `BindServer` 绑定 grpc.Server）。`Deps.CellResolver` + `Deps.CellIDClosedSet` 均必填，`NewUnaryChain` 缺任一即 panic（fail-closed，不静默 `_runtime`）。
