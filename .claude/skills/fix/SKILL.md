...
5. **只修 IN_SCOPE + RELATED 条目**，按分析结果决策：
   - IN_SCOPE + C1 + 满足自动执行条件 → 直接修
   - IN_SCOPE + C2 → 执行推荐方案（最小或彻底，由时机判断决定）
   - IN_SCOPE + C3/C4 → 只输出方案，标注"需人工决策"
   - RELATED + C1/C2 → 搭车修，标注"搭车"
   - OUT_OF_SCOPE → 登记 backlog，标注推荐归入的 batch
6. 按复杂度从低到高排序执行：先批量处理 C1，再逐条处理 C2，最后汇总 C3/C4 方案
...
**何时跳过**: C1 全跳过；纯业务 bug 全跳过
**不可跳过**（即使 C2）: 并发/锁、连接池/生命周期、重连/重试/超时、密码学/认证、事件发布/消费
...
每个方案须含：改动范围、原理、优缺点、遗留（仅最小修复）、预估改动量、参考来源（C2+ 必填）。
...
| 复杂度 | 条件 | 决策 |
|--------|------|------|
| C1 + IN_SCOPE + ≤2文件 + 不改 kernel 接口/migration/bootstrap | 全满足 | **[AUTO-FIX]** 直接修 |
| C1 + 不满足上述 | — | 记录报告，不修 |
| C2 + IN_SCOPE + 能做 | — | 执行推荐方案 |
| C2 + 不能做（有前置依赖） | — | 记录报告，标注阻塞 |
| C3/C4 | 任何 | 只输出方案，标注"需人工决策" |
| 任何 + OUT_OF_SCOPE | — | 记录 backlog，不修 |

**不可自动执行**: 并发语义变更、接口签名修改、新依赖、数据流方向变更、C2+。
...
- 单条 C1 IN_SCOPE → 跳过清单直接修；批量或 C2+ → 必须创建
...
| 4 | 回滚（`git checkout -- <文件>`），C1 标 ESCALATE，C2 降级到最小修复标遗留 |
