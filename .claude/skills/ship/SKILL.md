...
6. **切触发 label**：`gh pr edit` 切 `pr-status/needs-review-again`（移除 `pr-status/in-progress`）——OOS artifact（步骤 5）已先落地、pm:ship 的 OOS 指针不悬空，此刻 review-side 执行器可立即开始，无需等待 CI。
