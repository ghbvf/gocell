...
5. 读取 specs/{branch}/product-context.md（席位 6 产品/UX Reviewer 必读）
...
增加检查维度: kernel/cells/runtime/adapters 变更是否有对标引用（commit message 或 handoff note 中含 ref: 标记）？
...
**阻塞**: 必须等待全部 6 个 Reviewer 返回。逐一确认每个席位产出了 findings（即使无发现，也需要席位明确回报"无发现"）。任何席位超时或无响应，总负责人重新派发该席位。
...
架构师裁决后：
- **接受**: 确认为真正的 P0，Phase 挂起，总负责人裁决是否拆分 Phase
- **驳回**: 确认不是 P0（误判），关闭 finding，流程继续

不允许降级为 P1 或 tech-debt——P0 意味着阻塞合并，要么解决要么停下来。
...
```
[ ] 无 P0 遗留 [AGENT]
[ ] review-findings.md 已写且含审查基准版本（commit hash） [GATE]
[ ] tech-debt.md 已写且每条含 [TECH] 或 [PRODUCT] 标签 [GATE]
[ ] build + test 绿 [AGENT]
[ ] phase-gate-check.py --stage S6 --branch {branch} --check exit = PASS [GATE]
```
