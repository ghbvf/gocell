...
func TestService_HandleEvent(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "empty object rejected (eventId/type required)", payload: `{}`, wantErr: true},
		{name: "missing eventId rejected", payload: `{"type":"ping"}`, wantErr: true},
		{name: "missing type rejected", payload: `{"eventId":"evt-4"}`, wantErr: true},
		{name: "empty-string eventId rejected", payload: `{"eventId":"","type":"ping"}`, wantErr: true},
