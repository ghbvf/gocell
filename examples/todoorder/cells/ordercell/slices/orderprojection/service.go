// Package orderprojection implements the order-projection slice (L3):
// subscribes to order-created and order-status-changed events, maintains an
// in-memory "by-status" read model (OrderStatusSummary), and supports a
// business-level rebuild from the event log.
//
// This slice demonstrates CQRS projection: the command side (orderconfirm)
// emits events; this slice consumes them asynchronously and projects a
// derived view that the write-side orders map cannot cheaply provide.
//
// # Demo note
//
