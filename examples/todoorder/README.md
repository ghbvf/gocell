...
4. **Rebuild** — rebuild is framework-owned (`projection.Coordinator.Rebuild`),
   exposed as the **operator** control-plane endpoint `POST
   /admin/v1/projection/ordercell/order_status/rebuild` (mounted by bootstrap on
   the loopback `cell.AdminListener` via `WithProjectionRebuildEndpoint()`, #1505).
