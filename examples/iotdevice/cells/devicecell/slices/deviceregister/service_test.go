...
func TestService_Register_PublishFails_StillReturnsDevice(t *testing.T) {
	repo := mem.NewDeviceRepository()
	emitter, err := outbox.NewDirectEmitter(failPublisher{}, outbox.DirectPublishFailOpen, metrics.NopProvider{}, slog.Default())
	require.NoError(t, err)
...
func TestService_Register_PublishFails_FailClosedReturnsError(t *testing.T) {
	repo := mem.NewDeviceRepository()
	emitter, err := outbox.NewDirectEmitter(failPublisher{}, outbox.DirectPublishFailClosed, metrics.NopProvider{}, slog.Default())
	require.NoError(t, err)
	svc := NewService(repo, slog.Default(), WithEmitter(emitter))
...
func TestService_Register_FailOpenDoesNotLogPublished(t *testing.T) {
	...
	emitter, err := outbox.NewDirectEmitter(failPublisher{}, outbox.DirectPublishFailOpen, metrics.NopProvider{}, logger)
	require.NoError(t, err)
