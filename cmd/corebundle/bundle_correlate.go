...
// is nil the caller must fail-fast: auditcore is a fixed corebundle cell, so a
// missing AuditQueryStore export is a composition wiring bug, not a tolerable
// degradation (no soft fallback / silent endpoint disable).
