...
// carried inside PoolResource.Worker() — PoolResource wraps only the pool.
func TestBuildConfigCoreOpts_PGMode_ManagedResourceNonNil(t *testing.T) {
	...
	require.NotNil(t, result.PoolResource, "postgres mode must return a non-nil ManagedResource (pool)")
	assert.NotEmpty(t, result.CellOptions, "postgres mode must return cell options")

	// PoolResource.Worker() must be nil — pool has no background worker.
	assert.Nil(t, result.PoolResource.Worker(), "PoolResource must not carry a relay worker; relay is registered via bootstrapOpts")
	...
	require.NoError(t, result.PoolResource.Close(context.Background()))
}
