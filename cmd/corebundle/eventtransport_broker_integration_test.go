...
// Package main — #1940 "complete switch" e2e: in postgres topology the event
// transport must be a REAL broker, not the in-memory bus.
package main
...
// TestIntegration_EventTransport_PostgresResolvesLiveRabbitMQ proves that in
// postgres topology eventtransport.Resolve dials a REAL RabbitMQ broker and
// returns a live publisher/subscriber transport (the #1940 "complete switch":
// no in-memory bus in postgres mode). It verifies the wiring beyond compile-time:
//
...
// eventtransport seam (Resolve → live broker transport) + the corebundle topology
// contract, which is exactly the surface #1940 added.
func TestIntegration_EventTransport_PostgresResolvesLiveRabbitMQ(t *testing.T) {
	...
	tr, err := eventtransport.Resolve(clock.Real(), mkTopo("real", "postgres", true), eventtransport.Config{
		AMQPURL: amqpURL,
	})
