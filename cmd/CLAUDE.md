...
| `GOCELL_HTTP_DEVICE_MTLS_ADDR` | device-mTLS(renew) listener 绑定地址（`POST /api/v1/deviceidentity/renew`，设备使用 mTLS 续期证书）。生产须设可路由地址或置 TLS 终止代理前端 | 默认 `127.0.0.1:9092`（loopback）|
