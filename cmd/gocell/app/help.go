...
func printGenerateHelp() error {
	printHelp("generate", []helpEntry{
		...
		{"contract", []string{
			...
			"+ JSON schemas for every opted-in (codegen: true) contract.",
			"<contractID> | --all [--dry-run | --verify].",
			"--verify reports drift without writing; --dry-run prints",
			...
			"  gocell generate contract http.order.create.v1",
			"",
			"  # Generate all opted-in contracts:",
			"  gocell generate contract --all",
			"",
			"  # Diff mode: fail on drift, no write (used in CI):",
			"  gocell generate contract --all --verify",
			"",
			"  # Preview: print would-write paths, no write:",
			"  gocell generate contract --all --dry-run",
		}},
