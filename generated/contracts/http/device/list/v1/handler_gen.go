...
func NewHandler(svc Service, policy auth.Policy) *Handler {
	if policy == nil {
		...
		panic(errcode.Assertion("generated handler http.device.list.v1: policy must not be nil (non-public, non-bootstrap, non-clientsOnly endpoints require a real auth.Policy; for public/clients-only endpoints update contract.yaml auth flag and regenerate)"))
	}
