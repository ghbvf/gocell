...
func NewHandler(svc Service, policy auth.Policy) *Handler {
	...
	if err != nil {
		panic(panicregister.Approved("http-device-command-ack-v1-schema-compile-failed", errcode.Assertion("generated handler http.device.command.ack.v1: schema compile failed: %v (codegen invariant violation; regenerate via gocell generate contract --all)", err)))
	}
