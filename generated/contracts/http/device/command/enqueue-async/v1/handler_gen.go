...
var contractSpec = contractspec.ContractSpec{
	ID:        "http.device.command.enqueue-async.v1",
	Kind:      cellvocab.ContractHTTP,
	Transport: "http",
	Method:    "POST",
	Path:      "/api/v1/devices/{id}/async-commands",
}
...
var requestSchemaJSON = []byte("{\"$schema\":\"https://json-schema.org/draft/2020-12/schema\",\"title\":\"http.device.command.enqueue-async.v1.request\",\"type\":\"object\",\"properties\":{\"commandType\":{\"type\":\"string\",\"minLength\":1,\"maxLength\":64},\"payload\":{\"type\":\"string\",\"minLength\":1,\"maxLength\":4096}},\"required\":[\"payload\"],\"additionalProperties\":false}")

// Handler wires HTTP decode/encode + auth.Mount for http.device.command.enqueue-async.v1.
type Handler struct {
	svc              Service
	policy           auth.Policy
...
// NewHandler creates a Handler for http.device.command.enqueue-async.v1.
//
// Authorization is contract-derived (#2205/#2355): endpoints.http.permission is
// device:command. NewHandler takes the cell-level
...
func NewHandler(svc Service, resolver authz.MethodPolicyResolver) *Handler {
	if resolver == nil {
		...
		panic(panicregister.Approved("http-device-command-enqueue-async-v1-resolver-nil", errcode.Assertion("generated handler http.device.command.enqueue-async.v1: resolver must not be nil (endpoints.http.permission contracts require the cell authz.MethodPolicyResolver; composition root must inject the cellgen-built resolver)")))
	}
	...
	if err != nil {
		panic(panicregister.Approved("http-device-command-enqueue-async-v1-standard-schema-compile-failed", errcode.Assertion("generated handler http.device.command.enqueue-async.v1: schema compile failed: %v (codegen invariant violation; regenerate via gocell generate contract --all)", err)))
	}
...
func (h *Handler) handle(w http.ResponseWriter, r *http.Request) {
	...
	if visitErr := resp.visitEnqueueAsyncResponse(r.Context(), w); visitErr != nil {
