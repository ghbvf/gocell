...
// device:consume, owner-scoped on path param id. NewHandler takes the cell-level
...
func NewHandler(svc Service, resolver authz.MethodPolicyResolver) *Handler {
	if resolver == nil {
		...
		panic(panicregister.Approved("http-device-command-extend-lease-v1-resolver-nil", errcode.Assertion("generated handler http.device.command.extend-lease.v1: resolver must not be nil (endpoints.http.permission contracts require the cell authz.MethodPolicyResolver; composition root must inject the cellgen-built resolver)")))
	}
	...
	if err != nil {
		panic(panicregister.Approved("http-device-command-extend-lease-v1-standard-schema-compile-failed", errcode.Assertion("generated handler http.device.command.extend-lease.v1: schema compile failed: %v (codegen invariant violation; regenerate via gocell generate contract --all)", err)))
	}
...
func (h *Handler) handle(w http.ResponseWriter, r *http.Request) {
	...
	if visitErr := resp.visitExtendLeaseResponse(r.Context(), w); visitErr != nil {
