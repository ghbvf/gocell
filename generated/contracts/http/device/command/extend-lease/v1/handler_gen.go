...
func (h *Handler) handle(w http.ResponseWriter, r *http.Request) {
	...
	}
	{
		v := r.PathValue("cmdId")
		if len(v) < 1 {
			httputil.WriteError(r.Context(), w, errcode.New(errcode.KindInvalid, errcode.ErrValidationFailed, "cmdId: value too short"))
			return
		}
		if len(v) > 256 {
			httputil.WriteError(r.Context(), w, errcode.New(errcode.KindInvalid, errcode.ErrValidationFailed, "cmdId: value too long"))
			return
		}
		req.CmdId = v
	}
	...
	if req.ExtensionSeconds < 1 {
		httputil.WriteError(r.Context(), w, errcode.New(errcode.KindInvalid, errcode.ErrValidationFailed, "extensionSeconds: value below minimum"))
		return
	}
	if req.ExtensionSeconds > 3600 {
		httputil.WriteError(r.Context(), w, errcode.New(errcode.KindInvalid, errcode.ErrValidationFailed, "extensionSeconds: value above maximum"))
		return
	}
