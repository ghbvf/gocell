...
func NewHandler(svc Service) *Handler {
	...
	if err != nil {
		panic(errcode.Assertion("generated handler http.device.register.v1: schema compile failed: %v (codegen invariant violation; regenerate via gocell generate contract --all)", err))
	}
...
func (h *Handler) handle(w http.ResponseWriter, r *http.Request) {
	...
	if resp == nil {
		...
		panic(errcode.Assertion("generated handler http.device.register.v1: service returned nil response without error (typed envelope contract violation)"))
	}
	...
	if visitErr := resp.visitRegisterResponse(r.Context(), w); visitErr != nil {
		slog.ErrorContext(r.Context(), "http.device.register.v1: typed response visit failed",
