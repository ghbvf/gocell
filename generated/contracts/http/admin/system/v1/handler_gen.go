...
// Handler wires HTTP decode/encode + auth.Mount for http.admin.system.v1.
type Handler struct {
	svc    Service
	policy auth.Policy
}

// NewHandler creates a Handler for http.admin.system.v1.
//
// Authorization is contract-derived (#2205/#2355): endpoints.http.permission is
// system:read. NewHandler takes the cell-level
...
func NewHandler(svc Service, resolver authz.MethodPolicyResolver) *Handler {
	if resolver == nil {
		...
		panic(panicregister.Approved("http-admin-system-v1-resolver-nil", errcode.Assertion("generated handler http.admin.system.v1: resolver must not be nil (endpoints.http.permission contracts require the cell authz.MethodPolicyResolver; composition root must inject the cellgen-built resolver)")))
	}
...
func (h *Handler) handle(w http.ResponseWriter, r *http.Request) {
	req := &Request{}
	resp, err := h.svc.System(r.Context(), req)
	if err != nil {
	...
	if visitErr := resp.visitSystemResponse(r.Context(), w); visitErr != nil {
