...
var contractSpec = contractspec.ContractSpec{
	ID:        "http.order.projection-rebuild.v1",
...
var requestSchemaJSON = []byte("{\"$schema\":\"https://json-schema.org/draft/2020-12/schema\",\"title\":\"http.order.projection-rebuild.v1.request\",\"type\":\"object\",\"properties\":{},\"additionalProperties\":false}")

// Handler wires HTTP decode/encode + auth.Mount for http.order.projection-rebuild.v1.
type Handler struct {
	svc              Service
	policy           auth.Policy
...
// NewHandler creates a Handler for http.order.projection-rebuild.v1.
func NewHandler(svc Service, policy auth.Policy) *Handler {
	if policy == nil {
		...
		panic(panicregister.Approved("http-order-projection-rebuild-v1-policy-nil", errcode.Assertion("generated handler http.order.projection-rebuild.v1: policy must not be nil (non-public, non-bootstrap, non-clientsOnly, non-serviceOwned endpoints require a real auth.Policy; for public/clients-only/service-owned endpoints update contract.yaml auth flag and regenerate)")))
	}
	h := &Handler{svc: svc, policy: policy}
	v, err := schemavalidate.NewValidator(requestSchemaJSON)
	if err != nil {
		panic(panicregister.Approved("http-order-projection-rebuild-v1-standard-schema-compile-failed", errcode.Assertion("generated handler http.order.projection-rebuild.v1: schema compile failed: %v (codegen invariant violation; regenerate via gocell generate contract --all)", err)))
	}
