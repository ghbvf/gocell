...
var contractSpec = contractspec.ContractSpec{
	ID:        "http.registry.contract.reject.v1",
	Kind:      cellvocab.ContractHTTP,
	Transport: "http",
	Method:    "POST",
	Path:      "/api/v1/registry/contracts/{id}/reject",
}
...
var requestSchemaJSON = []byte("{\"$schema\":\"https://json-schema.org/draft/2020-12/schema\",\"title\":\"http.registry.contract.reject.v1.request\",\"description\":\"Reject a probing or pending-approval contract registration. The id is the path parameter; the body carries an optional free-form rejection reason (recorded on the registration's append-only migration event as the transition Reason).\",\"type\":\"object\",\"additionalProperties\":false,\"properties\":{\"reason\":{\"type\":\"string\",\"maxLength\":1024,\"description\":\"Optional free-form rejection reason recorded as the transition Reason.\"}}}")

// Handler wires HTTP decode/encode + auth.Mount for http.registry.contract.reject.v1.
type Handler struct {
	svc              Service
	policy           auth.Policy
...
// NewHandler creates a Handler for http.registry.contract.reject.v1.
//
// Authorization is contract-derived (#2205/#2355): endpoints.http.permission is
// registry:reject. NewHandler takes the cell-level
...
func NewHandler(svc Service, resolver authz.MethodPolicyResolver) *Handler {
	if resolver == nil {
		...
		panic(panicregister.Approved("http-registry-contract-reject-v1-resolver-nil", errcode.Assertion("generated handler http.registry.contract.reject.v1: resolver must not be nil (endpoints.http.permission contracts require the cell authz.MethodPolicyResolver; composition root must inject the cellgen-built resolver)")))
	}
	...
	if err != nil {
		panic(panicregister.Approved("http-registry-contract-reject-v1-standard-schema-compile-failed", errcode.Assertion("generated handler http.registry.contract.reject.v1: schema compile failed: %v (codegen invariant violation; regenerate via gocell generate contract --all)", err)))
	}
...
func (h *Handler) handle(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := h.svc.Reject(r.Context(), req)
	if err != nil {
		httputil.WriteError(r.Context(), w, err)
		return
	}
	...
	if visitErr := resp.visitRejectResponse(r.Context(), w); visitErr != nil {
