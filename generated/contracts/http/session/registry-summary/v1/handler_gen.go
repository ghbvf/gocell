...
var contractSpec = contractspec.ContractSpec{
	ID:        "http.session.registry-summary.v1",
	Kind:      cellvocab.ContractHTTP,
	Transport: "http",
	Method:    "GET",
	Path:      "/api/v1/access/sessions/registry-summary",
}

// Handler wires HTTP decode/encode + auth.Mount for http.session.registry-summary.v1.
type Handler struct {
	svc    Service
	policy auth.Policy
}

// NewHandler creates a Handler for http.session.registry-summary.v1.
...
func (h *Handler) handle(w http.ResponseWriter, r *http.Request) {
	req := &Request{}
	resp, err := h.svc.RegistrySummary(r.Context(), req)
	if err != nil {
	...
	if visitErr := resp.visitRegistrySummaryResponse(r.Context(), w); visitErr != nil {
