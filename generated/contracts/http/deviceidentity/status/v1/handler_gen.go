...
var contractSpec = contractspec.ContractSpec{
	ID:        "http.deviceidentity.status.v1",
	Kind:      cellvocab.ContractHTTP,
	Transport: "http",
	Method:    "GET",
	Path:      "/api/v1/deviceidentity/status/{deviceId}",
}

// Handler wires HTTP decode/encode + auth.Mount for http.deviceidentity.status.v1.
type Handler struct {
	svc    Service
	policy auth.Policy
}

// NewHandler creates a Handler for http.deviceidentity.status.v1.
