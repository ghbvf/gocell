...
// Handler wires HTTP decode/encode + auth.Mount for http.audit.get.v1.
type Handler struct {
	svc    Service
	policy auth.Policy
}

// NewHandler creates a Handler for http.audit.get.v1.
//
// Authorization is contract-derived (#2205/#2355): endpoints.http.permission is
// audit:read. NewHandler takes the cell-level
...
func NewHandler(svc Service, resolver authz.MethodPolicyResolver) *Handler {
	if resolver == nil {
		...
		panic(panicregister.Approved("http-audit-get-v1-resolver-nil", errcode.Assertion("generated handler http.audit.get.v1: resolver must not be nil (endpoints.http.permission contracts require the cell authz.MethodPolicyResolver; composition root must inject the cellgen-built resolver)")))
	}
