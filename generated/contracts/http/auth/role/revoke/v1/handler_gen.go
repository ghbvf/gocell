...
func NewHandler(svc Service, policy auth.Policy) *Handler {
	...
	if err != nil {
		panic(fmt.Sprintf("generated handler http.auth.role.revoke.v1: schema compile failed: %v (codegen invariant violation; regenerate via gocell generate contract --all)", err))
	}
