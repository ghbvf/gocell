...
var requestSchemaJSON = []byte("{\"$schema\":\"https://json-schema.org/draft/2020-12/schema\",\"title\":\"http.auth.role.assign.v1.request\",\"type\":\"object\",\"properties\":{\"userId\":{\"type\":\"string\",\"minLength\":1,\"maxLength\":64},\"roleId\":{\"type\":\"string\",\"minLength\":1,\"maxLength\":64}},\"required\":[\"userId\",\"roleId\"],\"additionalProperties\":false}")
...
func NewHandler(svc Service, policy auth.Policy) *Handler {
	if policy == nil {
		...
		panic(errcode.Assertion("generated handler http.auth.role.assign.v1: policy must not be nil (non-public, non-bootstrap, non-clientsOnly, non-serviceOwned endpoints require a real auth.Policy; for public/clients-only/service-owned endpoints update contract.yaml auth flag and regenerate)"))
	}
	h := &Handler{svc: svc, policy: policy}
	v, err := schemavalidate.NewValidator(requestSchemaJSON)
	if err != nil {
		panic(errcode.Assertion("generated handler http.auth.role.assign.v1: schema compile failed: %v (codegen invariant violation; regenerate via gocell generate contract --all)", err))
	}
...
func (h *Handler) handle(w http.ResponseWriter, r *http.Request) {
	...
	if visitErr := resp.visitAssignResponse(r.Context(), w); visitErr != nil {
