...
// role:read, owner-scoped on path param userID. NewHandler takes the cell-level
...
func NewHandler(svc Service, resolver authz.MethodPolicyResolver) *Handler {
	if resolver == nil {
		...
		panic(panicregister.Approved("http-auth-role-check-v1-resolver-nil", errcode.Assertion("generated handler http.auth.role.check.v1: resolver must not be nil (endpoints.http.permission contracts require the cell authz.MethodPolicyResolver; composition root must inject the cellgen-built resolver)")))
	}
...
func (h *Handler) handle(w http.ResponseWriter, r *http.Request) {
	...
	if visitErr := resp.visitCheckResponse(r.Context(), w); visitErr != nil {
