...
func NewHandler(svc Service, bootstrapAuth func(http.Handler) http.Handler) *Handler {
	if bootstrapAuth == nil {
		...
		panic(panicregister.Approved("http-auth-setup-admin-v1-bootstrap-auth-nil", errcode.Assertion("generated handler http.auth.setup.admin.v1: bootstrapAuth must not be nil (auth.bootstrap:true contracts require a per-route replacement authenticator; composition root must inject runtime/auth.NewBootstrapMiddleware)")))
	}
	h := &Handler{svc: svc, bootstrapAuth: bootstrapAuth}
	v, err := schemavalidate.NewValidator(requestSchemaJSON)
	if err != nil {
		panic(panicregister.Approved("http-auth-setup-admin-v1-bootstrap-schema-compile-failed", errcode.Assertion("generated handler http.auth.setup.admin.v1: schema compile failed: %v (codegen invariant violation; regenerate via gocell generate contract --all)", err)))
	}
