...
func NewHandler(svc Service, bootstrapAuth func(http.Handler) http.Handler) *Handler {
	...
	if err != nil {
		panic(panicregister.Approved("http-auth-setup-admin-v1-schema-compile-failed", errcode.Assertion("generated handler http.auth.setup.admin.v1: schema compile failed: %v (codegen invariant violation; regenerate via gocell generate contract --all)", err)))
	}
