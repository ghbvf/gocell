...
var contractSpec = contractspec.ContractSpec{
	ID:         "http.auth.decide.v1",
	Kind:       cellvocab.ContractHTTP,
	Transport:  "http",
	Method:     "POST",
	Path:       "/api/v1/access/decide",
	SelfScoped: true,
}
...
var requestSchemaJSON = []byte("{\"$schema\":\"https://json-schema.org/draft/2020-12/schema\",\"title\":\"http.auth.decide.v1.request\",\"type\":\"object\",\"properties\":{\"action\":{\"type\":\"string\",\"minLength\":1,\"maxLength\":64,\"description\":\"The action to check, as a registered permission name (e.g. \\\"audit:read\\\", \\\"user:read\\\"). An unregistered action is rejected with 400. The decision subject is the authenticated caller (JWT) — this request carries no subject.\"},\"resource\":{\"type\":\"string\",\"maxLength\":256,\"description\":\"Optional target resource id forwarded to the PDP for ownership-scoped actions (e.g. the user id for \\\"user:read\\\"). Omit for coarse action checks. NOTE: omitting it is NOT \\\"check self\\\" — an absent resource makes the ownership condition (subject.sub == resource.id) unsatisfiable, so owner-scoped actions evaluate as if querying an unknown resource.\"}},\"required\":[\"action\"],\"additionalProperties\":false}")

// Handler wires HTTP decode/encode + auth.Mount for http.auth.decide.v1.
type Handler struct {
	svc              Service
	policy           auth.Policy
...
// NewHandler creates a Handler for http.auth.decide.v1.
//
// Authorization is contract-derived (#2205/#2355): endpoints.http.permission is
// access:decide, self-scoped (caller's own subject). NewHandler takes the cell-level
...
func NewHandler(svc Service, resolver authz.MethodPolicyResolver) *Handler {
	if resolver == nil {
		...
		panic(panicregister.Approved("http-auth-decide-v1-resolver-nil", errcode.Assertion("generated handler http.auth.decide.v1: resolver must not be nil (endpoints.http.permission contracts require the cell authz.MethodPolicyResolver; composition root must inject the cellgen-built resolver)")))
	}
	...
	if err != nil {
		panic(panicregister.Approved("http-auth-decide-v1-standard-schema-compile-failed", errcode.Assertion("generated handler http.auth.decide.v1: schema compile failed: %v (codegen invariant violation; regenerate via gocell generate contract --all)", err)))
	}
...
func (h *Handler) handle(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := h.svc.Decide(r.Context(), req)
	if err != nil {
		httputil.WriteError(r.Context(), w, err)
		return
	}
	...
	if visitErr := resp.visitDecideResponse(r.Context(), w); visitErr != nil {
