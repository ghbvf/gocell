...
var requestSchemaJSON = []byte("{\"$schema\":\"https://json-schema.org/draft/2020-12/schema\",\"additionalProperties\":false,\"properties\":{\"expectedVersion\":{\"description\":\"Compare-and-swap guard. Must equal the resource's current version; mismatch returns 409 ERR_VERSION_CONFLICT.\",\"maximum\":99999,\"minimum\":1,\"type\":\"integer\"},\"version\":{\"maximum\":99999,\"minimum\":1,\"type\":\"integer\"}},\"required\":[\"version\",\"expectedVersion\"],\"title\":\"http.config.rollback.v1.request\",\"type\":\"object\"}")
...
// NewHandler creates a Handler for http.config.rollback.v1.
//
...
func NewHandler(svc Service, resolver authz.MethodPolicyResolver) *Handler {
	if resolver == nil {
		...
		panic(panicregister.Approved("http-config-rollback-v1-resolver-nil", errcode.Assertion("generated handler http.config.rollback.v1: resolver must not be nil (endpoints.http.permission contracts require the cell authz.MethodPolicyResolver; composition root must inject the cellgen-built resolver)")))
	}
	...
	if err != nil {
		panic(panicregister.Approved("http-config-rollback-v1-standard-schema-compile-failed", errcode.Assertion("generated handler http.config.rollback.v1: schema compile failed: %v (codegen invariant violation; regenerate via gocell generate contract --all)", err)))
	}
...
func (h *Handler) handle(w http.ResponseWriter, r *http.Request) {
	...
	if visitErr := resp.visitRollbackResponse(r.Context(), w); visitErr != nil {
