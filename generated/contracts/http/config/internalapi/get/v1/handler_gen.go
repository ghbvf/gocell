...
var requestSchemaJSON = []byte("{\"$schema\":\"https://json-schema.org/draft/2020-12/schema\",\"title\":\"http.config.internal.get.v1.request\",\"description\":\"GET endpoint — no request body. Key is passed via path parameter.\",\"type\":\"object\",\"additionalProperties\":false}")
...
// NewHandler creates a Handler for http.config.internal.get.v1.
...
func NewHandler(svc Service, policy auth.Policy) *Handler {
	...
	if err != nil {
		panic(fmt.Sprintf("generated handler http.config.internal.get.v1: schema compile failed: %v (codegen invariant violation; regenerate via gocell generate contract --all)", err))
	}
