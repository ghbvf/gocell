...
var requestSchemaJSON = []byte("{\"$schema\":\"https://json-schema.org/draft/2020-12/schema\",\"title\":\"http.config.flags.list.v1.request\",\"description\":\"GET endpoint — no request body.\",\"type\":\"object\",\"additionalProperties\":false}")

// Handler wires HTTP decode/encode + auth.Mount for http.config.flags.list.v1.
type Handler struct {
	svc              Service
	policy           auth.Policy
...
// NewHandler creates a Handler for http.config.flags.list.v1.
...
func NewHandler(svc Service, policy auth.Policy) *Handler {
	...
	if err != nil {
		panic(fmt.Sprintf("generated handler http.config.flags.list.v1: schema compile failed: %v (codegen invariant violation; regenerate via gocell generate contract --all)", err))
	}
