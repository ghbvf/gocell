...
var requestSchemaJSON = []byte("{\"$schema\":\"https://json-schema.org/draft/2020-12/schema\",\"title\":\"http.config.flags.create.v1.request\",\"type\":\"object\",\"required\":[\"key\"],\"properties\":{\"key\":{\"type\":\"string\",\"minLength\":1,\"maxLength\":128},\"enabled\":{\"type\":\"boolean\"},\"rolloutPercentage\":{\"type\":\"integer\",\"minimum\":0,\"maximum\":100},\"description\":{\"type\":\"string\",\"maxLength\":1024}},\"additionalProperties\":false}")
...
func NewHandler(svc Service, resolver authz.MethodPolicyResolver) *Handler {
	if resolver == nil {
		...
		panic(panicregister.Approved("http-config-flags-create-v1-resolver-nil", errcode.Assertion("generated handler http.config.flags.create.v1: resolver must not be nil (endpoints.http.permission contracts require the cell authz.MethodPolicyResolver; composition root must inject the cellgen-built resolver)")))
	}
	...
	if err != nil {
		panic(panicregister.Approved("http-config-flags-create-v1-standard-schema-compile-failed", errcode.Assertion("generated handler http.config.flags.create.v1: schema compile failed: %v (codegen invariant violation; regenerate via gocell generate contract --all)", err)))
	}
