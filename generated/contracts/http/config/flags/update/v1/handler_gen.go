...
var requestSchemaJSON = []byte("{\"$schema\":\"https://json-schema.org/draft/2020-12/schema\",\"title\":\"http.config.flags.update.v1.request\",\"description\":\"PUT = full replacement. All three fields are required so callers cannot implicitly zero-write a field they did not intend to change. Use POST /{key}/toggle for the partial 'just flip enabled' workflow.\",\"type\":\"object\",\"properties\":{\"enabled\":{\"type\":\"boolean\"},\"rolloutPercentage\":{\"type\":\"integer\",\"minimum\":0,\"maximum\":100},\"description\":{\"type\":\"string\",\"minLength\":0,\"maxLength\":1024},\"expectedVersion\":{\"type\":\"integer\",\"minimum\":1,\"maximum\":99999,\"description\":\"Compare-and-swap guard. Must equal the flag's current version; mismatch returns 409 ERR_VERSION_CONFLICT.\"}},\"required\":[\"enabled\",\"rolloutPercentage\",\"description\",\"expectedVersion\"],\"additionalProperties\":false}")
...
func NewHandler(svc Service, policy auth.Policy) *Handler {
	if policy == nil {
		...
		panic(errcode.Assertion("generated handler http.config.flags.update.v1: policy must not be nil (non-public, non-bootstrap, non-clientsOnly, non-serviceOwned endpoints require a real auth.Policy; for public/clients-only/service-owned endpoints update contract.yaml auth flag and regenerate)"))
	}
	h := &Handler{svc: svc, policy: policy}
	v, err := schemavalidate.NewValidator(requestSchemaJSON)
	if err != nil {
		panic(errcode.Assertion("generated handler http.config.flags.update.v1: schema compile failed: %v (codegen invariant violation; regenerate via gocell generate contract --all)", err))
	}
