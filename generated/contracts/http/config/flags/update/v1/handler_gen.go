...
var requestSchemaJSON = []byte("{\"$schema\":\"https://json-schema.org/draft/2020-12/schema\",\"additionalProperties\":false,\"description\":\"PUT = full replacement. All three fields are required so callers cannot implicitly zero-write a field they did not intend to change. Use POST /{key}/toggle for the partial 'just flip enabled' workflow.\",\"properties\":{\"description\":{\"maxLength\":1024,\"type\":\"string\"},\"enabled\":{\"type\":\"boolean\"},\"expectedVersion\":{\"description\":\"Compare-and-swap guard. Must equal the resource's current version; mismatch returns 409 ERR_VERSION_CONFLICT.\",\"maximum\":99999,\"minimum\":1,\"type\":\"integer\"},\"rolloutPercentage\":{\"maximum\":100,\"minimum\":0,\"type\":\"integer\"}},\"required\":[\"enabled\",\"rolloutPercentage\",\"description\",\"expectedVersion\"],\"title\":\"http.config.flags.update.v1.request\",\"type\":\"object\"}")
...
// flag:write. NewHandler takes the cell-level
...
func NewHandler(svc Service, resolver authz.MethodPolicyResolver) *Handler {
	if resolver == nil {
		...
		panic(panicregister.Approved("http-config-flags-update-v1-resolver-nil", errcode.Assertion("generated handler http.config.flags.update.v1: resolver must not be nil (endpoints.http.permission contracts require the cell authz.MethodPolicyResolver; composition root must inject the cellgen-built resolver)")))
	}
	...
	if err != nil {
		panic(panicregister.Approved("http-config-flags-update-v1-standard-schema-compile-failed", errcode.Assertion("generated handler http.config.flags.update.v1: schema compile failed: %v (codegen invariant violation; regenerate via gocell generate contract --all)", err)))
	}
