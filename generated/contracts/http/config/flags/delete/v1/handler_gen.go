...
func NewHandler(svc Service, resolver authz.MethodPolicyResolver) *Handler {
	if resolver == nil {
		...
		panic(panicregister.Approved("http-config-flags-delete-v1-resolver-nil", errcode.Assertion("generated handler http.config.flags.delete.v1: resolver must not be nil (endpoints.http.permission contracts require the cell authz.MethodPolicyResolver; composition root must inject the cellgen-built resolver)")))
	}
...

	{
		...
		if rawExpectedVersion == "" {
			httputil.WriteError(r.Context(), w, errcode.New(errcode.KindInvalid, errcode.ErrValidationFailed,
				...
				errcode.WithDetails(errcode.PublicString("field", "expectedVersion"))))
			return
		}
		...
		if err != nil {
			httputil.WriteError(r.Context(), w, errcode.New(errcode.KindInvalid, errcode.ErrValidationFailed,
				"validation: invalid request parameter",
				errcode.WithDetails(errcode.PublicString("field", "expectedVersion"), errcode.PublicString("reason", "must be an integer"))))
			return
		}
		if v < 1 {
			httputil.WriteError(r.Context(), w, errcode.New(errcode.KindInvalid, errcode.ErrValidationFailed,
				"validation: invalid request parameter",
				errcode.WithDetails(errcode.PublicString("field", "expectedVersion"), errcode.PublicString("reason", "invalid"))))
			return
		}
		if v > 99999 {
			httputil.WriteError(r.Context(), w, errcode.New(errcode.KindInvalid, errcode.ErrValidationFailed,
