...
// Handler wires HTTP decode/encode + auth.Mount for http.config.get.v1.
type Handler struct {
	svc    Service
	policy auth.Policy
}

// NewHandler creates a Handler for http.config.get.v1.
//
...
func NewHandler(svc Service, resolver authz.MethodPolicyResolver) *Handler {
	if resolver == nil {
		...
		panic(panicregister.Approved("http-config-get-v1-resolver-nil", errcode.Assertion("generated handler http.config.get.v1: resolver must not be nil (endpoints.http.permission contracts require the cell authz.MethodPolicyResolver; composition root must inject the cellgen-built resolver)")))
	}
