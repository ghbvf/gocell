...
func NewHandler(svc Service, resolver authz.MethodPolicyResolver) *Handler {
	if resolver == nil {
		...
		panic(panicregister.Approved("http-config-update-v1-resolver-nil", errcode.Assertion("generated handler http.config.update.v1: resolver must not be nil (endpoints.http.permission contracts require the cell authz.MethodPolicyResolver; composition root must inject the cellgen-built resolver)")))
	}
	...
	if err != nil {
		panic(panicregister.Approved("http-config-update-v1-standard-schema-compile-failed", errcode.Assertion("generated handler http.config.update.v1: schema compile failed: %v (codegen invariant violation; regenerate via gocell generate contract --all)", err)))
	}
