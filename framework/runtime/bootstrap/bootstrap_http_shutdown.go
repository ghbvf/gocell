...
// and the listener's shutGrace setting. The parent is the drainCtx threaded
// in by phase10OrchestrateShutdown via shutdownAllServers (stage 1+2 budget;
// see phases_shutdown.go and ADR 202605101730-adr-shutdown-budget-decouple).
//
...
//     parent.Deadline() and (now + shutGrace) comes first, so the drainCtx
//     budget always wins when shutGrace exceeds it (R2-03).
//   - shutGrace == 0 returns the parent unchanged (no per-listener override; the
//     server inherits the drainCtx budget). The returned cancel is a no-op so
...
// shutdownAllServers drains all servers in parallel. ctx is the drainCtx
// passed in by phase10OrchestrateShutdown (stage 1+2 budget). Each task uses
// a per-server context derived from this parent:
...
//     upper bound within the drainCtx budget.
//   - when shutGrace == 0, the parent drainCtx is passed through unchanged.
//
