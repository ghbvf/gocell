...
// # Usage
//
//	agg := healthz.NewAggregator(clock.Real())
//	agg.Register(healthz.NewProbe("postgres_ready", pool.Ping))
//	snap := agg.Evaluate(ctx)
//
// NewAggregator requires a clock as its first positional parameter and panics
// without one; pass clock.Real() in production or clockmock.New(...) in tests.
//
// # Conformance harness
//
