...
import (
	...
	"github.com/ghbvf/gocell/framework/pkg/csvparam"
...
// cell-owned routes. Built via contractbuild.NewFrameworkHTTP — the only
...
var specCatalog = contractbuild.NewFrameworkHTTP(
...
func writeValidationError(ctx context.Context, w http.ResponseWriter, detail string) {
	httputil.WriteError(ctx, w, errcode.New(
		...
		errcode.WithDetails(errcode.PublicString("detail", detail)),
	))
}
