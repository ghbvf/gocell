...
func TestCheckHTTPResponseAlignment_GeneratedHandlerInternalSegment(t *testing.T) {
	t.Parallel()
	root := t.TempDir()
	const contractID = "http.internal.foo.v1"

	genDir := filepath.Join(root, "generated", "contracts", "http", "internalapi", "foo", "v1")
...
`
	require.NoError(t, os.WriteFile(filepath.Join(genDir, "handler_gen.go"), []byte(content), 0o644))

	project := makeCodegenProject(contractID)
	c := makeContract(contractID, "contracts/http/internal/foo/v1/contract.yaml", nil)
	c.Codegen = true
	c.Endpoints.HTTP.Path = "/internal/v1/foo"
	project.Contracts[contractID] = c

	results := NewValidator(project, root, clock.Real()).checkCH04()

	require.NotEmpty(t, results,
		"CH-04 must resolve codegen handlers through internal->internalapi and report undeclared 400")
	assert.Equal(t, codeCH04, results[0].Code)
	assert.Contains(t, results[0].Message, "400")
}
...
func writeUUIDHandlerIfNeeded(t *testing.T, sliceAbsDir, contractID, handlerSrc string, noHandler, noAuthMount bool) {
	...
	if noAuthMount {
		// Write handler src without auth.Mount boilerplate to test fail-closed.
		path := filepath.Join(sliceAbsDir, "handler.go")
		content := "package x\n\nimport \"net/http\"\nimport \"github.com/ghbvf/gocell/framework/pkg/httputil\"\n\n" + handlerSrc
		require.NoError(t, os.WriteFile(path, []byte(content), 0o644))
	} else {
...
func TestCheckHTTPPathParamUUID_GeneratedHandlerInternalSegment(t *testing.T) {
	t.Parallel()
	root := t.TempDir()
	const contractID = "http.internal.uuid.v1"

	genDir := filepath.Join(root, "generated", "contracts", "http", "internalapi", "uuid", "v1")
	require.NoError(t, os.MkdirAll(genDir, 0o755))
	content := `// Code generated by gocell generate contract. DO NOT EDIT.
...
`
	...
	project := makeCodegenProject(contractID)
	c := makeUUIDContract(contractID, "contracts/http/internal/uuid/v1/contract.yaml", []string{"id"})
	c.Codegen = true
	c.Endpoints.HTTP.Path = "/internal/v1/uuid/{id}"
	project.Contracts[contractID] = c

	results := NewValidator(project, root, clock.Real()).checkCH05()

	require.NotEmpty(t, results,
		"CH-05 must resolve codegen handlers through internal->internalapi and inspect UUID params")
	assert.Equal(t, codeCH05, results[0].Code)
	assert.Contains(t, results[0].Message, `"id"`)
}
