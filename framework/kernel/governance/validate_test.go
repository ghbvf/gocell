...
func TestFMT41(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "vacuous entry (public:false, no permission) rejected — asserts no non-default",
		...
		{
			name:      "valid permission overlay (#2008) — passes",
			methods:   []metadata.GRPCMethodMeta{{Name: "Verify", Permission: "policy:read"}},
			codegen:   true,
			wantCount: 0,
		},
		{
			name:      "public ⊕ permission mutex (#2008) rejected — both set",
			methods:   []metadata.GRPCMethodMeta{{Name: "Verify", Public: true, Permission: "policy:read"}},
			codegen:   true,
			wantCount: 1,
			wantIssue: IssueInvalid,
		},
		{
			name:      "unknown permission (#2008) rejected — not in closed authz registry",
			methods:   []metadata.GRPCMethodMeta{{Name: "Verify", Permission: "bogus:action"}},
			codegen:   true,
			wantCount: 1,
			wantIssue: IssueInvalid,
		},
