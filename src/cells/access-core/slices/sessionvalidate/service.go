...
// Service validates JWT access tokens.
type Service struct {
	signingKey []byte
	logger     *slog.Logger
}
...
func NewService(signingKey []byte, logger *slog.Logger) *Service {
	return &Service{signingKey: signingKey, logger: logger}
}

// Verify validates the token string and returns decoded Claims.
func (s *Service) Verify(_ context.Context, tokenStr string) (auth.Claims, error) {
...
func IssueTestToken(signingKey []byte, subject string, roles []string, ttl time.Duration) (string, error) {
	now := time.Now()
