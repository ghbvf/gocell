...
// and optional AES-GCM encryption using Go standard library crypto only.
...
// NewSecureCookie creates a SecureCookie with the given hash key and optional
// block key. hashKey is required (min 32 bytes). blockKey may be nil
// (signing only) or 16/24/32 bytes (AES-128/192/256-GCM).
...
// HMAC input: name | timestamp | nonce | payload
func (sc *SecureCookie) Encode(name string, value []byte) (string, error) {
	...
	// 3. HMAC over (name | timestamp | nonce | payload)
...
func (sc *SecureCookie) Decode(name string, encoded string) ([]byte, error) {
	...
	if sc.aead != nil {
		minLen += nonceLen // nonce is present when encrypted
	}
	if len(raw) < minLen {
