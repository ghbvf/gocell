...
func TestService_List_FirstPage(t *testing.T) {
	...
	// configSort = key ASC, id ASC: items must be returned in ascending key order.
	for i := 1; i < len(result.Items); i++ {
		assert.LessOrEqual(t, result.Items[i-1].Key, result.Items[i].Key,
			"items must be ordered by key ASC (configSort); got %s before %s",
			result.Items[i-1].Key, result.Items[i].Key)
	}
}
