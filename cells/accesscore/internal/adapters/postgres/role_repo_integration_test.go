...
func TestCountEffectiveAdmins_LockedAdminExcluded_PG(t *testing.T) {
	...
	require.NoError(t, txMgr.RunInTx(ctx, func(txCtx context.Context) error {
		_, err := pgexec.ExecDirect(pgrepoapproved.Approve(pgrepoapproved.IntegrationTestLockUser), roleRepo.db, txCtx, "UPDATE users SET status = 'locked' WHERE id = $1", lockedAdmin.ID)
		return err
	}))
...
func TestRemoveFromUserIfNotLast_LockedPeerDoesNotCount_PG(t *testing.T) {
	...
	require.NoError(t, txMgr.RunInTx(ctx, func(txCtx context.Context) error {
		_, err := pgexec.ExecDirect(pgrepoapproved.Approve(pgrepoapproved.IntegrationTestLockUser), roleRepo.db, txCtx, "UPDATE users SET status = 'locked' WHERE id = $1", lockedPeer.ID)
		return err
	}))
...
func TestPGRoleRepo_EffectiveAdminExists_PG(t *testing.T) {
	...
	t.Run("locked_peer_still_true_when_other_active_admin_remains", func(t *testing.T) {
		...
		// stays active).
		require.NoError(t, txMgr.RunInTx(ctx, func(txCtx context.Context) error {
			_, err := pgexec.ExecDirect(pgrepoapproved.Approve(pgrepoapproved.IntegrationTestLockUser), roleRepo.db, txCtx,
...
func TestLastAdminTrigger_ConcurrentCascadeDelete_Serialized(t *testing.T) {
	...
	for _, userID := range []string{u1.ID, u2.ID} {
		userID := userID
		go func() {
			_, execErr := pgexec.ExecDirect(pgrepoapproved.Approve(pgrepoapproved.IntegrationTestDeleteUser), roleRepo.db, ctx, "DELETE FROM users WHERE id = $1", userID)
			results <- execErr
		}()
	}
