...
// inaccessible to corecells/auditcore by the IMPL-DECL-COVER-01 rule.
//
...
// for durable assemblies lives in corecells/auditcore/cell.go::initSlices, which
...
func (s *Service) HandleEvent(ctx context.Context, entry outbox.Entry) outbox.HandleResult {
	...
	if err := audit.AppendBootstrapAuthFail(ctx, s.bootstrapStore, s.clk, entry.ID(), payload.Reason, payload.ClientIPHash); err != nil {
		...
		// 409 to DLX. Mirrors corecells/auditcore/internal/appender.
