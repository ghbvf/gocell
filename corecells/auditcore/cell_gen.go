...
// Source: corecells/auditcore/cell.yaml
...
import (
	...
	sub3 "github.com/ghbvf/gocell/generated/contracts/event/config/rollback/v1"
	sub4 "github.com/ghbvf/gocell/generated/contracts/event/config/version-published/v1"
	sub5 "github.com/ghbvf/gocell/generated/contracts/event/role/assigned/v1"
	sub6 "github.com/ghbvf/gocell/generated/contracts/event/role/revoked/v1"
	sub7 "github.com/ghbvf/gocell/generated/contracts/event/session/created/v1"
	sub8 "github.com/ghbvf/gocell/generated/contracts/event/session/revoked/v1"
	sub9 "github.com/ghbvf/gocell/generated/contracts/event/user/created/v1"
	sub10 "github.com/ghbvf/gocell/generated/contracts/event/user/deleted/v1"
...
func (c *AuditCore) Init(ctx context.Context, reg cell.Registrar) error {
	...
	if err := sub0.NewSubscription(c.appendBootstrapSvc.HandleEvent, "auditcore", "auditcore", "auditappendbootstrap").Mount(reg); err != nil {
		return fmt.Errorf("auditcore: subscribe event.auth.bootstrap-failed.v1: %w", err)
	}

	if err := sub1.NewSubscription(c.appendConfigSvc.HandleEvent, "auditcore", "auditcore", "auditappendconfig").Mount(reg); err != nil {
		return fmt.Errorf("auditcore: subscribe event.config.entry-deleted.v1: %w", err)
	}

	if err := sub2.NewSubscription(c.appendConfigSvc.HandleEvent, "auditcore", "auditcore", "auditappendconfig").Mount(reg); err != nil {
		return fmt.Errorf("auditcore: subscribe event.config.entry-upserted.v1: %w", err)
	}

	if err := sub3.NewSubscription(c.appendConfigSvc.HandleEvent, "auditcore", "auditcore", "auditappendconfig").Mount(reg); err != nil {
		return fmt.Errorf("auditcore: subscribe event.config.rollback.v1: %w", err)
	}

	if err := sub4.NewSubscription(c.appendConfigSvc.HandleEvent, "auditcore", "auditcore", "auditappendconfig").Mount(reg); err != nil {
		return fmt.Errorf("auditcore: subscribe event.config.version-published.v1: %w", err)
	}

	if err := sub5.NewSubscription(c.appendRoleSvc.HandleEvent, "auditcore", "auditcore", "auditappendrole").Mount(reg); err != nil {
		return fmt.Errorf("auditcore: subscribe event.role.assigned.v1: %w", err)
	}

	if err := sub6.NewSubscription(c.appendRoleSvc.HandleEvent, "auditcore", "auditcore", "auditappendrole").Mount(reg); err != nil {
		return fmt.Errorf("auditcore: subscribe event.role.revoked.v1: %w", err)
	}

	if err := sub7.NewSubscription(c.appendSessionSvc.HandleEvent, "auditcore", "auditcore", "auditappendsession").Mount(reg); err != nil {
		return fmt.Errorf("auditcore: subscribe event.session.created.v1: %w", err)
	}

	if err := sub8.NewSubscription(c.appendSessionSvc.HandleEvent, "auditcore", "auditcore", "auditappendsession").Mount(reg); err != nil {
		return fmt.Errorf("auditcore: subscribe event.session.revoked.v1: %w", err)
	}

	if err := sub9.NewSubscription(c.appendUserSvc.HandleEvent, "auditcore", "auditcore", "auditappenduser").Mount(reg); err != nil {
		return fmt.Errorf("auditcore: subscribe event.user.created.v1: %w", err)
	}

	if err := sub10.NewSubscription(c.appendUserSvc.HandleEvent, "auditcore", "auditcore", "auditappenduser").Mount(reg); err != nil {
		return fmt.Errorf("auditcore: subscribe event.user.deleted.v1: %w", err)
	}

	if err := sub11.NewSubscription(c.appendUserSvc.HandleEvent, "auditcore", "auditcore", "auditappenduser").Mount(reg); err != nil {
		return fmt.Errorf("auditcore: subscribe event.user.locked.v1: %w", err)
	}

	if err := sub12.NewSubscription(c.appendUserSvc.HandleEvent, "auditcore", "auditcore", "auditappenduser").Mount(reg); err != nil {
		return fmt.Errorf("auditcore: subscribe event.user.unlocked.v1: %w", err)
	}

	if err := sub13.NewSubscription(c.appendUserSvc.HandleEvent, "auditcore", "auditcore", "auditappenduser").Mount(reg); err != nil {
