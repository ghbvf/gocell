...
var testTenantID = func() tenant.TenantID {
	...
	if err != nil {
		panic("rbaccheck_test: invalid testTenantID: " + err.Error())
	}
	return t
}()
...
// testAuthContext creates an auth test context with the canonical test tenant injected.
func testAuthContext(userID string, roles []string) context.Context {
...
func TestHttpAuthRoleListV1Serve(t *testing.T) {
	...
	req2 = req2.WithContext(withAllowAuthorizer(testAuthContext(testutil.TestID("user-1"), nil)))
	...
	req400 = req400.WithContext(withAllowAuthorizer(testAuthContext(testutil.TestID("user-1"), nil)))
	...
	reqBadCursor = reqBadCursor.WithContext(withAllowAuthorizer(testAuthContext(testutil.TestID("user-1"), nil)))
