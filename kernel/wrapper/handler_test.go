...
func TestHTTPHandler_EmitsSpanWithContractAttrs(t *testing.T) {
	...
	spec := loginSpec()
	h := wrapper.HTTPHandler(tr, spec, okHandler(http.StatusOK))
...
func TestHTTPHandler_MarksErrorOn5xx(t *testing.T) {
	tr := &spyTracer{}
	h := wrapper.HTTPHandler(tr, loginSpec(), okHandler(http.StatusInternalServerError))
...
func TestHTTPHandler_DoesNotMarkError_On4xx(t *testing.T) {
	tr := &spyTracer{}
	h := wrapper.HTTPHandler(tr, loginSpec(), okHandler(http.StatusBadRequest))
	req := httptest.NewRequest("POST", "/api/v1/auth/login", nil)
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
...
func TestHTTPHandler_CapturesDefault200WhenHandlerWritesNoStatus(t *testing.T) {
	...
	h := wrapper.HTTPHandler(tr, loginSpec(), http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte("hi"))
	}))
	req := httptest.NewRequest("POST", "/api/v1/auth/login", nil)
	rec := httptest.NewRecorder()
...
func TestHTTPHandler_PanicsOnInvalidSpec(t *testing.T) {
	...
	for _, spec := range cases {
		func(s wrapper.ContractSpec) {
			...
			_ = wrapper.HTTPHandler(wrapper.NoopTracer{}, s, okHandler(200))
		}(spec)
	}
}
...
func TestHTTPHandler_PanicsOnNilHandler(t *testing.T) {
	...
	_ = wrapper.HTTPHandler(wrapper.NoopTracer{}, loginSpec(), nil)
}
...
// TestHTTPHandler_NilTracer_FallsBackToNoop verifies the nil→NoopTracer
// fallback: a Router constructed without WithTracer still serves requests
// correctly (spans are silently discarded).
func TestHTTPHandler_NilTracer_FallsBackToNoop(t *testing.T) {
	t.Parallel()
	h := wrapper.HTTPHandler(nil, loginSpec(), okHandler(200))
	req := httptest.NewRequest("POST", "/api/v1/auth/login", nil)
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
	if rec.Code != 200 {
...
// TestHTTPHandler_ExplicitNoopTracer_DoesNotPanic parallels the above but with
// an explicit NoopTracer{} to cover the normal typed-noop path.
func TestHTTPHandler_ExplicitNoopTracer_DoesNotPanic(t *testing.T) {
	t.Parallel()
	h := wrapper.HTTPHandler(wrapper.NoopTracer{}, loginSpec(), okHandler(200))
	req := httptest.NewRequest("POST", "/api/v1/auth/login", nil)
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
	if rec.Code != 200 {
...
func TestHTTPHandler_ConcurrentRequests_UniqueSpans(t *testing.T) {
	tr := &spyTracer{}
	h := wrapper.HTTPHandler(tr, loginSpec(), okHandler(200))
	const N = 50
...
func TestStatusRecorder_ConcurrentRequests_NoRace(t *testing.T) {
	...
	for i := 0; i < N; i++ {
		...
		go func() {
			defer wg.Done()
			status := 200 + (i % 10)
			h := wrapper.HTTPHandler(tr, loginSpec(), okHandler(status))
			req := httptest.NewRequest("POST", "/api/v1/auth/login", nil)
			h.ServeHTTP(httptest.NewRecorder(), req)
		}()
	}
...
// TestTracerFromCarrier verifies the optional TracerCarrier pattern: when a
// mux-like type implements WrapperTracer() it is pulled; otherwise the helper
// falls back to NoopTracer{}.
func TestTracerFromCarrier(t *testing.T) {
	t.Parallel()
	tr := &spyTracer{}
	carrier := fakeCarrier{tr: tr}
	if got := wrapper.TracerFromCarrier(carrier); got != tr {
		t.Errorf("TracerFromCarrier: got %T, want spyTracer", got)
	}
	// Non-carrier value returns NoopTracer.
	got := wrapper.TracerFromCarrier(struct{}{})
	if _, ok := got.(wrapper.NoopTracer); !ok {
		t.Errorf("TracerFromCarrier non-carrier: got %T, want NoopTracer", got)
	}
	// Carrier returning nil tracer also falls back to NoopTracer.
	got = wrapper.TracerFromCarrier(fakeCarrier{tr: nil})
	if _, ok := got.(wrapper.NoopTracer); !ok {
		t.Errorf("TracerFromCarrier nil-tracer carrier: got %T, want NoopTracer", got)
	}
}

type fakeCarrier struct{ tr wrapper.Tracer }

func (c fakeCarrier) WrapperTracer() wrapper.Tracer { return c.tr }
