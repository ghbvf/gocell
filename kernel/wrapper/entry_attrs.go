...
// OccurredAt.
//
// State is read through the value-receiver getters Entry.Principal() and
// Entry.OccurredAt() — Entry is a sealed type (issue #1229) whose fields are
// unexported, so there is no struct-literal / field-read path.
//
...
//     returns true for "gocell.principal.session_id" (dotted-segment match on
//     the session_id key), so the value is unconditionally replaced with Mask
//     before reaching the OTel backend. Session IDs are credential-associated
//     identifiers whose leakage to trace backends carries session-hijack risk.
//     We emit the raw value here as a normal string attr; the masking is the
//     OTel boundary's responsibility, not this builder's.
...
// value so the adapter takes the OTel native int64 constructor path and
// bypasses the free-form string redactor entirely (a string-encoded timestamp
// would be needlessly scanned by RedactString).
//
...
func entryEnvelopeAttrs(entry outbox.Entry) []Attr {
	...
	if id := string(p.SubjectID); id != "" {
		attrs = append(attrs, Attr{Key: "gocell.principal.subject_id", Value: id})
	}
	if id := string(p.TenantID); id != "" {
		attrs = append(attrs, Attr{Key: "gocell.principal.tenant_id", Value: id})
	}
	if id := string(p.SessionID); id != "" {
		attrs = append(attrs, Attr{Key: "gocell.principal.session_id", Value: id})
	}
	if occurredAt := entry.OccurredAt(); !occurredAt.IsZero() {
		// int64 value (not string): keeps the adapter on the OTel native
		// int64 path, away from the free-form string redactor.
		attrs = append(attrs, Attr{Key: "gocell.event.occurred_at_unix_nano", Value: occurredAt.UnixNano()})
	}
	return attrs
}
