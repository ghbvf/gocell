...
func TestSignatureMatchesValidationResultEmitter_CrossPackageRejected(t *testing.T) {
	...
	// Signature: (x X) emit(s string) targetpkg.ValidationResult
	recv := types.NewVar(token.NoPos, pkgConsumer, "x", xNamed)
	p0 := types.NewVar(token.NoPos, pkgConsumer, "s", types.Typ[types.String])
	r0 := types.NewVar(token.NoPos, pkgConsumer, "", vrNamed)
...
// satisfies all three predicates (arg0 string, return ValidationResult,
// same-package owner). Together with the Rejected case, these two tests
// pin the guard at the function boundary, independent of BFS plumbing.
func TestSignatureMatchesValidationResultEmitter_SamePackageAccepted(t *testing.T) {
	...
	p0 := types.NewVar(token.NoPos, pkg, "s", types.Typ[types.String])
