...
// --- CODEGEN-01: routeMount listener-reference integrity ---

func TestCODEGEN01(t *testing.T) {
	tests := []struct {
		name      string
		setup     func(*metadata.ProjectMeta)
		wantCount int
		...
		{
			name:      "no routeMounts → 0 findings",
			setup:     func(_ *metadata.ProjectMeta) {},
			wantCount: 0,
		},
		{
			name: "routeMount references declared listener → 0 findings",
			setup: func(pm *metadata.ProjectMeta) {
				cell := pm.Cells["accesscore"]
				cell.Listeners = []metadata.ListenerDeclMeta{
					{Ref: "cell.PrimaryListener", Prefix: "/api/v1"},
				}
				pm.Slices["accesscore/session-login"].RouteMounts = []metadata.RouteMountMeta{
					{Listener: "cell.PrimaryListener", SubPath: "/sessions", HandlerField: "loginHandler"},
				}
			},
			wantCount: 0,
		},
		{
			name: "routeMount references undeclared listener → 1 error on slice.yaml",
			setup: func(pm *metadata.ProjectMeta) {
				// Cell declares only PrimaryListener; slice references InternalListener.
				cell := pm.Cells["accesscore"]
				cell.Listeners = []metadata.ListenerDeclMeta{
					{Ref: "cell.PrimaryListener", Prefix: "/api/v1"},
				}
				slc := pm.Slices["accesscore/session-login"]
				slc.RouteMounts = []metadata.RouteMountMeta{
					{Listener: "cell.InternalListener", SubPath: "/admin", HandlerField: "adminHandler"},
				}
			},
			wantCount: 1,
			wantField: "routeMounts[0].listener",
			wantInMsg: "cell.InternalListener",
		},
		{
			name: "routeMount references listener on cell with no listeners declared → 1 error",
			setup: func(pm *metadata.ProjectMeta) {
				slc := pm.Slices["accesscore/session-login"]
				slc.RouteMounts = []metadata.RouteMountMeta{
					{Listener: "cell.PrimaryListener", SubPath: "/x", HandlerField: "h"},
				}
				// cell.Listeners is left empty
			},
			wantCount: 1,
			wantField: "routeMounts[0].listener",
			wantInMsg: "undeclared listener",
		},
		{
			name: "slice with unknown belongsToCell is silently skipped (REF-* owns that error)",
			setup: func(pm *metadata.ProjectMeta) {
				pm.Slices["ghost/orphan"] = &metadata.SliceMeta{
					ID: "orphan", BelongsToCell: "ghost", Dir: "orphan",
					File: "cells/ghost/slices/orphan/slice.yaml",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.setup(pm)
			val := NewValidator(pm, "", clock.Real())
			got := findByCode(val.validateCODEGEN01(), "CODEGEN-01")
			assert.Len(t, got, tc.wantCount, "want %d CODEGEN-01 results, got %d: %v", tc.wantCount, len(got), got)
			if tc.wantCount > 0 && len(got) > 0 {
				assert.Equal(t, tc.wantField, got[0].Field, "field path")
				assert.Contains(t, got[0].Message, tc.wantInMsg, "message must mention %q", tc.wantInMsg)
			}
		})
	}
}
