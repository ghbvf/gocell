...
// --- deriveSubscribeVerifyContract tests ---

func TestDeriveSubscribeVerifyContract_Derives(t *testing.T) {
	sl := &SliceMeta{
		ID:            "auditingest",
		BelongsToCell: "auditcore",
		ContractUsages: []ContractUsage{
	...
	pm := buildSubscribeProject(
		map[string]*SliceMeta{"auditcore/auditingest": sl},
		map[string]*ContractMeta{},
	)

	deriveSubscribeVerifyContract(pm)

	assert.Equal(t, []string{"contract.event.session.created.v1.subscribe"}, sl.Verify.Contract)
}

func TestDeriveSubscribeVerifyContract_DedupedWhenAlreadyPresent(t *testing.T) {
	// slice.yaml already declares the verify entry — no duplicate
	sl := &SliceMeta{
		ID:            "auditingest",
		BelongsToCell: "auditcore",
		ContractUsages: []ContractUsage{
	...
	assert.Equal(t, []string{"contract.event.session.created.v1.subscribe"}, sl.Verify.Contract,
		"dedup must keep first occurrence only")
}

func TestDeriveSubscribeVerifyContract_NonSubscribeRoleIgnored(t *testing.T) {
	sl := &SliceMeta{
		ID:            "sessionlogin",
		BelongsToCell: "accesscore",
		ContractUsages: []ContractUsage{
	...
	pm := buildSubscribeProject(
		map[string]*SliceMeta{"accesscore/sessionlogin": sl},
		map[string]*ContractMeta{},
	)

	deriveSubscribeVerifyContract(pm)

	assert.Empty(t, sl.Verify.Contract, "non-subscribe roles must not generate verify entries")
}
