...
// WithContractAttrs returns a new context carrying a pre-built slice of
// contract-derived span attributes (gocell.contract.id / kind / transport +
// http.method/route or messaging.system/destination).
//
// wrapper.HTTPHandler writes the slice at the moment a request enters the
// contract-bound handler chain. The outer HTTP Tracing middleware reads it
// after next.ServeHTTP returns and late-binds the attributes onto the
// single request-owned span — achieving "one request → one span" on
