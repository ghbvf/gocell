...
// RunConformance runs the Registry conformance suite against the supplied
// factory. factory must return a fresh, independent Registry pre-loaded with
// the given defs on each call. emptyFactory must return a Registry with no
...
//	        func() saga.Registry { reg, _ := saga.NewInMemoryRegistry(defs...); return reg },
//	        func() saga.Registry { reg, _ := saga.NewInMemoryRegistry(); return reg },
//	        defs...,
//	    )
//	}
func RunConformance(t *testing.T, factory func() saga.Registry, emptyFactory func() saga.Registry, defs ...*saga.Definition) {
	t.Helper()

	cases := []struct {
		name string
		run  func(*testing.T, func() saga.Registry, func() saga.Registry, []*saga.Definition)
	}{
...
func conformLookupRegisteredID(t *testing.T, factory func() saga.Registry, _ func() saga.Registry, defs []*saga.Definition) {
	t.Helper()
	if len(defs) == 0 {
...
func conformLookupUnregisteredID(t *testing.T, factory func() saga.Registry, _ func() saga.Registry, _ []*saga.Definition) {
	t.Helper()
	reg := factory()
...
func conformLookupEmptyID(t *testing.T, factory func() saga.Registry, _ func() saga.Registry, _ []*saga.Definition) {
	t.Helper()
	reg := factory()
...
func conformLookupConcurrent(t *testing.T, factory func() saga.Registry, _ func() saga.Registry, defs []*saga.Definition) {
	t.Helper()
	reg := factory()
...
// conformLookupNilDefs asserts that a Registry constructed with no definitions
// returns (nil, false) for any Lookup call.
func conformLookupNilDefs(t *testing.T, _ func() saga.Registry, emptyFactory func() saga.Registry, _ []*saga.Definition) {
	t.Helper()
	reg := emptyFactory()
	got, ok := reg.Lookup(idutil.SafeID("saga-any"))
	if ok {
		t.Error("empty Registry Lookup: expected ok=false, got true")
	}
	if got != nil {
		t.Errorf("empty Registry Lookup: expected nil, got %v", got)
	}
}
