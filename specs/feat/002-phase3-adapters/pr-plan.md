...
|----|-------|-------|---------|--------|--------|
| PR-01 | Bootstrap 接口化 + outbox doc 增强 | T01, T02 | — | `go build ./runtime/... && go test ./runtime/bootstrap/...` | merged `feat/phase3/w0-bootstrap-refactor` |
| PR-02 | UUID 替换 + seed 脚本 + Wave 0 回归 | T03, T04, T05 | PR-01 | `go test ./...` (全量回归) | merged `feat/phase3/w0-uuid-seed-regression` |
...
| PR | scope | tasks | depends | verify | branch |
|----|-------|-------|---------|--------|--------|
| PR-03 | PostgreSQL adapter 基础 | T10, T11, T12, T13, T14, T15 | PR-02 | `go test ./adapters/postgres/... -count=1` (mock pgx) | merged `feat/phase3/w1-postgres-base` |
| PR-04 | Redis adapter | T16, T17, T18, T19, T20 | PR-02 | `go test ./adapters/redis/... -count=1` (mock redis) | merged `feat/phase3/w1-redis` |
| PR-05 | RabbitMQ adapter | T21, T22, T23, T24, T25, T26 | PR-02 | `go test ./adapters/rabbitmq/... -count=1` (mock amqp) | merged `feat/phase3/w1-rabbitmq` |
| PR-06 | DevOps: Docker Compose + Makefile + healthcheck | T06, T07, T08, T09 | PR-02 | `docker compose config --quiet && bash scripts/healthcheck-verify.sh` | merged `feat/phase3/w1-devops` |
...
| PR | scope | tasks | depends | verify | branch |
|----|-------|-------|---------|--------|--------|
| PR-07 | PostgreSQL Outbox Writer + Relay + 单测 | T27, T28, T29 | PR-03, PR-05 | `go test ./adapters/postgres/... -run Outbox -count=1` | merged `feat/phase3/w2-outbox-chain` |
| PR-08 | OIDC adapter | T30, T31, T32, T33, T34 | PR-02 | `go test ./adapters/oidc/... -count=1` (mock http) | merged `feat/phase3/w2-oidc` |
| PR-09 | S3 adapter（含 KS-10 间接层） | T35, T36, T37, T38 | PR-02 | `go test ./adapters/s3/... -count=1` + `grep -r "gocell/cells" adapters/s3/ \| wc -l` (须为 0) | merged `feat/phase3/w2-s3` |
| PR-10 | WebSocket adapter + Cell PG Repo + S3 ArchiveStore + 单测 | T39, T40, T41, T42, T43, T44, T45, T46 | PR-03, PR-09 | `go test ./adapters/websocket/... ./cells/.../adapters/... -count=1` | merged `feat/phase3/w2-ws-repo` |
...
| PR | scope | tasks | depends | verify | branch |
|----|-------|-------|---------|--------|--------|
| PR-11 | Cell Outbox 重构 + cmd/core-bundle 接线 | T47, T48, T49, T50 | PR-07, PR-01 | `go build ./cells/... ./cmd/core-bundle/... && go test ./cells/...` | merged `feat/phase3/w3-cell-outbox-rewire` |
| PR-12 | 安全加固（8 条 FR-9） | T51, T52, T53, T54, T55, T56, T57, T58 | PR-02 | `go test ./... -run TestSec -count=1` | merged `feat/phase3/w3-security` |
| PR-13 | Tech-Debt P0+P1 + 产品修复 | T59, T60, T61, T62, T63, T64, T65, T66, T67, T68, T69 | PR-07 (T63), PR-02 (其余) | `go build ./... && go test ./... -count=1` | merged `feat/phase3/w3-tech-debt` |
...
| PR | scope | tasks | depends | verify | branch |
|----|-------|-------|---------|--------|--------|
| PR-14 | 集成测试全链路（adapter + outbox + DLQ + Journey + Assembly） | T70, T71, T72, T73, T74 | PR-11, PR-06 | `make test-integration` (testcontainers, tags=integration) | merged `feat/phase3/w4-integration-tests` |
| PR-15 | 覆盖率补全 + Phase 2 回归 | T75, T76 | PR-11 | `go test -cover ./... kernel/ >= 90% && go test ./...` | merged `feat/phase3/w4-coverage-regression` |
| PR-16 | 文档（godoc + doc.go + 指南 + 配置参考） | T77, T78, T79, T80, T81, T82 | PR-06, PR-07 (参考) | 人工核查 godoc 渲染 + `go doc ./adapters/...` 无 error | merged `feat/phase3/w4-docs` |
| PR-17 | 元数据更新 + KG 全量验证 | T83, T84, T85, T86, T87, T88, T89, T90 | PR-14, PR-15 | `gocell validate && go build ./... && go vet ./... && bash scripts/kg-verify.sh` | merged `feat/phase3/w4-kg-verify` |
