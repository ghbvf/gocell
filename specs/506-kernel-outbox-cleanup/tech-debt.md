...
| 3 | [PRODUCT] | 运维 / 测试 | discard path 可观测不足：(a) `DiscardPublisher.Publish()` 走 `slog.Default()` 而非注入 logger，测试无法捕获 warn、生产日志缺 trace_id 上下文（PR#113 review F）；(b) 无 metrics/health 指标。 | 加 Logger 字段会破坏零值可用语义，需统一设计；metrics 需先定义 observability 契约。 | DISCARD-OBS-01（backlog Batch 8） |
...
- [TECH] 新增: 2 条
- [PRODUCT] 新增: 2 条
- 本轮已修复的 review finding: 3 条 P1 + 2 条 P2（含 IsDiscardPublisher 降 unexported）
