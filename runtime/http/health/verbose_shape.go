...
// runtime/http/health/healthtest) can type-assert per-entry values from a slog
// capture and read fields via the exported accessor methods below.
//
...
// producer is newRedactedErrorMsg in aggregateProbeResults. There is NO
// testing-only exported constructor: healthtest unit tests assert against
// SlogDependencyEntry{} zero-value plumbing (helper map-walk correctness)
// while semantic value tests live in this package's white-box tests where
// they exercise a real Handler.
//
...
//
// Serialization: logDiagnostics emits dependencies as
// `slog.Group("dependencies", slog.Any(name, entry)...)` — inside the Group,
// each sub-attr's value is a LogValuer SlogDependencyEntry, and ALL slog
// handlers (JSON / text / logfmt) call Value.Resolve() → LogValue() during
// serialization, producing the snake_case GroupValue from the LogValue
// method below. This is the ONLY shape that gives consistent snake_case
// output across handlers — slog.Any(map[K]Entry) is opaque-blob and bypasses
// LogValue entirely (JSON handler would emit "{}" because fields are
// unexported and there's no MarshalJSON; text handler would emit %+v with
// unexported field names). See ADR 202605171200 §2 D6.
...
// slog.Any inside a slog.Group("dependencies", ...), so handlers always call
// Value.Resolve() → LogValue() during serialization regardless of handler
// implementation (JSON / text / logfmt). Output: snake_case status /
// duration_ms / error_msg fields under the dep name key inside the
// dependencies Group.
