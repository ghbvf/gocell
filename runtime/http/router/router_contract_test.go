...
func TestMount_MiddlewareInheritance(t *testing.T) {
	r := mustNew(clock.Real()) // New() applies default middleware (RequestID, SecurityHeaders, etc.)
...
func TestMetricsEndpoint_NotOnPrimaryListener(t *testing.T) {
	r := mustNew(clock.Real()) // PrimaryListener router; no health/metrics handler registered
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/metrics", nil)
	r.ServeHTTP(rec, req)
	assert.Equal(t, http.StatusNotFound, rec.Code,
