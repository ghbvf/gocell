...
func TestExpandManagedResources_NilWorker_Skip(t *testing.T) {
	b := &Bootstrap{}
	b.lc.managedResources = []kernellifecycle.ManagedResource{nilWorkerResource{}}
	...
	assert.Len(t, b.lc.managedResourceTeardowns, 1, "teardown must still be registered for Close")
}
...
func TestExpandManagedResources_DuplicateChecker_Phase0Error(t *testing.T) {
	b := &Bootstrap{}
	b.lc.managedResources = []kernellifecycle.ManagedResource{
...
func TestExpandManagedResources_CloseFailure_TeardownChainContinues(t *testing.T) {
	...
	require.NoError(t, b.expandManagedResources())
	require.Len(t, b.lc.managedResourceTeardowns, 2)

	// Teardowns are in registration order; LIFO means we call them reversed.
	ctx := context.Background()
	for i := len(b.lc.managedResourceTeardowns) - 1; i >= 0; i-- {
		_ = b.lc.managedResourceTeardowns[i].fn(ctx) // ignore individual errors; chain must continue
	}
...
func TestExpandManagedResources_CloseFailure_TeardownErrorIncludesResourceType(t *testing.T) {
	...
	require.NoError(t, b.expandManagedResources())
	require.Len(t, b.lc.managedResourceTeardowns, 1)
