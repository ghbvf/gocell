...
| PR408-FU-GOVERNANCE-OWNER-AST-EXTRACTION-01 | **Inventory governance section AST owner 提取** — 持久反向索引同型反模式；inventory.md 与脚本 governance section 已被 PR-A' 一并删除。governance owner 现场用 `grep -hoE '(FMT\|REF\|TOPO\|...)-[0-9]+' kernel/governance/rules_*.go \| sort -u` 解决。 | arch-opt | P1/Cx2 | ❌ cancelled by PR-A' | — | — | 2026-05-07 review |
...
| PR419-FU-INVENTORY-CI-GATE-01 | **archtest-inventory drift gate 入 CI** — drift gate 已被 PR-A' 整体删除（持久 inventory.md 删除后 gate 失去对象，单源守卫由 INVENTORY-ANCHOR-REQUIRED-01 archtest 接替，CI 自然命中无需额外 workflow step）。 | arch-opt | P2/Cx1 | ❌ cancelled by PR-A' | — | — | PR#419 review F-OBS-03 |
...
| PR430-FU-USAGE-01-TYPE-AWARE | **SCANNER-FRAMEWORK-USAGE-01 type-aware 升级** — 现状: USAGE-01 用 AST + PackageAliases 检测包级目录遍历函数（path/filepath, os, io/ioutil, io/fs），可覆盖 dot-import / 别名 / SelectorExpr 全 5 种 AST 形态；但 method calls on values whose type 是 `*os.File` / `fs.FS` / `embed.FS` 等无法识别（receiver type 需 go/types info）。例：`f := os.Open("d"); f.ReadDir(-1)` 当前可绕过；修复: 升级到 `golang.org/x/tools/go/packages` + types.Info，从 SelectorExpr 解析 receiver 类型，匹配 `(*os.File).ReadDir` / `(fs.ReadDirFS).ReadDir` 等 method receivers；保持现有 fixture，新增 method-call positive cases | arch-opt+test | P2/Cx3 | 🟡 | method-call bypass 事故首现，或 USAGE-01 漏报投诉 | `tools/archtest/scanner_framework_usage_test.go` + 可能需 `tools/archtest/internal/typeseval/` 扩展 | PR#430 review P2.1 |
