...
| CREDENTIAL-AUTHORITY-READSIDE-FUNNEL-01 | **read-side credential-authority Hard funnel**（S-next）— ✅ closed by S-next PR：新建 `cells/accesscore/internal/credentialauthority` 包，`Assert(user *domain.User, checks ...Check) error` + 工厂 `SnapshotPasswordVersion` / `SessionNotRevoked`（unexported field 让 slice 永不直接读 `PasswordVersion` / `RevokedAt`）；archtest `CREDENTIAL-AUTHORITY-ASSERT-FUNNEL-01` 双向 Hard（下游 caller allowlist + 上游 mandatory funnel + 4 blind-spot + 2 RED fixture）；与 write-side `authzmutate.Mutator.Apply` 对称达成 read-side/write-side 双向闭合。ADR-credential §A11 → LANDED。 | arch-opt | P2/Cx2 | ✅ | S-next PR (2026-05-17) | `cells/accesscore/internal/credentialauthority/` + `cells/accesscore/slices/{sessionlogin,sessionrefresh,sessionvalidate}/` + `tools/archtest/credential_authority_assert_funnel_test.go` | ADR-credential §A11; S-next PR |
