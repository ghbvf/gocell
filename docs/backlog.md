...
> Wave 1 进行中: ✅ PR#116-128 (13 PRs 已合入)
...
| # | 任务 | 工时 | 文件 | 来源 | 合并 PR |
|---|------|------|------|------|---------|
| 25 | **HSTS 加固** C-H4: `security_headers.go` 补 `includeSubDomains` | 0.5h | `runtime/http/middleware/security_headers.go` | P2 tech-debt | MG-A 安全加固 |
| 28e | **AUTH-TTL-CONST-01** `accessTokenTTL` 3 处重复定义（login/refresh/main），提取共享常量或配置化 | 0.5h | `cells/access-core/slices/session*/service.go` + `cmd/core-bundle/main.go` | PR#127 review | MG-A |
| 24 | **Trace trust policy** TRUST-POLICY-01: public-facing endpoint trust-boundary 策略（参考 otelhttp `WithPublicEndpoint`：new root + link），当前默认 trusted-upstream + OBS-REQID-TRUST: request_id middleware 无条件信任外部 `X-Request-Id`，需信任边界校验 | 4h | `runtime/http/middleware/tracing.go` + `request_id.go` | 5B PR#112 review + 217 tech-debt | MG-A 安全加固 |
| 28d | **AUTH-REFRESH-VERIFIER-01** `sessionrefresh.Service` 注入 raw `jwtVerifier`（非 session-aware `validateSvc`），JWT 层 sid fail-closed 语义对 refresh 端点不成立。当前 DB 路径（`GetByRefreshToken + IsRevoked`）已确保撤销有效，安全风险低但架构不对称 | 2h | `cells/access-core/cell.go:203` | PR#127 review | MG-A |
| 27v | **DOMAIN-JSON-TAG-REMOVAL** Order/Device/Command domain entities 保留 json tags（事件序列化依赖），需设计 event payload DTO 层解耦后才能移除 | 4h | `cells/order-cell/internal/domain/order.go` + `cells/device-cell/internal/domain/device.go` | PR#126 review | MG-B DTO 对齐 |
| 27u | **DTO-COVERAGE-GAP** sessionlogin/sessionrefresh TokenPair + featureflag EvaluateResult 仍为 service 层类型直出，需补 handler 层 DTO | 2h | `cells/access-core/slices/session*/handler.go` + `cells/config-core/slices/featureflag/handler.go` | PR#126 review | MG-B |
| 27l | **EVALUATE-RESULT-LAYER** featureflag `EvaluateResult` 定义在 service.go 而非 handler 层，轻微违反 DTO-in-handler 模式。功能等同 DTO（2 字段 + json tags），实际风险极低 | 0.5h | `cells/config-core/slices/featureflag/service.go` + `handler.go` | PR#126 review | MG-B |
...
| # | 任务 | 工时 | 文件 | 来源 | 合并 PR |
|---|------|------|------|------|---------|
| 27n | **HANDLER-TEST-CAMELCASE-ASSERT** 13 个 handler_test.go 无显式 camelCase key 断言（如 `assert.Contains(body, "createdAt")`），camelCase 合规由 contract_test + schema 守护 | 2h | 12+ `handler_test.go` | PR#126 review | MG-E 测试质量 |
| 27k | **DTO-CONVERTER-UNIT-TEST** 8 个 DTO converter 函数（toXxxResponse）无独立单测，仅靠 handler httptest 间接覆盖。若 converter 增加条件逻辑需补专项测试 | 2h | 6 个 `handler_test.go` | PR#126 review | MG-E |
...
| #10 Watcher 核心增强 | 7h | 无依赖，随时可做 |
| #11 Watcher 状态面 | 4h | 依赖 #10 |
...
已完成:
  ...
  Wave 1 已合入: PR#116-128 (13 PRs)

Wave 1 剩余:
  MUST (Batch A-D):    ~35h / 4-5 工作日 — v1.0 最低门禁
  SHOULD (Batch E-G):  ~16.5h / 2 工作日 — 提升 review 通过率
  P1+运维 (穿插):      ~19h — 独立无依赖，可与 MUST/SHOULD 并行
  Batch 8 (v1.0 后):   ~57h — 不阻塞发布
