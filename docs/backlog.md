...
| SCAFFOLD-BUNDLE-ARCHTEST-HARDEN | **SCAFFOLD-BUNDLE-ARCHTEST-HARDEN** — 现状: INVARIANT SCAFFOLD-BUNDLE-MARKER-01 + SCAFFOLD-BUNDLE-NO-CODEGEN-LITERAL-01 当前为 Soft（string anchor on template content）；Hard upgrade path：将 template 转为 typed struct embedded via go:embed + AST validation of cell.go template emit，或使用 markergen typed marker registry assert presence | arch-opt | P2/Cx2 | 🟡 | K#09 ship 后首次 scaffold drift incident 或 AI 暴露面 ≥3 | `tools/archtest/scaffold_bundle_test.go` + `tools/codegen/cellgen/templates/` | K#09 PR#442 review |
