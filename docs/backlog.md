...
| P3-TD-10 | **TOCTOU 竞态修复** — 现状: Phase 2 #54 session TOCTOU 未修；修复路径**已从 Redis 分布式锁改为 authz_epoch 闭环**（plan 034 S4b 范围，对标 OAuth2 token versioning / OIDC sid 演化模式比 distlock 更便宜）；S4b 完成 (a) JWT issue 写 epoch claim (b) verifier 读 epoch (c) sessionvalidate 比对 user.authz_epoch (d) 所有 credential event bump epoch 后，本条 TOCTOU 主体消失；涵盖 PR #482 F2（login 在 tx 外读 user state/password vs credential mutation tx 内 revoke 竞态）(also: cap-11) | bug | P2/Cx3 | 🟠 | S4b authz_epoch 闭环 ship | `cells/accesscore/` | tech-debt-registry P3-TD-10 + PR #482 F2 |
