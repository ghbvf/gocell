...
| IDENTITYMANAGE-LOCKUNLOCK-CO-TX-UPGRADE-01 | **identitymanage Lock/Unlock split-tx 升级** — 现状: `cells/accesscore/slices/identitymanage/service.go:687-707` (Lock) 和 :763-777 (Unlock) 走 authzmutator.Apply 完成自己 tx 后另起 RunInTx publish event；事件可能在状态提交后丢失。godoc 已 accept trade-off（`service.go:680-686` "Changing this to a single-tx design would require passing the RunInTx context through authzmutate..."）。**触发条件**：第 2 次 split-tx 引发实际部分提交事故 / authzmutate API 重设计（同 IDENTITYMANAGE-UPDATE-CO-TX-UPGRADE-01）。**修复方向**：让 authzmutator 接受外部 tx context，把 lock/unlock 状态变更 + event publish 合并到同一个 RunInTx 闭包。 | arch-opt | P3/Cx3 | 🟠 | 第 2 次同模式事故 / authzmutate API 重设计 | `cells/accesscore/slices/identitymanage/service.go` + `cells/accesscore/internal/authzmutate/` | PR #525 deep review P1-1 |
