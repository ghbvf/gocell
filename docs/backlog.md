...
| R-03 | **BOOTSTRAP-NIL-OPTION-CONSISTENCY** — `WithManagedCloser(nil)` 静默接受，`WithManagedResource(nil)` phase0 fail-fast — 相邻 API 风格冲突；修复: 两者均改 fail-fast；扩项一并对齐 `WithRateLimiter`（同根隐式 nil 接受 → 请求路径 panic）+ 抽 `pkg/nilutil.IsNil` 单源 helper（替代 `bootstrap.isNilManagedResource` / `middleware.IsTypedNilAllower` / `router.isNilIntentTokenVerifier` / `auth.isNilInterfaceValue` ×2 共 5 处重复实现）| bug | P2/Cx1 | ✅ | fix/300 | `pkg/nilutil/` + `runtime/bootstrap/` + `runtime/http/middleware/circuit_breaker.go` + `runtime/http/router/router.go` + `runtime/auth/jwt.go` + `runtime/auth/config/registry.go` | 030 §2 R-03 |
