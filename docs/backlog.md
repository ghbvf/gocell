...
| SCAFFOLD-DRY-RUN-COMPLETE-INVENTORY | **SCAFFOLD-DRY-RUN-COMPLETE-INVENTORY** — 现状: `scaffold cell --dry-run` 仅打印 2 行 (cell.yaml + cell.go)，实际 bundle 写 8 文件；`scaffold assembly --dry-run` 仅打印 3 行实际写 6 文件；修复: dry-run 输出由 ScaffoldCellBundle / Generator.Scaffold 真实 planned files 列表驱动，覆盖 slice/contract/schemas + auto-generate 产物；加测试断言 dry-run 行数 == 实际产出文件数 | feat | P2/Cx2 | 🟡 | K#09 ship 后 v1.1 文档化 | `cmd/gocell/app/scaffold.go` + `cmd/gocell/app/scaffold_assembly.go` | K#09 PR#442 round-2 F8/F9 |
| SCAFFOLD-HELP-COMPLETE | **SCAFFOLD-HELP-COMPLETE** — 现状: `gocell scaffold --help` 未列 `assembly` 子命令；`scaffold cell --help` 未列 `--with-http/--with-events/--with-both/--skip-generate`；修复: `cmd/gocell/app/help.go` 同步 K#09 命令矩阵 + 全部新 flag；archtest 守 help 文本与 dispatch 注册的子命令集合一致 | feat | P2/Cx1 | 🟡 | K#09 ship 后立即 / 教程化推广 | `cmd/gocell/app/help.go` + `cmd/gocell/app/scaffold.go` | K#09 PR#442 round-2 F10 |
| SCAFFOLD-KEBAB-MIGRATION-DOC | **SCAFFOLD-KEBAB-MIGRATION-DOC** — 现状: K#09 ScaffoldCellBundle 因 Go 包名约束自动 dash-strip cellID（"test-cell" → "testcell"），既有用户的 kebab cellID 工作流被静默改写；修复: README + CHANGELOG 加 BREAKING CHANGE 段说明；`gocell scaffold cell --id=foo-bar` 在 dash 出现时打印迁移提示（"K#09: cellID 已规范化为 'foobar'，原 'foo-bar' 不再支持"）| docs | P2/Cx1 | 🟡 | K#09 ship 后 | `README.md` + `CHANGELOG.md` + `cmd/gocell/app/scaffold.go` | K#09 PR#442 round-2 F11 |
| SCAFFOLD-FILE-PERMISSION-ALIGN | **SCAFFOLD-FILE-PERMISSION-ALIGN** — 现状: K#09 scaffold 输出文件 0o600 / 目录 0o750（cellgen.ScaffoldCell 既有惯例），与 helm/kubebuilder 0o644 + 0o755 不符；多用户 CI 环境读不到；修复: 文件 0o600 → 0o644，目录 0o750 → 0o755；secrets 类（auth tokens / hmac keys）保留 0o600 | arch-opt | P2/Cx1 | 🟡 | 多用户 CI 报权限错误时 / 与 helm 对齐 | `tools/codegen/cellgen/scaffold.go` + `tools/codegen/cellgen/scaffold_bundle.go` + `kernel/assembly/generator.go` | K#09 PR#442 round-2 F12 |
| SCAFFOLD-FAILURE-CLEANUP-RECOVERY | **SCAFFOLD-FAILURE-CLEANUP-RECOVERY** — 现状: scaffold 失败后已写文件留在磁盘；`--skip-generate` 重试仍被 conflict detection 阻塞，错误信息无恢复指引；修复: 失败时自动清理本次写入的所有文件（render plan + commit 三阶段，所有文件全成功才 commit；任一失败 → rollback）；conflict 错误信息加 "rerun with --force or rm -rf cells/{id}" 建议 | feat | P2/Cx2 | 🟡 | scaffold 失败案例 ≥3 次 | `tools/codegen/cellgen/scaffold_bundle.go` + `kernel/assembly/generator.go` | K#09 PR#442 round-2 F13 + half-bake |
| SCAFFOLD-GENERATOR-PURE-BYTES | **SCAFFOLD-GENERATOR-PURE-BYTES** — 现状: `kernel/assembly.Generator.Scaffold` 同时承担渲染+写入，破坏 Generator "只产字节" 模式（其他 GenerateXxx 都返 []byte，由 caller 调 codegen.Write）；修复: 拆 `Scaffold(spec) -> map[string][]byte` 渲染 + `WriteScaffold(plan, opts)` 写入；codegen.Write 路径统一接管 | refactor | P3/Cx3 | 🟢 | 架构清理 / Hard 化 input contract 时一并 | `kernel/assembly/generator.go` | K#09 PR#442 round-2 F14 |
...
| SCAFFOLD-CONFLICT-ERRCODE-SEMANTICS | **SCAFFOLD-CONFLICT-ERRCODE-SEMANTICS** — 现状: `renderAssemblyScaffoldFiles` 冲突错误用 `errcode.ErrValidationFailed` (语义错误)；path 信息放 `WithInternal` 对 CLI 用户不可见；修复: 改用 `errcode.ErrConflict` (KindConflict)；path 移到 `WithDetails(slog.String("path", ...))` 让 4xx 响应/CLI stderr 看到 | arch-opt | P2/Cx1 | 🟡 | errcode 一致性扫描 / 第 3 个 conflict 类错误出现 | `kernel/assembly/generator.go` + 同模式扫描 | K#09 PR#442 round-2 F16 |
