...
| G-07 | **OUTBOX-WRITER-MUST-CONTRACT** — (a) `Writer.Write` SHOULD→MUST + `TxRunner.RunInTx` godoc 强化；(b) `MaxMetadataKeys` 等四常量提取到 `kernel/metautil`；(c) `HandleResult.Receipt` 已在 029 K#12 移除（`OUTBOX-HANDLERESULT-NO-RECEIPT-FIELD-01` 守）；(d) 加 `outbox.Ack()/Requeue(err)/Reject(err)` 工厂 + kernel/outbox + outboxtest 已迁移；剩余 ~150 sites 机械迁移见 `OUTBOX-FACTORY-ADOPTION-01` | arch-opt | P1/Cx2 | ✅ closed by W7 (PR#298) | — | `kernel/outbox/` + `kernel/command/` + `kernel/metautil/` | 030 §3 G-07 | cap-07 |
