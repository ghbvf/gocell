...
| AUTHZ-MUTATION-FUNNEL-UPGRADE-01 | **funnel 上游 Medium → Hard 升级**（S4e）— 现状: S4d funnel 上游用 `CREDENTIAL-INVALIDATE-UPSTREAM-CALLER-01` (Medium archtest, caller allowlist) 锁，但不防"新 authz-affecting 字段 setter 漏调 invalidator"（P1-#1 回归形态）。修复路径（plan #034 S4e）：(a) `domain.User` 的 status / passwordResetRequired / authzEpoch 字段私有化 + 删 setter（compile 期包外不可改）；(b) 新 `cells/accesscore/internal/authzmutate` 包 + sealed `Mutation` interface + `Apply(ctx, txMgr, repo, inv, userID, m, now)` 唯一入口（compile 期强制 `inv` 非 nil 入参）；(c) identitymanage / rbacassign 等改走 authzmutate.Apply（不再手动调 invalidator）；(d) archtest 升 Hard — `DOMAIN-AUTHZ-FIELD-PRIVATE-01` + `AUTHZ-MUTATION-APPLY-FUNNEL-01`，`CREDENTIAL-INVALIDATE-UPSTREAM-CALLER-01` 收窄到 authzmutate + sessionrefresh。范围 ~30 文件；不动 schema / service 行为。 | arch-opt | P2/Cx2 | 🟡 | S4d ship 后立即立项 | `cells/accesscore/internal/{domain,authzmutate,credentialinvalidate,slices/identitymanage,slices/rbacassign}/` + `tools/archtest/` | ADR-credential §A10; PR S4d review checklist |
