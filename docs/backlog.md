...
| ~~A3~~ | ✅ **RL-INT-01**: Relay PG 集成测试（6 个 testcontainers PG+RMQ 测试：happy-path、transient publish failure retry、dead-letter、concurrent claim no double-publish、clean shutdown via reclaimStale recovery、**broker TCP disconnect/recovery（testcontainers Stop/Start）**）。broker-side 断连/重连由本测试（relay 视角）+ `adapters/rabbitmq/integration_test.go`（connection adapter 视角）分工覆盖。 | — | `adapters/postgres/outbox_relay_integration_test.go` | PR-PG-HARDEN |
