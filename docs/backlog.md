...
| ~~A3~~ | ✅ **RL-INT-01**: Relay PG 集成测试（5 个 testcontainers PG+RMQ 测试：happy-path、transient publish failure retry、dead-letter、concurrent claim no double-publish、clean shutdown via reclaimStale recovery）。真实 RMQ TCP-level disconnect/reconnect 由 `adapters/rabbitmq/integration_test.go` 覆盖，不在本 PR 范围。 | — | `adapters/postgres/outbox_relay_integration_test.go` | PR-PG-HARDEN |
