...
| B2-X-04 | **HEALTH-LISTENER-DEFAULT-LOOPBACK** — 现状: `cmd/corebundle/shared_deps.go:128-135` health listener 默认 `127.0.0.1:9091`，real mode 仅靠注释 + `GOCELL_HTTP_HEALTH_LOCAL_ONLY=1` opt-in 提醒，未显式绑定 PodIP/Service 时 K8s kubelet/Prometheus probe 不可达且无 fail-fast；修复: real mode 缺显式 bind 时 fail-fast，或默认 Pod-reachable `:9091` + archtest 锁 internal healthz only via mTLS | bug | P1/Cx2 | 🟡 | — | `cmd/corebundle/shared_deps.go` | backlog2 §3 B2-X-04（030 §6.1 删除回灌 2026-05-19）|
