...
| PR445-FU-PACKAGEALIASES-TYPE-AWARE-01 | **三个非守卫规则的 PackageAliases 升级 type-aware** — 现状: PR-Φ 把 SCANNER-FRAMEWORK-USAGE-01 自身升 type-aware（path A'），但 `scanner.PackageAliases` helper 仍被 3 个非守卫规则使用：`svctoken_caller_cell_test.go:65`、`role_admin_literal_test.go:166`、`outbox_invariants_test.go:1976`。这些规则按 AST 字符串匹配，dot-import 和 import alias 组合可能绕过；charter §6 第 3 条"全 archtest type-aware 升级"deferred 到 Wave N+ Pass-mode 改造（80-150h），故本 PR 不做。修复: 为这三个规则各自引入 typeseval.SharedResolver + types.Info，receiver-type-aware 替代 PackageAliases；统一删除 PackageAliases helper（无他用即可）| arch-opt+test | P3/Cx2 | 🟡 | dot-import / 别名 bypass 事故首现 / 或 charter §6 全 archtest type-aware Wave 启动 | `tools/archtest/svctoken_caller_cell_test.go` + `tools/archtest/role_admin_literal_test.go` + `tools/archtest/outbox_invariants_test.go` + `tools/archtest/internal/scanner/imports.go` | PR#445 reviewer P1.4 finding（2026-05-10）|
