...
| P1-16 | **CREDENTIAL-SWEEP-STARTUP-01** (P1, Cx2): `initial_admin_password` 凭据文件仅由"本次引导返回的 cleaner worker"清理，进程重启且 admin 已存在时直接 skip 引导，现存文件无人兜底扫尾，形成离线接管窗口。对标 GitLab initial_admin_password 短窗策略。**修复**: 启动期增加独立 sweep 步骤（不依赖 adminExists 分支）：过期文件即删，未过期文件重新注册 cleaner 任务 | 2h | `cells/access-core/internal/initialadmin/bootstrap.go` | 2026-04-18 Auth 域审查 |
