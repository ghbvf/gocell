...
| PR424-FU-SCANNER-NON-GO-FUNNEL-01 | **scanner framework 收编非 Go 内容扫描（funnel 终态）** — 现状: PR#424 加 SCANNER-FRAMEWORK-USAGE-02 + 13 个 `// SCANNER-ESCAPE-HATCH:` 锚点暴露 archtest 里 13 个仍直接用 `os.ReadDir`/`fs.WalkDir`/`fs.Glob`/`fs.ReadDir` 的站点；锚点是文档+静态检查双层约束，但 AI 仍可用 `fs.WalkDir(os.DirFS(root), ".", fn)` 走 .go 文件 + 谎报 category 绕开 USAGE-01（USAGE-02 不验 category 真实性）；修复: scanner 加 `EachYAMLFile/EachJSONFile/EachMarkdownFile/EachSQLFile`（薄封装在 `EachContentFile(scope, suffixes, fn)` 之上）+ `IncludeTestdata()` option（fixture 扫描需要）；迁移 13 个 escape-hatch 站点（6 deferred-scanner-migration → `scanner.EachFile`；4 非 Go → 对应 `EachXxxFile`；2 testdata-fixture-bypass → `scanner.EachFile + IncludeTestdata`；1 yaml-workflow → 视 .github 路径专项处理）；USAGE-02 升级为禁所有 `os.ReadDir`/`fs.WalkDir`/`fs.Glob`/`fs.ReadDir` 在 archtest（删 escape-hatch 概念 + 删 13 锚点）；TDD 红→绿 commit 序列，零 allowlist | arch-opt | P2/Cx3 | 🟡 | PR#424 ship 后 / 任何 escape-hatch 锚点假报 category 事故 / archtest 文件再次出现新 `deferred-scanner-migration` 锚点 | `tools/archtest/internal/scanner/` + 13 archtest 文件 + `tools/archtest/scanner_framework_usage_test.go` | PR#424 review — risk #2 path C |
