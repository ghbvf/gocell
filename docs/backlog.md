...
| EXAMPLES-ASSEMBLY-MINIMAL-CLEANUP | **EXAMPLES-ASSEMBLY-MINIMAL-CLEANUP** — 现状: K#09 ship 后，`examples/{ssobff,todoorder,iotdevice}/assembly.yaml` 仍是 K#10 minimal 形之前的全字段写法；修复: 简化为 `id+cells+owner` 三字段（与 corebundle 对齐）+ build/deploy 字段全派生；K#10 ADR `202605061800` line 103 explicit defer | arch-opt | P2/Cx2 | 🟡 | K#09 ship 后立即 | `examples/{ssobff,todoorder,iotdevice}/assembly.yaml` | K#09 carveout |
...
| CONTRACT-YAML-CODEGEN-DEFAULT-CLEANUP | **CONTRACT-YAML-CODEGEN-DEFAULT-CLEANUP** — 现状: K#09 让 parser 默认 `Codegen=true`（funnel）；46+ 既有 contract.yaml 仍显式写 `codegen: true` 行（K#06 PR-4 时代留下），现在是冗余；修复: `sed -i '' '/^codegen: true$/d' contracts/**/contract.yaml` 一次清；archtest 加 K#09 funnel 守卫扫已知 contract 不再含 `codegen: true` 字面 | arch-opt | P2/Cx2 | 🟡 | K#09 ship 后 | `contracts/**/contract.yaml` + `tools/archtest/` | K#09 carveout |
| SCAFFOLD-BUNDLE-ARCHTEST-HARDEN | **SCAFFOLD-BUNDLE-ARCHTEST-HARDEN** — 当前 Medium（real-source AST capture via ScaffoldCellBundle 产出的 cell.go / contract.yaml）。Hard upgrade path（typed marker + reflect 字段冻结）可进一步消除 AI 可绕过性，但边际收益低；触发条件出现时再升级 | arch-opt | P2/Cx2 | 🟢 | K#09 ship 后第一次 scaffold 输出漂移 / Hard 升级（typed marker + reflect 字段冻结）| `tools/archtest/scaffold_bundle_test.go` + `tools/codegen/cellgen/templates/` | K#09 PR#442 round-2 RC-4 |
...
| SCAFFOLD-INLINE-TEMPLATE-ARCHTEST | **SCAFFOLD-INLINE-TEMPLATE-ARCHTEST** — 现状: `cmd/gocell/app/scaffold.go::inlineContractYAMLTpl` 含 `codegen: false` 字面（RC-2 修复），但无 archtest 守卫；模板退化（删 codegen 行）会让生成的 draft contract 走 funnel 默认 true → generate 失败；修复: 加 archtest 在 ScaffoldContract 实际产出上断言 contract.yaml 含 `codegen: false` | arch-opt | P2/Cx1 | 🟡 | 模板退化事故 / Hard 化 input contract | `tools/archtest/` + `cmd/gocell/app/scaffold.go` | K#09 PR#442 round-2 F15 |
...
| ASSEMBLY-RUN-RUNTIME-SMOKE | **ASSEMBLY-RUN-RUNTIME-SMOKE** — 现状: K#09 scaffold 出的 `cmd/{id}/run.go` stub 仅保证 `go build` 通过，runtime 直接 `not implemented`；smoke gate 不验运行时；修复: scaffold 模板加最小可运行 stub（noop bootstrap + 立即 graceful shutdown），smoke gate 加 `go run ./cmd/{id} -timeout=5s` 验证退出码 0 | feat | P2/Cx2 | 🟢 | scaffold 用户实际尝试 `go run` 时报告问题 | `kernel/assembly/gentpl/scaffold-run-go.tpl` + `hack/verify-scaffold-assembly.sh` | K#09 PR#442 round-2 F17 |
...
| SCAFFOLD-WRITE-FUNNEL-HARD-UPGRADE | **SCAFFOLD-WRITE-FUNNEL-HARD-UPGRADE** — 现状: `SCAFFOLD-WRITE-FUNNEL-01` 已 Hard（typed function call + AST scanner allowlist），残留 escape 是"新增 scaffold 子包未加入 allowlist"的字符串硬编码；进一步 type-system 强制：暴露 typed `pathsafe.Writer` interface + 在 scaffold 包不 import `os`（用 depguard 拦），让违反在编译期不可表达 | arch-opt | P3/Cx3 | 🟢 | 第 3 个新 scaffold 子包出现 / 类型系统强制成本下降 | `pkg/pathsafe/` + `tools/archtest/scaffold_write_funnel_test.go` + `.golangci.yml` | K#09 PR#442 round-4 carryover |
...
| SCAFFOLD-INPUT-CONTRACT-TYPED-ID-01 | **SCAFFOLD-INPUT-CONTRACT-TYPED-ID-01** — 现状: scaffold 输入契约分散在 cmd/gocell/app + cellgen.ScaffoldSpec + assembly.AssemblyScaffoldSpec 三处；CLI 层 reject kebab，cellgen.ScaffoldCellBundle round-5 已对齐 reject（最小切片），但跨包 typed `ScaffoldID` value type + 共享 validator 的完整重构待做；修复: pkg 抽 typed `ScaffoldID` + 共享校验器 + 三处 spec 字段类型升级 | arch-opt | P2/Cx3 | 🟢 | 跨包 spec 输入误用 / 第 4 个 scaffold 入口出现 | `pkg/scaffoldid/` (新) + `tools/codegen/cellgen/scaffold.go` + `kernel/assembly/generator.go` + `cmd/gocell/app/scaffold.go` | K#09 PR#442 round-5 R6 carryover |
