...
> PR#160 follow-up: 🔄 PR-X1 DemoMode 显式化 + fail-closed 统一（进行中）/ ⏳ PR-X2 pkg/query 稳定性 / ⏳ PR-X3 cursor key rotation 接线
...
| **PR#160 follow-up PR-X2 (pkg/query 稳定性)** | ⏳ **PR160-P1-C** codec nil 崩溃链结构化修复（service 不再提前解引用 codec，统一下沉 helper） + **PR160-P2-2-parse** `ParsePageRequest` 层 cursor 长度上限（与 Decode 层形成双层校验） + **PR160-P1-B** KnownDemoKeys 深拷贝（已由 PR-X1 删除该 API 自动消解）+ errcode 余项。discovered via PR#160 六席位审查 (2026-04-17) | 3h |
| **PR#160 follow-up PR-X3 (cursor key rotation 接线)** | ⏳ **PR160-P1-D** `NewCursorCodec(current, previous)` 的启动接线与 env 通道：`GOCELL_AUDIT_CURSOR_KEY_PREVIOUS` / `GOCELL_CONFIG_CURSOR_KEY_PREVIOUS`（对标 K8s `--service-account-key-file` 单签名/多验证、gorilla/securecookie `CodecsFromPairs`）+ 轮换兼容回归测试。依赖 PR-X1 删除 `IsDemoKey` 语义耦合。discovered via PR#160 六席位审查 (2026-04-17) | 4h |
