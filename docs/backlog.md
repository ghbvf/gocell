...
| SCAFFOLD-WRITE-FUNNEL-01 | **SCAFFOLD-WRITE-FUNNEL-01** — ✅ done in K#09 round-4（PR #442）：scaffold/codegen 文件写入唯一漏斗化为 `pkg/pathsafe.WritePlannedFiles`；archtest（Hard 档）静态拦截 `tools/codegen/cellgen` / `kernel/assembly` / `cmd/gocell/app/scaffold*.go` 内裸 `os.MkdirAll`/`WriteFile`/`Mkdir`/`Create`/`OpenFile`，allowlist 仅 `pkg/pathsafe`；symlink escape 5 路径 + bundle/assembly 原子性一并 close | arch-hard | P1/Cx2 | ✅ | — | `pkg/pathsafe/pathsafe.go` + `tools/archtest/scaffold_write_funnel_test.go` | K#09 PR#442 round-4 |
