...
| S2-follow-2 | **CONTRACT-ERROR-CODE-PER-STATUS-01** (P2, Cx3, 🟡 可延后): 端点级精确 per-status 错误码 schema — `contracts/shared/errors/error-response-v1.schema.json` 仅约束 ERR_* 信封格式；各端点应声明 `responses[401] → {code const: ERR_AUTH_UNAUTHORIZED}`、`responses[403] → {code const: ERR_AUTH_FORBIDDEN}`（而非手写样本中的 ERR_AUTH_INVALID_TOKEN）。延后原因：各端点实际 401/403 code 取决于 handler 路径（RequireAnyRole 的 missing-subject vs missing-role 分支，以及 bearer middleware），需逐端点确认后才能在 schema 中锁定常量。来源：PR#181 外部审查 F2 | 4h | `contracts/http/**/contract.yaml` + `contracts/shared/errors/` | PR#181 review F2 |
| S13-follow | **4XX-LOG-SAMPLING-01** (P3, Cx1, 🟡 可延后): 高频端点（如 GET config）4xx 日志可加采样（1%）或在 rate-limiter 触发后限速，避免大量 429 日志淹没告警通道 | 1h | `pkg/httputil/response.go` | PR-CONFIG-POLISH 后续 |
| S13-follow2 | **HTTPUTIL-4XX-LOG-SAMPLING-01** (P2, Cx3, 🟡 可延后): 4xx WARN log 在 credential-stuffing / brute-force 场景无采样/限流机制；当前方案由 slog handler 层配置，但未定义接口契约。需要：(a) 添加 `slog.Attr` key `log_type="4xx_client_error"` 供下游采样信号；(b) observability.md 文档化 slog handler 采样约定；(c) 考虑 per-errcode token-bucket rate-limit（需状态） | 4h | `pkg/httputil/response.go` + `docs/rules/observability.md` | PR#181 review F-OPS |
