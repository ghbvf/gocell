...
| G-08 | **OUTBOX-FAILOPEN-COUNTER + INMEM-RECEIPT-FIX** — (a) ✅ fail-open `RecordDrop()` 无 metrics；(b) ✅ `inMemReceipt.Commit/Release` 共享 `sync.Once`，Release 先于 Commit 静默 false-success；(c) ✅ PR #582 (SafeID typed funnel + WireMessage/Observability 字段类型化, AI Hard 落地)：`UnmarshalEnvelope` `msg.ID` 仅非空检查，可日志注入（CWE-117）；修复: increment `outbox_failopen_drops_total{cell}` + `committed atomic.Bool` 区分 + 复用 `idutil.IsSafeID` | bug | P1/Cx2 | ✅ | — | `kernel/outbox/` + `runtime/outbox/` + `pkg/idutil/` | 030 §3 G-08 → PR #582 closed |
| ENTRY-VALIDATE-CHARSET-01 | **kernel/outbox.Entry charset 校验** — 现状: PR #582 在 wire-decode boundary 关闭 CWE-117（WireMessage 字段类型 idutil.SafeID）；in-memory `outbox.Entry` 字段仍是 string，`Entry.Validate()` 仅做非空检查不验 charset。`runtime/outbox.WriteBatchFallback` 失败路径 `slog.String("entry_id", e.ID)` 在 entry.ID 含 unsafe char 时仍写入服务端 slog（CWE-117 残余）。生产路径所有 Entry 构造受信（`MustNewEntryID` 或 `UnmarshalEnvelope.ParseSafeID`），但 archtest 不锁这一假设；修复方向: `Entry.Validate()` 补 `idutil.IsSafeID` 检查 5 字段（或仅 ID + EventType） + WriteBatchFallback 失败路径 wrap 改用 `entry_index` 替代 entry_id (或 wrap 时确保 ParseSafeID 通过)。触发条件: 出现非受信 Entry 构造路径（业务侧手写 Entry 字面量 + 直接 Writer.Write）/ 审计 log 中观察到 unsafe entry_id 写入告警 | bug-hardening | P2/Cx2 | 🟡 | 同 触发条件 | `kernel/outbox/outbox.go` Validate + `runtime/outbox/relay.go` WriteBatchFallback | PR #582 review F4 (B) |
