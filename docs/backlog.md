...
| CREDENTIAL-AUTHORITY-READSIDE-FUNNEL-01 | **read-side credential-authority Hard funnel**（S-next）— ✅ closed by S-next PR：新建 `cells/accesscore/internal/credentialauthority` 包，`Assert(user *domain.User, checks ...Check) error` + 工厂 `SnapshotPasswordVersion` / ~~`SessionNotRevoked`~~（已移出，见 §A11 重写）（unexported field 让 slice 永不直接读 `PasswordVersion`）；archtest `CREDENTIAL-AUTHORITY-ASSERT-FUNNEL-01` 四勿一 Hard（下游 caller allowlist + 上游 mandatory direct + 上游 sealed-by-name + 上游 value-capture + 4 blind-spot + 3 RED fixture per-bucket）；与 write-side `authzmutate.Mutator.Apply` 对称达成 read-side/write-side 双向闭合。ADR-credential §A11 → LANDED + 2026-05-17 重写（session-revoke 出 funnel）。 | arch-opt | P2/Cx2 | ✅ | S-next PR (2026-05-17) + PR #542 §A11 重写 (2026-05-17) | `cells/accesscore/internal/credentialauthority/` + `cells/accesscore/slices/{sessionlogin,sessionrefresh,sessionvalidate}/` + `tools/archtest/credential_authority_assert_funnel_test.go` | ADR-credential §A11; S-next PR; PR #542 reviewer P1-B/P2-A/P2-B/#10 |
| SESSION-REVOKED-FIELD-ACCESS-01 | ✅ **PR #542 (2026-05-17)** — session-state Hard funnel：typed FieldAccess archtest 守护 `session.{Session,ValidateView}.RevokedAt` owner-package allowlist（`sessionvalidate/` + `sessionrefresh/` + `runtime/auth/session/`）。从 §A11 user-bound funnel 分离出来的独立 funnel，消除 SessionNotRevoked 借住 user-bound Check 的建模错位 + 修复 PR #542 P1-A wire-status drift（revoked + inactive → 403 / revoked + repoErr → 503）。Blind-spot：reflect.FieldByName + unsafe import 反向自检；RED fixture `testdata/session_revoked_field_red`。 | arch-opt | P2/Cx2 | ✅ | PR #542 (2026-05-17) | `tools/archtest/session_revoked_field_access_test.go` + `cells/accesscore/internal/credentialauthority/testdata/session_revoked_field_red/` | ADR-credential §A11 重写 + §A13; PR #542 reviewer P1-A |
...
| WIRE-UNIFORM-RESPONSE-ARCHTEST-01 | **wire-level single-envelope 升 Hard** — 现状：§A13 wire-uniformity 由 per-slice `service_test.go` 断言（结构化字段 + 组合用例 revoked+inactive、revoked+repoErr 等）守护，Medium 评级。修复方向：扫三 slice 入口（VerifyIntent / Refresh / Login）的 error return path 类型身份，按入口归类断言每类入口对应的 errcode 集合是 enumerable allowlist（typed `*types.Info` + `errcode.Code` const 集合）。**触发条件**：本 PR 内 §A13 已落地后立项作为后续 hardening。 | arch-opt | P2/Cx3 | 🟠 | 下一次 archtest hardening pass | `tools/archtest/wire_uniform_response_test.go`（新建）+ `cells/accesscore/slices/{sessionvalidate,sessionrefresh,sessionlogin}/service.go` | ADR-credential §A13; PR #542 reviewer P1-A 闭环 |
