...
|---|---|---|---|---|---|---|---|
| ~~B2-PROVISIONER-MUTEX-REVIEW~~ | ✅ **关闭** — Provisioner.mu 已删除 + WithSetupLock 升级为强依赖 wiring option (initValidate phase0 fail-fast)。Memstore caller 用新增的 public `accesscore.NoopSetupLock{}` typed marker 显式声明（memTxRunner.RunInTx 已经持 store.mu 全闭包）；PG caller 用 `accesspg.NewSetupLock(deps)`（pg_advisory_xact_lock）。两侧都走 `setup.Service.CreateAdmin` 内 `txRunner.RunInTx → setupLock.Acquire → provisioner.Ensure` 同源路径。**升级后评级**: Hard 下游 (PG SQL 字面量) + Medium 上游 (mandatory option + IsNilInterface fail-fast) — caller 在 PG 模式仍可误传 NoopSetupLock，由 `ADMINPROVISION-SETUPLOCK-PAIRED-CTOR-HARD-02` 追踪后续 Hard 化。 | refactor | P2/Cx2 | ✅ | — | `cells/accesscore/internal/adminprovision/provisioner.go` + `cells/accesscore/cell.go` + `cells/accesscore/setup_lock_noop.go` | backlog2 §13 + PR #482 unblock + PR #629 |
| ADMINPROVISION-SETUPLOCK-PAIRED-CTOR-HARD-02 | **Hard 化 SetupLock 形态选择**（触发型）— 现状: `WithSetupLock` 必填 + fail-fast on typed-nil（Medium 上游），但 caller 可在 PG 模式误传 `NoopSetupLock{}`（type system 不区分"该传哪个"）；升级路径: 把 `(persistence.CellTxManager, ports.SetupLock)` 改为 adapter 工厂返回的绑定对（如 `accesspg.NewBundle(deps) → Bundle` / `accessmem.NewBundle(clk) → Bundle`），`accesscore.NewAccessCore` 接受 `Bundle` 而非散装 option，type system 阻止错配 → 闭环 Hard funnel。 | refactor | P3/Cx2 | 🟢 | 出现 setupLock 错配事故 / 第二个 cell 复用 SetupLock 概念 / accesscore 新增一致性域 lock | `cells/accesscore/cell.go` + `cells/accesscore/postgres/setup_lock.go` + composition root | PR #629 follow-up，`cells/accesscore/setup_lock_noop.go` godoc 点名引用 |
| C-04 | **CELLS-INIT-TEMPLATE-CONVERGE**（含 C-07 emitter health probe helper）— 3 cell Init 切分各异 + internal/ 子包不对称；修复: `kernel/cell` 提供 `BaseCell.RegisterStandard(reg, StandardInit{...})` 模板 + scaffold 预生成 `internal/{ports,domain,dto,events,mem}` 五目录 + 3 cell 改造 + scaffold 升级 + 抽 `cell.RegisterEmitterHealthProbes(reg, emitter)` helper（删 3 cell **3 处** `c.emitter.(cell.HealthProber)` 重复：configcore/cell_init.go:87 · auditcore/cell.go:279 · accesscore/cell_init.go:409；RepoHealthProber 已独立 typed funnel 不计）+ 吸收 C-09 三 cell 文件布局规范化（cell_lifecycle.go/cell_helpers.go 惯例 + scaffold 模板 + archtest）| refactor | P2/Cx2 | 🟡 | K-06 落地后 | `kernel/cell/` + `cells/{accesscore,auditcore,configcore}/` + scaffold 模板 | 030 §3 C-04 + C-07 |
