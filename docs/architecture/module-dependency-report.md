...
`config-publish.Rollback()` 只发布 `event.config.rollback.v1`，但 `config-subscribe` 只监听 `event.config.changed.v1`，所以 configcore 自己的本地 cache 不会自动随 rollback 更新。
...
2. 补 `accesscore` 与 `configcore` 的 metadata/contract 漂移
