...
- 6 个 Cell
- 25 个 Slice（19 platform + 6 examples）
- 52 个 active Contract（38 platform + 14 examples）
- 9 个 Journey
- 1 个 Assembly
...
- Platform: 3 Cell, 19 Slice, 38 active contracts.
- Examples: 3 Cell, 6 Slice, 14 active contracts.
- Repository total: 6 Cell, 25 Slice, 52 active contracts, 9 Journey.
...
`configpublish.Rollback()` 只发布 `event.config.rollback.v1`，但 `configsubscribe` 只监听 `event.config.changed.v1`，所以 configcore 自己的本地 cache 不会自动随 rollback 更新。
