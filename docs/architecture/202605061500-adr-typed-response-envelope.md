...
- HANDLER-POLICY-REQUIRED-01 — PR#411 review F1 fix 将此条升级为 **funnel + 平铺兜底两级防御**：funnel 端通过 `auth.clientsOnly` 新标志（`kernel/metadata.HTTPAuthMeta.ClientsOnly`）生成单参 `NewHandler(svc Service)`（无 policy 参数，caller 无法传 nil），Default 分支构造期加 `if policy == nil { panic(errcode.Assertion(...)) }` 把所有 nil-policy 产生源消灭；archtest scanner 退化为简化平铺兜底，删除 `handlerPolicyPublicExemptPkgs` 字符串豁免列表（alias 伪装绕过路径），捕获"字面 nil 传给 2-arg NewHandler"的代码气味（此类代码在 runtime 也会被构造期 panic 炸出，scanner 只是更早发现）。保留为 `tools/archtest/handler_policy_required_test.go`（一个文件、一个 Test func、一个 negative fixture，无豁免列表）。
