...
| 净新 `PROD-MAIN-WIRING-NOOP-REJECT-01`（reviewer P0 #3）**已落（#1303）** | 扫 `cfg.ProductionMainPkgs` 声明的 composition-root（main）包，纯 ban 直接构造 raw-noop *emitter* sink 的**完整 5 符号集**（`kernel/outbox` 的 `NoopWriter` / `NewNoopEmitter` / `NewDirectEmitter` / `DiscardPublisher` / `DemoTxRunner`）；go/types 解析（alias / dot-import / 函数值全证），sealed demo funnel（`DemoCellEmitter`/`DemoCellTxManager`）+ intended-L4 `NewDirectCellEmitter` + `ResolveEmitter` 放行。`ProductionMainPkgs` 字段同 PR 加回 `ConfigForExternalCell`，gocell dogfood 传 `{"./cmd/corebundle"}`，opt-in（空=不扫）。评级：下游 Hard（纯 ban，零 allowlist + anti-vacuity 防 vacuous-green）/ 上游 Medium（permanent Go 天花板，noop 符号须保持 public）。**威胁矩阵修订**：原文「扫 noop/**in-memory** wiring」**收窄**为 noop *emitter* funnel —— `runtime/eventbus.InMemoryEventBus` **刻意排除**（全仓唯一接线的进程内 publisher，非 Nooper；排除即 option A）。「postgres 模式 publisher 仍是 in-memory」缺口非本规则覆盖，落 **#1940** 跟踪。失败域与 runtime `CheckNotNoop`（cell Init）/ `OUTGUARD-01`（cell.yaml）/ `CELL-L2-INIT-CHECKNOTNOOP-CALLED-01`（cell 包）正交。 | #1303 / #1940 |
