...
| `pass_test.go` | Unit-tests the `archtest.Run` driver (via its scope constructors) / `buildTypedPass` / `newPackageRel` / `isPackageWithTestFiles`; the latter three accept or construct `*packages.Package` fixtures by signature. |
...
### #944 amendment — funnel hardening (2026-05-25)

PR #536 R1.1 left three upstream gaps that made the "the only way to load a fixture is RunTypedFixture" claim false. #944 closes all three:

1. **`FlatNonDefaultTags()` carried the fixture tag.** `typeseval.KnownNonDefaultTags()` listed `{"archtest_fixture"}`, so the flat union included it; a business `RunTyped(Tags: FlatNonDefaultTags())` scan loaded fixture-tagged code, and the detector missed it (the tag arrived as a function-call result, not a const-resolvable expr). Fix: remove the entry from `KnownNonDefaultTags()`; recognize the tag in `repoSkipTagAllowlist`, **path-bound to `tools/archtest/`** (coverage self-test stays fail-closed without granting a global by-name skip — see new threat-matrix row below); delete the now-redundant `ProductionFlatTags()` band-aid (it was `FlatNonDefaultTags()` minus the fixture tag — premise gone) and migrate its callers. `TestFlatNonDefaultTagsExcludesFixtureTag` locks it.
2. **Same-package `runTypedWithRoot` bypass.** The unexported shared impl behind RunTyped/RunTypedDir/RunTypedFixture was not in `fixtureTagLoaderSet`; an in-package business test could call it directly with the tag. Fix: add `runTypedWithRoot` to the set (Form E, in-package RED fixture `passfunnel_inpkg_redfixture.go`).
3. **Var-indirection arg shape.** `var t = []string{fixtureBuildTag}; loader(Tags: t)` was an accepted Blind spot. Fix: `collectFixtureTagBoundObjects` (file-scoped, single-binding, ports `collectKnownTagsBoundObjects`) records objects bound to a fixture-tag slice; the Ident walker reports a loader-arg ident resolving to one (Form F, with GREEN-parity negatives under an exact-count lock).

Additionally, **`FixtureBuildTag` is unexported → `fixtureBuildTag`** (type-system Hard): the former Form D (cross-pkg SelectorExpr) is a compile error outside package `archtest`, not an archtest finding. The cross-pkg union re-introduction path is also structurally barred — `typeseval` cannot import `archtest` (cycle), so the union physically cannot symbol-reference the fixture tag const.

**Threat matrix re-evaluation (per ai-robust.md §ADR amendment 落地必查):**

| Vector | Before #944 | After #944 | Status change |
|--------|-------------|------------|---------------|
| `FlatNonDefaultTags()` carries fixture tag → loader scan loads fixtures | **Latently legal** (tag in union; opaque to const-resolve detector) | Tag absent from union; `TestFlatNonDefaultTagsExcludesFixtureTag` + buildtags literal-ban lock; `typeseval`-can't-import-`archtest` bars symbol re-add | ✅ Closed (symbol path type-system, literal path archtest-Hard) |
| Same-package unexported `runTypedWithRoot` with fixture tag | **Not detected** (not in loader set) | In `fixtureTagLoaderSet`; Form E RED coverage | ✅ Closed (archtest-bound Hard) |
| Same-file var-binding `var t=[]string{tag}; loader(Tags:t)` | **Accepted Blind spot** | `collectFixtureTagBoundObjects` detects; Form F + exact-count | ✅ Closed (Medium upstream — same-file single-binding) |
| Cross-pkg SelectorExpr `archtest.FixtureBuildTag` → loader (old Form D) | archtest-detected (Medium-archtest) | **Compile error** — `fixtureBuildTag` unexported | ✅ Upgraded archtest→type-system Hard |
| Cross-func / cross-file var escape; multi-RHS positional binding | Accepted Blind spot | Accepted Blind spot (unchanged; inter-procedural Hard tracked via gh issue #973) | ⚠️ Residual Medium, tracked |
| `fixtureTagLoaderSet` enumeration maintenance | Review-bound | Review-bound (unchanged) | ⚠️ Unchanged (same grade as sibling banned-symbol lists) |
...
3. ✅ The true end-state constraints are:
   - (a) `archtestmeta.LegacyAllowlist` is empty and package deleted (criterion #1 above). `archtestmeta` package deleted entirely; the `Fixture` loader — the `Run` dispatch for a `fixtureRunScope` (#1037 §1d; formerly `RunTypedFixture`) — supplies the build-tag literal via function-body inlining (single source for the `archtest_fixture` string; Go build directive syntax does not allow const reference).
   ...
   - (d) No production archtest `*_test.go` CallExpr resolves to a `fixtureTagLoaderSet` member (archtest.{Typed, Production, StandaloneModule, runTypedWithRoot} + typeseval.{SharedResolver, LoadPackages, LoadProductionPackages}; constructor members renamed by #1037 §1d) with any arg subtree containing an Expr whose `EvaluateConstString` value equals `"archtest_fixture"` — enforced by PASS-FUNNEL-FIXTURE-TAG-01 (defense #4 upstream). Catches BasicLit literal / same-pkg const Ident / BinaryExpr const-concat uniformly (the former cross-pkg SelectorExpr Form D `archtest.FixtureBuildTag` is now a compile error since `fixtureBuildTag` is unexported per #944, not an archtest finding). Fixture loading uses `Run(t, Fixture(...))` (the literal is supplied inside the `Run` dispatch for a `fixtureRunScope`, single source = the unexported `fixtureBuildTag` const); in-package Go-code tag-identity paths (callee NOT in loader set) reference that same const. Added in PR #536 review R1 (2026-05-17), corrected to (callee, arg) pair form in same-PR R1.1 rewrite to close BasicLit-only Soft gap.
...
| build-constraint | `ParseBuildConstraint` `BuildContextPredicate` `IsGeneratedRelPath` | `(*Pass).IsFileInScope` (default-predicate bool) **plus** free funcs `archtest.ParseBuildConstraint` / `archtest.BuildContextPredicate` for call sites needing the raw `constraint.Expr` under multiple tag-set predicates. `IsGeneratedRelPath`: **`(*Pass).IsGenerated` only** — the free-func re-export was removed in #1037 (see erratum below) |
...
- `IsGeneratedRelPath`: PR #495 added a `func IsGeneratedRelPath(rel string) bool` free func on the premise that the Wave3 anchor needed a raw-string rel path "not from a `*ast.File`". **That premise was incorrect (see #1037 amendment below)** — the call site iterates `p.Files` (real `*ast.File`s), so `(*Pass).IsGenerated(f)` covers it.

Both gaps were closed in PR #495 by adding `func ParseBuildConstraint(filePath string) (constraint.Expr, error)` and `func IsGeneratedRelPath(rel string) bool` as thin-delegate free funcs in `resolve.go`. Both symbols remain in the RESOLVE-01 banned map (the ban targets `typeseval.`-qualified direct calls; business code uses the `archtest.` façade). The invariant (as amended by #1037): **every RESOLVE-01 banned symbol has a sanctioned façade route** — a free-func re-export (`ParseBuildConstraint` / `BuildContextPredicate` for the raw-`constraint.Expr` multi-predicate case) **or** a `Pass` method (`IsGeneratedRelPath` → `(*Pass).IsGenerated`).

**Amendment 2026-05-31 (#1037 — façade contraction Phase 0/1):** Three façade exits were removed and the contraction locked by a new defense:

- `type FileContext = scanner.FileContext` (the alias at row "`FileContext`" of the §"considered & rejected" notes above) — deleted: a dead re-export with zero call sites. New rules use `Pass.Files` / `Pass.Fset` / `Pass.Rel`.
- `func LoadContentFiles(...)` — unexported to `loadContentFiles`: its callers are pure non-test helpers returning `([]X, error)` (no `*testing.T`), so the fail-loud `EachContentFile` cannot replace them; the in-package reader stays, the public export is removed.
- `func IsGeneratedRelPath(rel string) bool` — deleted: the sole call site (the Wave3 anchor) had a `*ast.File` and migrated to `(*Pass).IsGenerated(f)`, disproving the PR #495 raw-string rationale. `ParseBuildConstraint` + `BuildContextPredicate` are **retained** — their callers run outside a Pass over out-of-default-scope files (e.g. `//go:build integration`) and evaluate the raw `constraint.Expr` under several tag-set predicates, which a Pass cannot reach or express.

This adds **defense #3** to the closure table: `FACADE-CONTRACTED-EXPORTS-01` (`pass_test.go`, sibling of `FACADE-NO-LOADER-LEAK-01`, same AST-scan helper) statically bans re-export of these three names from the façade's own `.go` files. Grade: **Hard for direct re-declaration** (literal exported-name match, top-level decls only — no func-body-local false positives) / Medium for alias-of-alias. The defense governs the façade surface only; reaching the internal equivalents is separately barred by Go internal-package visibility (outside `tools/archtest`) and `PASS-FUNNEL-RESOLVE-01` (within).
