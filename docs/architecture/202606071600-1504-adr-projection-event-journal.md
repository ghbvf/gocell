...
| **PR-04（#1771，已落地）** | **T-06-2 PG e2e rebuild 证明（已通过）+ 删 gate（已删）+ runbook/rollback**：T-06-2 = ① `adapters/postgres` testcontainers 白盒（cleaned-outbox 独立性 + crash-restart resume + no-double-apply + rebuild-from-0 + cold-start）+ ② corebundle HTTP e2e（登录产 `event.session.created.v1` → accesscore `session_registry` 投影 → 查 registry-summary **live 收敛 + 鉴权边界**；**不含 POST rebuild / 验证重建**——operator HTTP rebuild 端点本 PR 未接线（需 AdminListener + operator-credential auth），rebuild 正确性由 ① testcontainers 白盒独立证明，corebundle rebuild 端点接线为 follow-up #2209）；**e2e 绿后同 PR 删 `GOCELL_PROJECTION_PG_JOURNAL_PREVIEW` gate** → durable journal 成生产默认（无 env opt-out，per 不留软回退）→ `202605261620` compensation 段原地重写为已落地表述 | D1/D9（已验证 + 已 flip） | PR-03 + PR-05 前置均已满足；T-06-2 已通过；**gate 已删** |
...
依赖：PR-01→02→03 串行；PR-04 依赖 PR-03 + no-DELETE 守卫前置——均已满足。**关键不变式（已满足）**：fail-closed gate 的移除（production-default flip）已在 PR-04（#1771）完成——T-06-2 e2e 已通过、`GOCELL_PROJECTION_PG_JOURNAL_PREVIEW` gate 已删除；durable journal 现为生产默认。#1482（per-spec replay filtering）已落（Coordinator 层，与位置源正交，无需改）。
