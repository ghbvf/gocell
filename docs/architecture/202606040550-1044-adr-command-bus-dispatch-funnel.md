...
**落地形态**：`examples/iotdevice/cells/devicecell/slices/devicecertrenewal.Reconciler` 实现 `reconcile.Reconciler`（producer 落在专属 `devicecertrenewal` slice，声明 `command.devicecommand.enqueue.v1` `role: invoke` + waiver——formal contract-usage 所有权，#1812 F3；证书 store 仍在共享 `internal/devicecert`，由 deviceregister 播种），由 cell 的第二个 `reconcile.Loop`（`devicecert.renewal`，`TickerTrigger` off `c.clk`，`buildCertRenewalSweeper`，镜像既有 `buildCommandSweeper`）驱动。每 tick 扫 cell 内 ephemeral 证书 store 的近过期证书 → 对每台设备经 `command.EmitAsync(ctx, clk, bootstrapEmitter, cmdenqueue.DispatchID, deviceID, commandID, {commandType: "rotate-cert", …})` emit 一条 `command.devicecommand.enqueue.v1`，复用 #1698 已激活的 bootstrap emitter/txRunner + 既有 `WithCommandDispatch` dispatcher-map（**复用同一 enqueue dispatch，无新 map 项、无新契约**）。

**幂等 forcing function（#1812 F1 重评）**：sweep 每 interval 都扫到同一台未续期设备，但 cert store 记录该 epoch 已请求续期、`ScanNearExpiry` 跳过已请求 epoch → 同一 epoch 跨整个（多日）near-expiry 窗口仅 emit **一条** entry（single-emit 由 store 状态保证，**不依赖** relay 的 24h command-done TTL）。`commandID` 由 `(deviceID, certEpoch)` 经单源 helper `rotateCommandID` 确定性派生；relay Claimer 按 `DeriveCommandKey(tenant, deviceID, commandID)` 去重，仅作同窗口重发（same-tick / emit 已提交但 mark 丢失）的二级兜底。`cert_renewal_e2e_test.go` 全链路锁：2 tick → **1** outbox entry（store 级 per-epoch dedup）→ 1 dispatch → 1 可 dequeue 的 rotate-cert 命令。post-rotation 重签发（新 epoch）→ 新 commandID → 可再 dispatch。

**安全声明重评（ai-robust ADR amendment 必查）**：原落地把「设备永不被刷 N 条续期命令」寄托在 relay Claimer 的 24h done-TTL 上，但 near-expiry 窗口为 30d、sweep interval 1h——24h 后 done-key 过期即每日重发（#1812 F1 缺陷）。修正后 single-emit 由 cert store 的 renewal-requested 状态闭合、覆盖整个窗口、与 24h TTL 解耦；威胁「设备被刷命令」由 store 状态保证（Medium：违反可被 reconciler/store 单测 + e2e 抓住），Claimer 退为二级兜底。
...
**无 ✅→⚠️/❌ 降格，无补偿措施**：未改 sealed `Entry` wire envelope、未新增 errcode/Kind/schema/migration/interface 签名——**未触发 contract-fanout 5 载体**（rotate-cert 内嵌既有 `enqueue` 的 opaque `payload`，`commandType` 字段既存）。enforcement 索引（§7）**无新增**。

**范围（显式，非 silent defer）**：证书状态（含 renewal-requested epoch 标记）用 **cell 内 ephemeral 内存 store**（`internal/devicecert.Store`，非持久化到 `devices` 表）——不动 `adapters/postgres` migration / `schema_guard.go` / PG device repo；durable 重启后 store 为空、仅对重注册设备再签发（renewal 标记同失，重发由 Claimer 24h 兜底），是 reference 样例运行期操作态语义（store godoc 写明）。设备侧轮换完成（epoch advance / 应用新证书）属设备 dequeue 后固件行为，在本 producer 范围外。archetype ③（saga step→command）仍为独立 issue。
