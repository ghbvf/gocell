...
**落地形态**：`examples/iotdevice/cells/devicecell/internal/devicecert.Reconciler` 实现 `reconcile.Reconciler`，由 cell 的第二个 `reconcile.Loop`（`devicecert.renewal`，`TickerTrigger` off `c.clk`，`buildCertRenewalSweeper`，镜像既有 `buildCommandSweeper`）驱动。每 tick 扫 cell 内 ephemeral 证书 store 的近过期证书 → 对每台设备经 `command.EmitAsync(ctx, clk, bootstrapEmitter, cmdenqueue.DispatchID, deviceID, commandID, {commandType: "rotate-cert", …})` emit 一条 `command.devicecommand.enqueue.v1`，复用 #1698 已激活的 bootstrap emitter/txRunner + 既有 `WithCommandDispatch` dispatcher-map（**复用同一 enqueue dispatch，无新 map 项、无新契约**）。

**幂等 forcing function**：sweep 每 interval 都扫到同一台未续期设备 → 每 tick emit 一条 entry；`commandID` 由 `(deviceID, certEpoch)` 经单源 helper `rotateCommandID` 确定性派生，relay Claimer 按 `DeriveCommandKey(tenant, deviceID, commandID)` 去重 → 同一 epoch 跨 tick 仅 dispatch 一次（cross-tick dedup，由 `cert_renewal_e2e_test.go` 全链路锁：2 tick → 2 outbox entry 同 claim key → 1 dispatch → 1 可 dequeue 的 rotate-cert 命令）。post-rotation 重签发（新 epoch）→ 新 commandID → 可再 dispatch。
...
- `COMMAND-ASYNC-EMIT-FUNNEL-01`：✅ **不变**（上游 **Medium** / 下游 **Hard**）。新增**第 2 个** `command.EmitAsync` 生产调用方（`devicecert.Reconciler`，archetype ②；archetype ① devicebootstrap 为第 1 个）。上游单包守卫「`command.*` topic 的 raw `kout.NewEntry`/`Emit` 仅限 `runtime/command` 包」对新 producer **by-construction 合规**（走 sanctioned `EmitAsync` 公门）→ **无 archtest 改动**，坐实 issue「零新增 funnel」；won't-do ceiling 不变。下游 `EmitAsync` subject/commandID required 位置参（Hard）继续保证「emit 异步命令必带身份槽」编译期不可绕过。
...
**范围（显式，非 silent defer）**：证书状态用 **cell 内 ephemeral 内存 store**（`internal/devicecert.Store`，非持久化到 `devices` 表）——不动 `adapters/postgres` migration / `schema_guard.go` / PG device repo；durable 重启后 store 为空、仅对重注册设备再签发，是 reference 样例运行期操作态语义（store godoc 写明）。设备侧轮换完成（epoch advance / 应用新证书）属设备 dequeue 后固件行为，在本 producer 范围外。archetype ③（saga step→command）仍为独立 issue。
