...
`.github/workflows/_build-lint.yml` tools shard `pkgs` 改为 sentinel `_dynamic_archtest_excluded`；Test step 用 `go list ./tools/...` + grep 排除 archtest 顶层包。**违反「漏覆盖新 tools 子包」在 type system 不可表达**：任何新加 `tools/<sub>` package 自动入列，archtest 是唯一显式排除目标。AI-rebust **Hard**，无需 archtest 元规则守卫。
