...
| **PROJECTION-CONSISTENCY-01** (codegen funnel) | gh #960 (delivered) | 下游 **Hard** — contractgen emits `const _ = uint(cellvocab.<level> - cellvocab.L3)` into the projection `types_gen.go`; a level below L3 overflows uint at compile time, so an invalid `codegen=true` projection cannot exist in a buildable tree. The governance rule `PROJECTION-CONSISTENCY-01` is the **Medium** backstop for `codegen=false` contracts + in-memory fixtures. The original "parser load-time `jsonschema.Validate`" framing was **rejected** (a parse-time validator is a Medium runtime guard and does not cover the in-memory vector — see §Amendment 2026-06-02 #960). | codegen funnel + compile-error downstream |
...
gate), `triggers` must be added to the schema first.
