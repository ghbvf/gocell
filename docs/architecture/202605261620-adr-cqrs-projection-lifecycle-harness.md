...
- **Argument.** A is the lowest-complexity safe v1. The PG checkpoint schema
  ...
  `token_entry.owner`). `SaveOffset` INSERT/UPDATE paths **do not touch owner**
  (base CheckpointStore contract). `PROJECTION-CHECKPOINT-OWNER-COLUMN-V1-RESERVED-01`,
  the v1-scoped AST guard for this omission, was **retired in PR-PG (#1630
  Batch 2)** when `OwnerCheckpointStore.AdvanceIfOwner` activated the owner
  write path. The replacement enforcement is the `OwnerCheckpointStore` interface
  narrowing (type-system Hard: `SaveOffset` is absent from the interface, so a
  deposed leader holding an `OwnerCheckpointStore` cannot bypass fencing via
  `SaveOffset`) + `SAGA-OWNER-CHECKPOINT-CONFORMANCE-ENROLL-01` (Medium).
  Because `SaveOffset` still omits `owner`, the NOT NULL constraint is satisfied
  solely by the `DEFAULT ''`; that load-bearing default is still asserted at
  startup by `schema_guard.verifyDefaults`. Multi-pod safety in v1 is the
  responsibility of upper-layer leader election; the `OwnerCheckpointStore` CAS
  is the per-projection fencing mechanism (D5(b) of #1609 ADR).
...
| 7 | **multi-pod concurrency (v1 boundary)** | v1 single-pod (Q5); `owner` column activated by `OwnerCheckpointStore.AdvanceIfOwner` (PR-PG, #1630 Batch 2); multi-pod safety = upper-layer leader election + per-projection distlock + CAS; **2+ replicas without leader election is unsafe** | `schema_guard.verifyDefaults` asserts the load-bearing `owner DEFAULT ''` (still needed for `SaveOffset`); `OwnerCheckpointStore.AdvanceIfOwner` CAS fences checkpoint advance (D5(b)); `SAGA-OWNER-CHECKPOINT-CONFORMANCE-ENROLL-01` ensures every impl passes conformance; documented v1 limitation still applies (Q5) |
