...
| D4 | **tailing = 独立 `Tailer`（Option B）**：自有 `Start/Stop/tickLoop`，持 `journal.GlobalReader`，**共享同一 `distlock.Locker` 实例但用 per-(cellID, projectionID) key**（如 `saga-journal-tailer:{cellID}:{projection}`——投影身份是 (cellID, projectionID) 对，两 cell 可合法共用同一 projectionID，key 必须带 cellID 否则跨 cell 抢同一 leader 锁）；**不** piggyback 进 saga `Coordinator.tickOnce`（saga distlock 是 per-instance 粒度、无 process-level global leader 可复用；5 框架对标一致） | 单 checkpoint-advancer funnel + leader-gate | advancer funnel 下游 **Hard**（caller-allowlist，go/types 解析）+ 上游 **Medium**（Go 可见性天花板；Hard 化追踪 **gh #1612**） |
...
| 威胁 | 机制 | 覆盖 | 遗留 |
|------|------|------|------|
| leader 交接 mid-drain，checkpoint 推进不安全 | per-projection distlock + apply/advance 同事务（窄 `OwnerCheckpointStore`）+ 单调-offset CAS（旧 leader 落后 advance 必败，语义同 `OUTBOX-LEASE-ID-CAS-01`） | ✅ | **PR-04 已交付 mem CAS**：新窄接口 `projection.OwnerCheckpointStore.AdvanceIfOwner(ownerToken, offset)`（semantics B：`token==owner` 或 `offset>committed` 才接受，否则 `ErrStaleOwner`）+ `MemOwnerCheckpointStore` + `RunOwnerCheckpointConformance`。**PR-PG（#1630 Batch 2）已交付 PG 侧 CAS**：`ProjectionCheckpointStore.AdvanceIfOwner` 实现 SELECT FOR UPDATE + 条件 INSERT/UPDATE，通过 `projectiontest.RunOwnerCheckpointConformance` 闭环验证 + 并发 leader-handoff 集成测试。`PROJECTION-CHECKPOINT-OWNER-COLUMN-V1-RESERVED-01` 已退役；替换为 `OwnerCheckpointStore` 接口窄化（type-system Hard：SaveOffset 无 owner 写路径）+ `SAGA-OWNER-CHECKPOINT-CONFORMANCE-ENROLL-01`（Medium，PG impl 自动纳入）。 |
...
| **全局 journal 跨租户可见**（saga journal 是 model-a 全局 `global_seq` 序列、无 tenant 分区——A 租户的 saga 事件经全局序列流过 B projection 消费者；§Amendment 2026-06-15 新增，#1882/#1883） | 纵深四层：① per-spec topic filter（`PROJECTION-REPLAY-PER-SPEC-FILTER-01`，投影只 apply 自身 stream）；② **handler 按事件自身 tenant 自滤**写对应租户读模型（对标 Marten conjoined `tenant_id` 模式：事件载体含 tenant、消费端定向，非依赖 ctx——`ref: Marten events/multitenancy`）；③ system 重放清空 ctx.tenant（`InstallSystemPrincipal`，见上 impersonation 行）→ 无 handler 继承陈旧 ambient tenant；④ **typed `tenant.FromContext` 对空/nil tenant Hard fail-close（→403，`ParseTenantID` 拒空串）**（既存）+ **`CTXKEYS-TENANT-READ-CALLER-01`**（裸 `ctxkeys.TenantIDFrom` caller-funnel，业务/Apply 裸读 → CI 红，逼回 typed accessor）→「空 ctx tenant ⟹ 跨租户全权」在业务码**不可表达** | ⚠️ | handler「写哪个租户读模型」的语义（忽略事件 tenant、写固定/全租户目标）是通用正确性 bug，未机器强制（残留盲区，已登记 backlog **gh #2203**）。identity 与 row-scope 正交：system 重放是 identity，**不**复用 `RowScopeAll` / 不经 `ROWSCOPEALL-AUDIT-FUNNEL-01`（业界对标确认无框架把 replay 身份建模成 row-visibility 枚举） |
| checkpoint advance 多写入路径 | 单 advancer funnel：`AdvanceIfOwner` sanctioned caller set = `{commitEvent（正常 apply 推进）, skipPoisonEvent（poison-event dead-letter skip，#2110 新增）}`，由 `SAGA-TAILER-CHECKPOINT-ADVANCER-CALLER-01` caller-allowlist 锁（下游 Hard，go/types）。`skipPoisonEvent` 是永久 apply 错误时记 dead-letter 并推过 checkpoint 的合法 advance 路径，详见 ADR `202606191200-2110-adr-saga-tailer-poison-dead-letter.md` | ✅（PR-04 已交付；#2110 扩 allowlist） | 上游 Medium（Go 可见性天花板——`AdvanceIfOwner` 是导出接口方法，无法编译期强制单一 caller；Hard 化追踪 **gh #1612**，同 #851/#893/#1282 族） |
...
| `SAGA-TAILER-CHECKPOINT-ADVANCER-CALLER-01`（PR-04 已落；#2110 扩 allowlist） | saga 投影 checkpoint advance（`OwnerCheckpointStore.AdvanceIfOwner`）只在 sanctioned caller set `{runtime/saga/tailer.(*Tailer).commitEvent（正常 apply 推进）, runtime/saga/tailer.(*Tailer).skipPoisonEvent（poison-event dead-letter skip，#2110 新增）}` 调用（FuncLit 下降归属）。`skipPoisonEvent` 是永久 apply 错误时记 dead-letter 并推过 checkpoint 的合法 advance 路径，见 ADR `202606191200-2110-adr-saga-tailer-poison-dead-letter.md`。落地于 `tools/archtest/saga_invariants_test.go`（`SAGA-INVARIANTS-FILE-CONSOLIDATED-01` 要求 `SAGA-*` 同文件） | 下游 **Hard**（caller-allowlist，go/types）+ 上游 **Medium**（Go 天花板；Hard 化追踪 **gh #1612**，PR-04 archtest godoc 点名该 issue） |
| `SAGA-OWNER-CHECKPOINT-CONFORMANCE-ENROLL-01`（PR-04 已落；PR-PG enroll 已落） | 每个 `projection.OwnerCheckpointStore` 实现入 `projectiontest.RunOwnerCheckpointConformance`（cold claim / same-owner re-advance / new-leader claim-ahead / stale-owner reject）。实现：`MemOwnerCheckpointStore`（PR-04，已 enroll）+ `ProjectionCheckpointStore`（PR-PG，#1630 Batch 2，已 enroll）| **Medium**（Hard 路径 = codegen golden 枚举实现，gh #1003 同源） |
...
| `CTXKEYS-TENANT-READ-CALLER-01`（本 PR #2198 落地，#1882） | 裸 `ctxkeys.TenantIDFrom`（tenant ctx 键唯一裸读）生产引用点冻结到 3 个 infra allowlist（`pkg/tenant.FromContext` 自身 / `kernel/outbox/principal.go` / `adapters/postgres/tx_manager.go`），业务/Apply 裸读 → CI 红，逼回 fail-close 的 typed `tenant.FromContext`，封闭「空 ctx tenant ⟹ 跨租户全权」误判面（§5 跨租户行机制④）。落地于 `tools/archtest/ctxkeys_tenant_read_caller_test.go`（权威盲区清单在该 godoc） | 下游 **Hard**（go/types caller-allowlist，alias/dot-import/函数值引用不可绕）+ 上游 **Medium**（`TenantIDFrom` 跨包必导出、Go 可见性无法限定单一 caller，永久天花板，同 #851/#893/#1282 族——非待办 TODO，与 `CTXKEYS-REALIP-READ-CALLER-01`/`CTXKEYS-PRINCIPAL-WRITE-CALLER-01` 同模式）。与 typed accessor 空/nil 值拒绝（既存 **Hard**）合为双层 |
...
| PR-PG（**已落，#1630 Batch 2**） | PG saga-journal source 生产化（Batch 1：global_seq scan + conformance）+ 激活 `projection_checkpoints.owner` 列 CAS（D5(b) PG 侧 fencing，Batch 2）：`ProjectionCheckpointStore.AdvanceIfOwner` SELECT FOR UPDATE + 条件 INSERT/UPDATE；`PROJECTION-CHECKPOINT-OWNER-COLUMN-V1-RESERVED-01` 退役；conformance + concurrency integration test 落地 | D3/D5 |
