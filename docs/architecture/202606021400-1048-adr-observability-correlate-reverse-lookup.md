...
| 威胁 | 防御 | 评级 |
|------|------|------|
| 业务代码伪造 `Correlation` read-model | 全字段 unexported → 包外 struct literal 编译不可表达（Hard against literal-fab）；但 `New(traceID, requestID, correlationID string)` 是公开通用构造器，任何代码均可调用 `correlation.New("fake", …)`——`Correlation` 本身不是 provenance 边界。trace_id provenance 可信性来自：(a) 上游 sealed `outbox.Entry`（appender 读 `entry.Observability()` 值，Hard 继承 OUTBOX-ENTRY-SEALED-CONSTRUCTION-01）；(b) 下游 `AUDIT-TRACE-ID-WRITE-CALLER-01` caller-allowlist（Medium，appender 是唯一写 `ledger.Entry.TraceID` 的生产点） | **seal=Hard（unexported 字段拒 external literal-fab）；非 provenance 边界（`New` 公开）；trace_id provenance = 上游 sealed outbox.Entry（Hard 继承）+ 下游 AUDIT-TRACE-ID-WRITE-CALLER-01（Medium）** |
