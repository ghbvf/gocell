...
3. **Static enforcement (archtest `FIXTURE-CELLID-TYPED-BUILDER-01`)**:
   ...
   - **A3** (meta self-test): `archtest_fixture` sub-package containing deliberate bad/good usages; asserts A1 fires on bad and stays silent on good. Includes `good_framework_sentinel.go` (sentinel at owner/provider positions → no false positive) AND `bad_framework_sentinel_at_id.go` (sentinel at the real-cell `CellMeta.ID` position → must be flagged), the field-aware pair (§1a, #2178).
...
Therefore A1 accepts `metadata.FrameworkOwnerSentinel` as a third sanctioned source, identity-locked via TypesInfo: the resolved object must be `*types.Const` with `Pkg().Path() == metadataPkgPath` and `Name() == "FrameworkOwnerSentinel"`. A homonymous const from another package resolves to a different package path and is rejected.

**Field-position-aware acceptance (#2178).** The sentinel is sanctioned **only at owner/provider-endpoint positions** — the `allowFrameworkSentinel` column of `cellIDFieldPositions` is true exactly for `ContractMeta.OwnerCell` and `EndpointsMeta.{Server,Publisher,Handler,Provider}`. At every real-cell position (`CellMeta.ID`, `ProjectMeta.Cells` map keys, `SliceMeta.BelongsToCell`, `AssemblyCellRef.ID`, `L0DepMeta.Cell`, consumer endpoint lists) the sentinel is **rejected** — those positions require a legal cell id, and `"_framework"` is not one (locked by `metadata.TestFrameworkOwnerSentinel_NotALegalCellID`'s `!MatchCellID` assertion). This mirrors Kubernetes' field-specific name validators (`NameIsDNSLabel` vs `NameIsDNSSubdomain`): the underscore exception is bound to the specific semantic surface, not granted globally. The earlier "accept everywhere, FMT-C1 will catch CellMeta.ID" rationale was rejected — that assumption was never test-locked, and a good-fixture that asserted `CellMeta.ID == sentinel` is acceptable actively codified the imprecision. A1 now rejects it directly; the RED `bad_framework_sentinel_at_id.go` + GREEN `good_framework_sentinel{,OtherEndpoints}` pair proves both directions. Whether a framework-owned *command/projection* is eligible is `FRAMEWORK-OWNED-CONTRACT-SCOPED`'s concern (not A1's), so all four provider endpoints accept the sentinel.
