...
| `runtime/auth/provider.go:MustNewTestKeyProvider` | `runtime/auth/keystest/keys.go` | 同上 |
...
| `cmd/corebundle/secrets.go` | 1 | `auth.MustGenerateTestKeyPair` → `auth.GenerateRSAKeyPair` + godoc dev-only |
...
- `runtime/auth/keystest/keys.go`（`MustGenerateKeyPair`、`MustNewKeySet`、`MustNewKeyProvider`）
