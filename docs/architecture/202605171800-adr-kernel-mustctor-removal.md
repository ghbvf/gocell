...
- `runtime/auth/authtest/`（新建，承接迁移来的三个 Must*）
- `pkg/contracttest/`
...
| 现位置 | 新位置 | 理由 |
|---|---|---|
| `runtime/auth/keys.go:MustGenerateTestKeyPair` | `runtime/auth/authtest/keys.go` | K8s `httptest` 范本：物理迁出 production namespace |
| `runtime/auth/keys.go:MustNewTestKeySet` | `runtime/auth/authtest/keys.go` | 同上 |
| `runtime/auth/provider.go:MustNewTestKeyProvider` | `runtime/auth/authtest/provider.go` | 同上 |

迁包同时在 `runtime/auth/authtest/keys.go` 新增 error-first `GenerateRSAKeyPair`，
...
- 反向自检 fixture：`tools/archtest/internal/kernelmustctorfixture/redfixture/decl.go`
  包含故意违规的 `func MustViolation()`，子测试断言命中 fixture 且未命中 production
  carve-out
...
- AI 在 production 路径写 `auth.MustGenerateTestKeyPair()` → symbol 物理迁到 `authtest/`
  → import 自动暴露调用者语义（"我在 import test-only 包"），archtest 可以低成本锁定
  production 包不准 import `authtest/`
...
| `runtime/auth` | 5 | `MustNewProtocol`（session）、`MustFromContext`、`MustMount`、`MustGenerateTestKeyPair`（迁包）、`MustNewTestKeySet`（迁包） |
| `runtime/auth` | +1（迁包） | `MustNewTestKeyProvider`（迁包） |
...
| `cmd/corebundle/secrets.go` | 1 | `auth.MustGenerateTestKeyPair` → `authtest.GenerateRSAKeyPair` + godoc dev-only |
...
- `cmd/corebundle/*_test.go` 中 `auth.Must*` → `authtest.*`
- `kernel/wrapper/handler_test.go`、`consumer_test.go` 中 panic 验证测试 → error-first 测试

### 新增文件

- `runtime/auth/authtest/keys.go`（`MustGenerateTestKeyPair`、`MustNewTestKeySet`、新增 `GenerateRSAKeyPair`）
- `runtime/auth/authtest/provider.go`（`MustNewTestKeyProvider`）
...
# authtest 循环 import 检查
go list -json ./runtime/auth/authtest/... | grep '"Imports"'
# 期望：不包含 "runtime/auth"（authtest 不可 import 被测包）
```
...
ref: kubernetes/kubernetes `k8s.io/apimachinery/pkg/api/resource` MustParse godoc;
uber-go/fx error-first constructor convention; go-kratos/kratos middleware option pattern;
zeromicro/go-zero `MustNewServer` cmd-layer pattern.
