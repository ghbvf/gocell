...
3. **role_assignments revoke 路径**：`RemoveFromUserIfNotLast` 必须保证至少一个 admin 的 single-admin 不变量，而该不变量在高并发撤权场景下无法仅靠 partial UNIQUE 索引串行化。
...
**对应代码**：`cells/accesscore/slices/sessionlogin/service.go` `Login` 方法内 `RunInTx + GetByUsernameForUpdate`

在 `Login` 事务内，先以 `SELECT ... FOR UPDATE` 拿住 `users` 行，再检查状态（locked / not found）并写入 `sessions`：
...
### 策略 3：pg_advisory_xact_lock（single-admin revoke 路径）

**对应代码**：`adapters/postgres/user_roles_repo.go` `RemoveFromUserIfNotLast`
...
`$1` 为 `role_id`（目前唯一的 builtin role 为 `'admin'`）。同 role 下的并发 revoke 请求在此串行排队；事务提交后，下一个请求才会进入临界区并重新检查 "至少还有一个 admin"。

**选择理由**：

- **high-contention safety-critical singleton**：单一 admin 不能被撤销，必须串行化。partial UNIQUE 索引（`WHERE role_id = 'admin'`，`019_roles.sql`）只能防止表层约束被直接写入突破，无法防止"check-then-act"并发窗口：两个并发 revoke 同时读到"还有 2 个 admin"，然后同时删一个，最终只剩 1 个，但若两者各自认为对方还在，则都会执行删除，导致 0 个 admin。
...
- **BUILTIN-ROLE-ID-NAME-EQ-01 archtest**（`tools/archtest/builtin_role_invariants_test.go`）锁定了 `runtime/auth.RoleAdmin = "admin"` 这一隐式约定。如果未来引入 id≠name 的 builtin role，必须同时更新 `019_roles.sql` 的 partial UNIQUE 表达，并放宽对应断言。
...
- `adapters/postgres/migrations/019_roles.sql`：roles + partial UNIQUE（single-admin）
- `tools/archtest/builtin_role_invariants_test.go`：BUILTIN-ROLE-ID-NAME-EQ-01
- PR #417 review P2c（single-admin 不变量绑定分析）、P2f（文档同步）
