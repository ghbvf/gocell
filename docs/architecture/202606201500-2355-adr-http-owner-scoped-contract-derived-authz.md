...
owner/self-scoped 基座落地后，其余 served-HTTP cell 续波迁移（devicecell/ordercell examples、auditcore-list query-param）见 #2355 续波 PR。全部迁完后：ledger 清零 + FMT-42 收紧为强制 + 裸 `RequirePermission*` 收口为「仅生成 Contract helper 可调」（funnel 终态闭环）。
