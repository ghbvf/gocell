...
**怎么做**：
- ✅ `runtime/observability/poolstats` 接口下沉到 `kernel/observability/poolstats`，adapters 通过 DI 接收 sink — **完成于 PR#387**（merged）。新增 archtest `KERNEL-POOLSTATS-LOCATION-01a/01b` 守护：禁止旧路径 import + contract 包零非 stdlib 依赖。
- ✅ ~~Noop 集中到 `runtime/testutil/`，archtest `NOOP-PROD-IMPORT-01` 守护~~ — superseded：等价异形 = `kernel/cell/durability.go::Nooper + CheckNotNoop`。生产 cell 在 `Init()` 调 `cell.CheckNotNoop(durabilityMode, cellID, deps...)`，Durable 模式下任一 dep 实现 `Nooper` 立即拒绝。对标 OTel/fx/Watermill/K8s client-go/Prometheus 均无 archtest 守卫此场景，本项不再追加编译期 archtest。
- ✅ ~~合并 `metadata.CellMeta` 与 `cell.CellMetadata` 单一类型~~ — 完成：仅 `kernel/metadata/types.go::CellMeta` 单源；`type CellMetadata` 已不存在。archtest `CELLMETA-SINGLE-SOURCE-01`（`tools/archtest/cellmeta_single_source_test.go`）守卫 `kernel/cell` 不得定义 `CellMetadata` / `Owner` / `SchemaConfig` / `CellVerify` / `L0Dep`；codegen K#04/K#05 守卫 cell.go 不得直接写 `CellMeta{}` literal，必须经 `cell_gen.go::loadCellMetadata()` 单一入口。
- ✅ ~~adapters 公开 API 重审：amqp.* / \*sql.DB / \*amqp.Error 全部包装~~ — 实质已完成：`AMQPConnection` / `AMQPChannel` / `DialFunc` interface（`adapters/rabbitmq/connection.go:334-362`）封住 amqp 类型；`pgx.Tx` 经 `kernel/persistence/txctx.go::TxCtxKey` + cell-side `tx interface` 解耦（`pgxpool.Pool` 与 `pgx.Tx` 同时满足）；`*amqp.Error` 仅出现在 unexported wrapper / unexported method；cells/runtime/cmd 三层 0 处 import 第三方 SDK 类型。`*sql.DB` 不适用（项目使用 pgx 而非 database/sql）。
