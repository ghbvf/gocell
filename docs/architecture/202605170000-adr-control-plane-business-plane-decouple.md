...
- **A1-1 (review P1-1) — startup deadlock backstop**: `Bootstrap.Run` now
  ...
  `ownerCancel()`s, waits **bounded by `startupUnwindGraceTimeout` (5s)** for
  `lifecycle.Start` to unwind, then rolls back. A ctx-respecting OnStart
  unwinds in microseconds (clean path, unchanged). A ctx-IGNORING OnStart
  never observes the cancel; in Go that goroutine cannot be force-killed
  (irreducible residual), so once the grace elapses it is **abandoned** and
  `superviseLifecycleStart` returns anyway — Run() rolls back, exits, and the
  orchestrator restarts the process, instead of Run() wedging forever on a
  bare `<-startErr` (the pre-amendment behaviour). The owner-ctx single-truth
  (§D-B) is unchanged — the bound lives at the orchestration layer (mirrors
  ...
  `ErrBootstrapLifecycle` — no new errcode); the abandon path joins it with an
  explicit "goroutine abandoned (OnStart ignored ctx)" wrap.
...
（解除响应 ctx 的 wedged OnStart）→ 在 `abortStartupAndUnwind` 内以
`startupUnwindGraceTimeout`（5s 内部常量，非运维 SLA）**有界**等待 Start
unwind：

- **响应 ctx 的 hook**（含 respecting-but-slow）：grace 内 unwind 完成，返回
  `errors.Join(cause, unwind)`——与 amendment 前行为完全一致，零可观测变化
  （预算路径 cause = `ErrBootstrapStartupTimeout`，复用 `ErrBootstrapLifecycle`，
  不新增 errcode）。
- **忽略 ctx 的 hook**：grace 耗尽，该 `lifecycle.Start` goroutine 被
  **abandon**（Go 无法强杀，leaked goroutine 泊到进程退出——不可消除的物理
  残余），`superviseLifecycleStart` 仍返回（join `cause` +
  `"goroutine abandoned after 5s unwind grace (OnStart ignored ctx)"` wrap），
  Run() 据此 rollback + 退出，由 orchestrator 重启进程。**编排层有界**——不再
  退化为 amendment 前的裸 `<-startErr` 无界 re-wedge；与同仓库
  `kernel/assembly/hookDispatcher` 放弃 stuck sink 的范式一致。
...
> Run() 仍 wedge"的退化尾巴，现编排层确定性返回，残余收窄为不可消除的物理
> leaked goroutine（无格子退化，仅改善）。下方新增 A1 引入/暴露的威胁行。
...
| OnStart 永不返回 wedge `Run()` | 缓解（A1-1 amended） | `superviseLifecycleStart`：caller ctx cancel 或 `WithStartupTimeout` 预算耗尽触发 `ownerCancel()`；`abortStartupAndUnwind` 以 `startupUnwindGraceTimeout`（5s）**有界**等待 unwind。响应 ctx 的 hook（含 slow）grace 内 unwind，行为同 amendment 前。**忽略 ctx 的 hook**：grace 耗尽即 abandon 该 `lifecycle.Start` goroutine 并返回，Run() 确定性 rollback + 退出，orchestrator 重启——编排层不再随违规 hook 一起无界 wedge。残余收窄为**唯一不可消除项**：被 abandon 的 leaked goroutine 泊到进程退出（Go 无法强杀任意 goroutine，A1/B 任何方案皆同），归属违规 hook 自身 bug（hook 必须响应 ctx）。回归测试 `TestSuperviseLifecycleStart_CtxIgnoringHookDetaches` 钉死 detach 路径。|
