...
| OnStart 永不返回 wedge `Run()` | 消除（A1-1） | `superviseLifecycleStart`：caller ctx + `WithStartupTimeout` 预算，触发即 ownerCancel + rollback；原"StartTimeout informational ⇒ 无 backstop"残留已闭环。残留诚实声明：忽略 ctx 的真·wedged OnStart goroutine 本身无法回收（Run 仍能返回，goroutine 泄漏由该 hook 自身 bug 负责）|
