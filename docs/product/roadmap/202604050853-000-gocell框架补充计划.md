...
### Week 6 (Days 43-49): accesscore Cell

当前 accesscore 事实源为 9 个 slice：

| Slice | 现有? | 内容 |
|-------|-------|------|
| identitymanage | 新增 | 用户 CRUD, 锁定/解锁 |
| sessionlogin | 已有 YAML | OIDC/密码登录, JWT 签发, session 创建 |
| sessionrefresh | 新增 | token 刷新, 滚动过期 |
| sessionlogout | 新增 | session 吊销 + event.session.revoked 发布 |
| authorizationdecide | 新增 | RBAC 权限判定 |
| sessionvalidate | 已有 | 内部会话验证 service capability |
| rbaccheck | 已有 | RBAC 角色查询与检查 |
| rbacassign | 已有 | internal role assign/revoke |
| configreceive | 已有 | 配置变更接收 |
...
`sessionvalidate` 与 `authorizationdecide` 不作为公开 HTTP route 记录；公开 route 以 active `contract.yaml` 为准。

当前契约覆盖：session created/revoked、user created/locked、role assigned/revoked、login/refresh/logout、user CRUD/lock/unlock/change-password、RBAC list/check、internal assign/revoke。
补齐 Journey：J-sessionrefresh, J-sessionlogout, J-useronboarding, J-accountlockout

### Week 7 (Days 50-56): auditcore Cell

当前 auditcore 为 4 个 slice：auditappend, auditverify, auditquery, auditarchive
- HMAC-SHA256 hash chain 实现
- 消费 session.*/user.*/config.* 事件
- 补齐契约：`event.audit.integrity-verified.v1`
- 补齐 Journey: J-auditlogintrail（跨 cell）

### Week 8 (Days 57-63): configcore Cell

当前 configcore 为 6 个 slice：configread, configwrite, configpublish, configsubscribe, featureflag, flagwrite
- 配置 CRUD + 版本管理 + 热更新事件
- Feature flags（开关/灰度/rollout）
- 当前契约覆盖 config get/write/publish/rollback 与 flag list/get/evaluate/create/update/toggle/delete
- 补齐 Journey: J-confighotreload, J-configrollback（跨 cell）

**Phase 2 Gate**: 3 个 cell 在 corebundle assembly 中运行，8 条 journey 通过
...
- `examples/ssobff/` — SSO 完整登录流程
- `examples/todoorder/` — CRUD + 事件驱动 + 自定义 cell
- `examples/iotdevice/` — L4 设备管理
...
Phase 2 (6-8 parallel):
  runtime/http ──┐
  runtime/auth ──┤
  runtime/obs  ──┼── cells/accesscore ──┐
  runtime/worker ┤   cells/auditcore  ──┼── Integration
  runtime/config ┘   cells/configcore ──┘
...
| 2 | `./gocell verify cell --id=accesscore && ./gocell verify journey --id=J-ssologin` |
| 3 | `docker compose up -d && go test ./adapters/... -tags=integration` |
| 4 | `cd examples/ssobff && go run .` — 30 分钟内完成 |
