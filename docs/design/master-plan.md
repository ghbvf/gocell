...
**Week 11 — Examples：**
- examples/ssobff / todo-order / iot-device
...
**GoCell 不是"再多几个中间件"，而是把 Cell/Slice 运行时 + metadata + assembly + outbox/replay/reconcile + verify + journey/status + webhook/feature-flags 做成真正可复用的 cell-native 框架，并内置 accesscore + auditcore + configcore 三个开箱即用的通用 Cell。**
