...
| **Built-in Cells** | 开箱即用的通用 Cell | accesscore + auditcore + configcore |
...
| 内置 Cell | **accesscore + auditcore + configcore**（3 个） | v2 更新 |
...
| Feature flags | **进入 Kernel**（configcore 承载） | 531 + 用户要求 |
| Config 热更新 | **做成 Cell + Journey**（configcore） | 用户要求 |
...
### 4.1 accesscore — SSO/BFF 认证

```yaml
# cells/accesscore/cell.yaml（V3 格式）
id: accesscore
type: core
consistencyLevel: L2
owner:
...
```yaml
# cells/auditcore/cell.yaml（V3 格式）
id: auditcore
type: core
consistencyLevel: L2
owner:
...
### 4.3 configcore — 配置热更新 + Feature Flags（v2 新增）

```yaml
# cells/configcore/cell.yaml（V3 格式）
id: configcore
type: core
consistencyLevel: L2
owner:
...
发布契约：event.config.changed.v1 / event.config.rollback.v1 / event.flag.changed.v1
...
| Journey | Cells | 验证要点 |
|---------|-------|---------|
| J-ssologin | access | OIDC 完整流程 |
| J-sessionrefresh | access | token 刷新 |
| J-sessionlogout | access | session 吊销 + 事件 |
| J-useronboarding | access | 用户创建到可登录 |
| J-accountlockout | access | 锁定 + 解锁 |
| J-auditlogintrail | access + audit | **跨 cell** — L2 事件消费 + hash chain |
| J-confighotreload | config + all | **跨 cell** — 配置传播 + 健康验证 |
| J-configrollback | config + all | **跨 cell** — 版本回滚 + 审计 |
...
examples/
├── ssobff/               # SSO + BFF 登录完整旅程
├── todoorder/            # CRUD + 事件驱动
└── iotdevice/            # IoT 设备（L4 DeviceLatent）
```
...
**Week 6 — accesscore Cell：**
- 9 个 slice / JWT / OIDC / 登录锁定 / RBAC hook / internal service capabilities
- J-ssologin 等 5 条 journey

**Week 7 — auditcore Cell：**
- 4 个 slice / HMAC-SHA256 hash chain / audit query / archive stub
- J-auditlogintrail 跨 cell journey

**Week 8 — configcore Cell（v2 新增）：**
- 6 个 slice / 配置 CRUD / 版本管理 / feature flags / 热更新事件
- J-confighotreload + J-configrollback journey
- 跨 cell 配置传播验证
...
**Week 11 — Examples：**
- examples/ssobff / todoorder / iotdevice
...
| 版本 | 内容 |
|------|------|
| **v1.0** | Kernel(45+项) + 3 platform Cell + 19 platform Slice + 8 platform Journey + 3 examples |
...
**GoCell 不是"再多几个中间件"，而是把 Cell/Slice 运行时 + metadata + assembly + outbox/replay/reconcile + verify + journey/status + webhook/featureflags 做成真正可复用的 cell-native 框架，并内置 accesscore + auditcore + configcore 三个开箱即用的通用 Cell。**
