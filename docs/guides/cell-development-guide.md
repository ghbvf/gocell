...
func (c *MyCell) initInternal(ctx context.Context, reg cell.Registrar) error {
    ...
    // 健康探针注册走两个 typed funnel（reg.Healthz() 已删除，由 PROBENAME-SEALED-FUNNEL-01 守 reg.RegisterReadiness 单一写面）：
    // - cell repo readiness：cellgen 生成的 RegisterReadiness(reg, c.store)（healthz_gen.go 产物）
...
> 健康探针只能经 typed funnel 注册：cell repo probe 走 cellgen `RegisterReadiness`，emitter
