...
**绝对禁止跳步。** 每个阶段有明确的入口条件和出口条件，未满足不得进入下一阶段。每个阶段出口必须通过 `python3 .claude/skills/phase-gate/scripts/phase-gate-check.py --stage SN --check exit = PASS` 才可进入下一阶段。
...
所有硬性准入准出条件定义在 `.claude/skills/phase-gate/phase-gates.yaml` 中。本文档和 stage SKILL 中的出口清单仅作为人类参考说明，**不构成独立规则**。如有差异以 YAML 为准。

阶段门脚本（`phase-gate-check.py`）是 **fail-closed** 的：YAML 解析失败、阶段不存在、检查集为空均视为 FAIL。
...
**出口条件**: `phase-gate-check.py --stage S0 --branch {branch} --check exit = PASS`
...
**入口条件**: `phase-gate-check.py --stage S1 --branch {branch} --check entry = PASS`
...
**出口条件**: `phase-gate-check.py --stage S1 --branch {branch} --check exit = PASS`
...
**入口条件**: `phase-gate-check.py --stage S2 --branch {branch} --check entry = PASS`
...
**出口条件**: `phase-gate-check.py --stage S2 --branch {branch} --check exit = PASS`
...
**入口条件**: `phase-gate-check.py --stage S3 --branch {branch} --check entry = PASS`
...
**出口条件**: `phase-gate-check.py --stage S3 --branch {branch} --check exit = PASS`
...
**入口条件**: `phase-gate-check.py --stage S4 --branch {branch} --check entry = PASS`
...
**出口条件**: `phase-gate-check.py --stage S4 --branch {branch} --check exit = PASS`
...
**入口条件**: `phase-gate-check.py --stage S5 --branch {branch} --check entry = PASS`
...
**出口条件**: `phase-gate-check.py --stage S5 --branch {branch} --check exit = PASS`（tasks.md 无未勾选项 + 新增 API 在 contracts/ 有定义）
...
**入口条件**: `phase-gate-check.py --stage S6 --branch {branch} --check entry = PASS`
...
**出口条件**: `phase-gate-check.py --stage S6 --branch {branch} --check exit = PASS`
...
**入口条件**: `phase-gate-check.py --stage S7 --branch {branch} --check entry = PASS`
...
7.1 QA Agent: 执行测试（测试脚本已在 S5 编写完成）
    - 运行: cd src && go test ./... -v -count=1（输出到 evidence/go-test/）
    - 运行: gocell validate（输出到 evidence/validate/）
    - 运行: gocell verify journey --id=<journeyID>（逐个执行，输出到 evidence/journey/）
    - 运行: npx playwright test（仅 QA自动化=ON 且有 UI 时；否则标注 N/A:SCOPE_IRRELEVANT）
    - 收集结果，补充回归脚本（如发现 S5 遗漏的场景）
    - **统一证据目录**: specs/{branch}/evidence/{go-test,validate,journey,playwright}/
...
**出口条件**: `phase-gate-check.py --stage S7 --branch {branch} --check exit = PASS`
...
**入口条件**: `phase-gate-check.py --stage S8 --branch {branch} --check entry = PASS`
...
**出口条件**: `phase-gate-check.py --stage S8 --branch {branch} --check exit = PASS`
...
| A-01 | phase-gate-check.py 依赖 Python+PyYAML。已接受，PyYAML 已安装 | 已关闭 |
