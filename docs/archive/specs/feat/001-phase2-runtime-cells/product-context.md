...
| S2 | Journey 验证通过 | Hard Gate (5 条): J-sso-login / J-session-refresh / J-session-logout / J-user-onboarding / J-account-lockout 必须 PASS；Soft Gate (3 条): J-audit-login-trail / J-config-hot-reload / J-config-rollback 通过 in-memory EventBus 验证 |
...
| S8 | 外部依赖可控 | Phase 2 新增直接依赖 6 个（go-chi/chi/v5, golang.org/x/crypto, fsnotify/fsnotify, prometheus/client_golang, go.opentelemetry.io/otel, golang-jwt/jwt/v5），不主动引入白名单外的直接依赖 |
