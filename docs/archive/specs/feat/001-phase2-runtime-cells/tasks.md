...
### [x] T-030: runtime/http/health [S] → T-028
- health.go + health_test.go
- /healthz + /readyz + Assembly.Health() 集成

### [x] T-031: runtime/http/router [S] → T-010..T-016, T-030
- router.go + router_test.go
- chi-based + 自动挂载 /healthz /readyz /metrics

### [x] T-032: runtime/bootstrap [S] → T-020, T-021, T-022, T-027, T-031
...
### [x] T-033: Wave 2 Gate 验证 [S] → T-032, T-040..T-042
...
### [x] T-050: config-core Cell 实现（参考实现）[S] → T-033
...
### [x] T-051: access-core Cell 实现 [S] → T-033, T-050(模式参考)
...
### [x] T-052: audit-core Cell 实现 [S] → T-033, T-050(模式参考)
...
### [x] T-053: Wave 3 Gate 验证 [S] → T-050..T-052
...
### [x] T-060: cmd/core-bundle 更新 [S] → T-053
- 更新 main.go 使用 Bootstrap 编排
- 注册顺序: config-core → access-core → audit-core
- 验证: go build + 可启动

### [x] T-061: Journey Hard Gate 测试 [S] → T-060
- J-sso-login (密码登录路径)
- J-session-refresh
- J-session-logout
- J-user-onboarding
...
### [x] T-068: 覆盖率 Gate [S] → T-060
- runtime/ >= 80%
- cells/ >= 80%
- kernel/ >= 90% (维持)

### [x] T-069: Wave 4 最终 Gate [S] → T-061..T-068
- go build + go test + gocell validate
