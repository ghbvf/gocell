...
| C-17 | access-core 3 slice 不含 HS256 默认路径 | `grep -r "SigningMethodHS256" cells/access-core/` | 0 匹配（或仅在 test 文件/Deprecated path 中出现） |
...
| 编号 | 约束 | 验证方法 | 验收标准 |
|------|------|---------|---------|
| C-19 | kernel/ 代码零修改（或仅文档注释） | `git diff develop -- kernel/ \| grep "^[+-]" \| grep -v "^[+-]\s*//"` | 零行非注释修改 |
...
| 编号 | 约束 | 验证方法 | 验收标准 |
|------|------|---------|---------|
| C-23 | kernel/cell.Cell 接口签名无变更 | `git diff develop -- kernel/cell/interfaces.go` | 零 diff（或仅注释） |
| C-24 | kernel/outbox.Writer/Publisher/Subscriber 接口签名无变更 | `git diff develop -- kernel/outbox/outbox.go` | 零 diff（或仅注释） |
