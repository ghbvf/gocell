...
- **Affected file**: `cells/access-core/cell.go:169, 172`
- **Evidence**:
  ```go
...
- **Seat**: S2 Security
- **Affected file**: `cells/access-core/cell.go:159-184`
- **Evidence**:
  ```go
...
- **Seat**: S1 Architecture / S5 DX
- **Affected file**: `runtime/bootstrap/bootstrap.go:86`
- **Evidence**:
  ```go
...
- **Affected files**: `cells/order-cell/**`, `cells/device-cell/**`
- **Evidence**: `Glob("cells/order-cell/**/*_test.go")` and `Glob("cells/device-cell/**/*_test.go")` return no results.
...
- **Seat**: S6 Product/UX
- **Affected files**: `cells/order-cell/slices/order-query/handler.go:42-45`, `cells/device-cell/slices/device-command/handler.go:64-67`
- **Evidence**:
  ```go
...
- **Seat**: S6 Product/UX
- **Affected files**: `cells/order-cell/slices/order-query/service.go:30`, `cells/device-cell/slices/device-command/service.go:66`
...
- **Seat**: S4 Ops/Deploy
- **Affected files**: `examples/todo-order/docker-compose.yml`, `examples/sso-bff/docker-compose.yml`, `examples/iot-device/docker-compose.yml`
...
- **Seat**: S6 Product/UX
- **Affected file**: `examples/sso-bff/README.md`
...
- **Evidence**: `ConfigFromEnv()` in `adapters/s3/client.go:49` reads `GOCELL_S3_REGION`, and `Config.Validate()` requires region to be non-empty. But `.env.example` does not include `GOCELL_S3_REGION`. Users copying `.env.example` will get an S3 validation error.
...
- **Affected file**: `cells/order-cell/cell.go:87-89`, `cells/order-cell/slices/order-create/service.go:54-71`
...
- **Seat**: S5 DX
- **Affected files**: `examples/todo-order/main.go:52`, `examples/iot-device/main.go:48`, `examples/sso-bff/main.go:99`
...
- **Seat**: S1 Architecture
- **Affected files**: `cells/order-cell/slices/order-query/handler.go:6`, `cells/device-cell/slices/device-command/handler.go:7`, `cells/device-cell/slices/device-status/handler.go:6`
...
- **Seat**: S5 DX
- **Affected file**: `examples/sso-bff/main.go:30-34`
...
- **Seat**: S4 Ops/Deploy
- **Affected file**: `examples/todo-order/docker-compose.yml:1`
...
- **Seat**: S5 DX
- **Affected files**: `README.md:220`, `templates/`
- **Evidence**: README says:
  ```
  ├── templates/    — Project templates (ADR / cell-design / ...)
  ```
  suggesting templates are at the top-level `templates/` directory. But `Glob("templates/**/*")` at root returns nothing; the actual files are under `templates/`. The directory tree in README shows 项目根目录 as the root of the codebase structure which is correct, but the bullet "Project Templates" section at line 262 lists paths without the 项目根目录 prefix:
  ```
  - `templates/adr.md`
  ```
  This is fine since the working context is within 项目根目录, but could confuse users who are navigating from the repo root.
- **Suggestion**: Clarify in README that template paths are relative to 项目根目录 or use full paths from repo root.
...
- **Seat**: S1 Architecture
- **Evidence**: Contract files exist under `contracts/http/order/v1/contract.yaml`, `contracts/event/order-created/v1/contract.yaml`, `contracts/http/device/v1/contract.yaml`, etc. The slice YAML files reference these contracts in `contractUsages`. However, CI validation step uses `|| true` (P1-9), so these contract references are never actually validated in CI.
