...
- 6 个 adapter 全部有 `doc.go`，且内容充实 -- postgres doc.go 列出了子模块（Pool, TxManager, Migrator, RowScanner；QueryBuilder 已迁至 `pkg/pgquery.Builder`），rabbitmq doc.go 说明了实现的接口和核心特性。
