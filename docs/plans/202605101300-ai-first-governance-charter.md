...
| **PR-Φ-HARD-EACHNODE-WALKDEPTH-01**（PR-Φ 补遗）✅ 已 ship (PR-553) | 12-16h dev / 4-5h review | **已 ship (PR #553)**。`scanner.EachNode[N](root, fn)` 拆为 `EachInSubtree[N]` + `EachInChildren[N]`，删 `EachNode`；让 walk depth 在编译期成为 typed choice（违反不可表达 = Hard）。实际范围：297 调用站点 / 64 文件 audit + bulk-rename；高风险站点（CompositeLit/SwitchStmt/SelectStmt 内嵌 KeyValueExpr/CaseClause/CommClause 类）逐个语义审计。完成后删除 `eachnode.go`（符号彻底消除）。**Commit 链路**：44ac7cc4 (Stage 1) → f895d1ae (2B) → 360f3f90 (2C) → 4851e8a0 (2A) → a7e02327 (2D) → 2eb53e7f (3B) → bf9bbf86 (3A)。**paired-index 收编**：原 plan 估 8 处，实际扩展到 14 处（path B 守卫翻牌命中 PR #445 之外的隐性 Soft 残留：adapter_returns_declared_types / goose_session_locker / redis_key_namespace / setup_admin_bootstrap_closure 等）——符合 §6.2 一锅替换不 ratchet。**PR445-FU F1**（contract_spec_clients_test.go paired-index）+ **F3**（rmq_invariants_test.go SelectStmt/容器）一并收编关闭。 |
