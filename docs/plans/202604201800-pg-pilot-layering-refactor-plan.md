...
├── access_module.go                    ← accesscore 自管 wiring
├── audit_module.go
└── config_module.go
```
...
**修改**：
- `cmd/corebundle/bundle.go:509` — `crypto.NewVaultTransitKeyProviderFromEnv()` → `vault.NewTransitKeyProviderFromEnv()`
...
func TestTransitEnvelope_AADBindingAgainstRealVault(t *testing.T) {
    ...
    aad1 := []byte("cell:configcore/key:db_password")
    aad2 := []byte("cell:configcore/key:api_secret")
...
```go
// cmd/corebundle/config_module.go
...
```go
// cmd/corebundle/main.go (精简后 ~30 行)
func main() {
    ctx := context.Background()
...
```go
// cells/configcore/slices/flagwrite/service.go
...
- `cmd/corebundle/audit_module.go`

**修改**：
- `cmd/corebundle/main.go` — 精简至 module 拼接
- `cmd/corebundle/bundle.go` — 拆出 SharedDeps 后减半
- `cells/configcore/slices/flagwrite/service.go:67` — panic → return error
- `cells/configcore/config_core.go` — 调用 `NewService` 处处理 error

**删除**：
- `cmd/corebundle/bundle.go` 的 `AppDeps.Validate()` / `AppDepsFromEnv()` / `BuildBootstrap()` 合并到 SharedDeps 与 BuildApp

### 验收

- `go build ./cmd/corebundle/...` 通过
- `go test -tags=integration ./cmd/corebundle/...` 所有测试通过（测试调用 `BuildApp(shared, modules...)` 而非 `BuildBootstrap(deps)`）
- `AppDeps` 类型不再存在
...
| S2 | `GOCELL_MASTER_KEY` 未纳入 `rejectDemoKey` | `cmd/corebundle/config_module.go::buildKeyProvider` local-aes 分支追加 `rejectDemoKey(adapterMode, "GOCELL_MASTER_KEY", masterKeyBytes)` | R2 |
...
**修改**：
- `cmd/corebundle/config_module.go` — S2 rejectDemoKey
- `cmd/corebundle/shared_deps.go` — A3 KeyProvider 存在性强制
- `adapters/vault/transit_provider.go` — S4b real-mode guard + **A13 LifetimeWatcher + A14 AuthMethod 参数**
- `cells/configcore/internal/adapters/postgres/config_repo.go` — M3 stale slog.Warn + 指标
...
- `cmd/corebundle/security_integration_test.go` — table-driven 覆盖 S2/S4b/A13 renewal/A14 auth mode 拒绝路径
...
| ②每次 Vault 调用生成新 DEK，无缓存 | 高频场景会撑不住 | config 值低频，当前可接受。若未来 accesscore session 走此路径再评估 DEK cache |
...
敏感配置是否应存在 PG 里？替代方案：Vault KV secrets engine 直接作为敏感值存储，configcore 退化为薄 proxy，PG 只存非敏感配置。代价是强依赖 Vault 存储路径。当前 encrypt-in-PG 的结构性复杂度来自"用通用存储承载密钥管理职责"，不会因代码质量提升而消失。若 Vault 已是强依赖，可在 v2.0 评估此方向。
