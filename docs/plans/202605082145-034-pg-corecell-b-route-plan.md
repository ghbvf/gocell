...
**生成日期**: 2026-05-08
**最后更新**: 2026-05-12（v4 状态同步：S3+S5/S7/S6/S3F 全部 ship，PR#459 拉前 S4 部分 carry-over，S4 余下拆 S4.0/S4a/S4b/S4c 推进）
**对接来源**:
...
| **S4.0** | effective last-admin invariant (status=active∧admin 原子语义 + DB trigger 一致) | cell + adapter | S3+S5 | — | PR#449/#459 review last-admin 语义漏洞 |
| **S4a** | PG session/refresh durable wiring (删 mem session/refresh + composition-root 显式 protocol + forced re-login) | cell 接入 | S3F / S4.0 | — | S4-PG-SESSION-REFRESH-WIRING / PR338-FU / B5-FU |
| **S4b** | authz_epoch + credential event closed loop (JWT issue/verify epoch + sessionvalidate + lock/delete/password/revoke 撤权闭环) | cell + JWT | S4a | — | JWT-AUTHZEPOCH-CLOSED-LOOP / LOCKOUT-AUTO-LOCK / TOCTOU-10 |
| **S4c** | accesscore cleanup / race / L2 e2e (level audit / RBAC waiver / AUTH-CACHE / L2 e2e) | cleanup | S4b | — | LEVEL-MISLABEL / B2-T-02/07 / B2-C-06/13 / AUTH-CACHE |
...
**worktree 容量**：Wave 0 三个 + Wave 1 S1 = 4 worktree；Wave 2 已完成框架抽取（S2/S6/S7）与 PG adapter（S3+S5/S3F）。当前 wave 焦点：S4 拆 S4.0/S4a/S4b/S4c 串行收口 accesscore 余下 correctness 项；D4 docs sync 与 DX4 maintainability 可并行小 PR。

**关键路径（v4）**：S2/S3+S5/S3F/S6/S7 已 ship；当前关键路径 = **S4.0 (effective last-admin) → S4a (durable session/refresh wiring) → S4b (authz_epoch closed loop) → S4c (cleanup / L2 e2e)**。

**v3 简化**：v2 的 ADR-B（接口归属 6 条边界规则）已被 ADR-Typed 替代，关键路径从"两份 ADR"简化为"一份 S1"。

**v4 状态同步（2026-05-12）**：S3+S5 (PR#449) / S7 (PR#450) / S6 (PR#464) / S3F (PR#465) / S4 部分 carry-over (PR#459 LastAdminGuard + setup_pg_integration_test outbox event 断言) 全部已 ship；S4 单巨型 PR 改为 S4.0/S4a/S4b/S4c 四个 correctness-first PR，避免 PR #445 类多维捆绑教训。
...
### S4.0 effective last-admin invariant（PR #449/#459 review follow-up，S4 前置）
...
**PR449 follow-up 已拉前（不再作为 S4 carry-over）**：LastAdminGuard 基础 service wiring 已接入 `identitymanage.Delete` / `Lock`（`rbacassign` 既有 `RemoveFromUserIfNotLast` 继续负责 revoke），DB trigger 增加 transaction-scoped advisory lock 覆盖 direct SQL / cascade delete 并发；`cmd/corebundle/setup_pg_integration_test.go` 已改用 `adapterpg.NewOutboxWriter` 并断言已提交的 `event.user.created.v1` outbox row，另用 writer 失败注入验证 setup user/role/outbox 在真实 PG 事务内同步回滚。2026-05-11 review 新发现的 effective-admin 语义缺口由 S4.0 单独收口。
...
#### S4a PG session/refresh durable wiring
...
**范围**：
- JWT issue 写入 `jti` + `authz_epoch` claim；verifier 读取并暴露 typed claims
...
**验收**：
- role revoke 后旧 access JWT 立即失效
...
| S4.0 | 6h | 3h | effective last-admin invariant（PR#449/#459 review 漏洞）|
| S4a | 10h | 5h | PG session/refresh durable wiring + forced re-login |
| S4b | 8h | 4h | authz_epoch + credential event closed loop |
| S4c | 6h | 3h | cleanup / race / L2 e2e |
...
| **已 ship 累计** | **~116h** | **~59h** | S0/S1/S2/S3+S5/S3F/S6/S7/W9 |
| **剩余** | **~30h** | **~15h** | S4.0/S4a/S4b/S4c + B2.B + D4/DX4 并行小 PR |
...
**v4 关键路径**（2026-05-12）：S2/S6/S7/S3+S5/S3F 已 ship；当前关键路径 = **S4.0 (effective last-admin invariant) → S4a (PG session/refresh durable wiring) → S4b (authz_epoch closed loop) → S4c (cleanup / L2 e2e)**。每个 S4.x 都是 correctness-first，按 PR #445 教训不再单 PR 巨型化。

B2.B / D4 / DX4 / 路线外独立项可与 S4 序列并行。
