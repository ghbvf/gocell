...
| **T5** ✅ shipped (PR #524 + PR #533 第三轮 review 修复 PR) | AUTH-CACHE-01 Redis session cache 注入（默认关闭）+ 第三轮 review **PR #533** 闭环 8 IN_SCOPE finding（archtest CACHING-SESSION-REVOKE-DELEGATE-ONLY-01 Hard 锁 body 形态：单 ReturnStmt 转发同名 inner method + 4 RED fixture）+ 同 PR 登记 2 触发型 backlog（`AUTH-CACHE-AFTER-COMMIT-INVALIDATION-01` Spring afterCommit 范式 / `TYPED-NIL-ARCHTEST-ADAPTERS-EXTENSION-01`）；PR267-FU-AUTHTEST-INTERNAL（需先 ADR/dry-run，建议剥离独立 PR） | `adapters/redis`、`cmd/corebundle`；`runtime/auth/authtest`→internal | ✅ 并行 | ✅ |
...
- AUTH-CACHE-01：Redis session cache adapter 注入 ✅ closed by S4c-T5 (PR #524 + PR #533 第三轮 review 修复)
