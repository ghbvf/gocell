...
**生成日期**: 2026-05-08
**最后更新**: 2026-05-13（v5 状态同步：**S4a** ship by PR #482 — PG session/refresh durable wiring + refresh stable-sid + sessionlogout 503/404 区分；剩余 S4b/S4c 继续串行；PR #482 review 暴露的三件未做 finding（rbacassign `syncSessionRevocation` 二态 / sessionvalidate 路径 store-infra 仍全 401 / contract 403 漂移 + archtest 升级）直接 in-scope 折进 S4b/S4c/D4 同 PR 处理，**不开新 backlog 条目**）
**对接来源**:
...
| **S4b** | authz_epoch + credential event closed loop (JWT issue/verify epoch + sessionvalidate + lock/delete/password/revoke 撤权闭环) **+ S4a 遗留 P1**：删除 rbacassign `syncSessionRevocation` 二态恢复 same-tx revoke（ADR D5 合规）；sessionvalidate 路径 store-infra error 改 `KindUnavailable` → 503（与 PR #482 sessionlogout 同步） | cell + JWT | S4a ✅ | — | JWT-AUTHZEPOCH-CLOSED-LOOP / LOCKOUT-AUTO-LOCK / TOCTOU-10 |
| **S4c** | accesscore cleanup / race / L2 e2e (level audit / RBAC waiver / AUTH-CACHE / L2 e2e) **+ S4a 遗留 P2**：`session_protocol_composition_root_test.go` / `refresh_invariants_test.go` Soft → Medium (type-aware) 升级 | cleanup | S4b | — | LEVEL-MISLABEL / B2-T-02/07 / B2-C-06/13 / AUTH-CACHE |
...
**worktree 容量**：Wave 0 三个 + Wave 1 S1 = 4 worktree；Wave 2 已完成框架抽取（S2/S6/S7）、PG adapter（S3+S5/S3F）、S4.0 effective last-admin invariant、S4a PG session/refresh durable wiring。当前 wave 焦点：S4b/S4c 串行收口 accesscore 余下 correctness 项（含 S4a 遗留 P1/P2 finding，**不开新 backlog 条目**）；D4 docs sync 与 DX4 maintainability 可并行小 PR。

**关键路径（v5）**：S2/S3+S5/S3F/S6/S7/S4.0/**S4a** 已 ship；当前关键路径 = **S4b (authz_epoch closed loop + rbacassign same-tx revoke 回退 + sessionvalidate 503 区分) → S4c (cleanup / L2 e2e + archtest Soft → Medium 升级)**。
...
**范围**：
- JWT issue 写入 `jti` + `authz_epoch` claim；verifier 读取并暴露 typed claims；删除 `Session.AuthzEpochAtIssue: 0` 硬编 placeholder
- `sessionvalidate` 查 `users.authz_epoch` 与 `sessions.authz_epoch_at_issue`，旧 epoch access JWT fail-closed
- lock / delete / change-password / role-revoke / refresh-reuse 统一 bump epoch + revoke session/refresh
- refresh token reuse 检测命中时，同时撤 refresh 链与 access session，外部错误仍保持统一认证失败
- credential event 的 outbox/audit 可作为 fanout/audit，不作为本地鉴权撤权的唯一机制
- **S4a 遗留 FU-1**：rbacassign 删 `syncSessionRevocation bool` 二态字段（critical-path credential invalidation 不变量不应是 wiring 可配项）；durable mode 恢复 same-tx `sessionStore.RevokeForSubject + refreshStore.RevokeUser`，与 identitymanage ChangePassword 对齐；`event.role.{assigned,revoked}.v1` consumer 降格为 fanout/audit（不作为 primary credential invalidation 路径）
- **S4a 遗留 FU-2**：sessionvalidate `enforceSessionState` 把 `sessionStore.Get` 的 infra error 改 `KindUnavailable` + 新错误码 `ErrAuthServiceUnavailable`；`runtime/auth/middleware.go` AuthMiddleware 按 Kind 分流 401 / 503，防枚举仍统一文案（redaction 在 wire 层做）

**收口项**：
- `JWT-AUTHZEPOCH-CLOSED-LOOP-S4-01`
- `ACCESSCORE-ACCOUNT-LOCKOUT-AUTO-LOCK-01`
- `P3-TD-10 TOCTOU 竞态修复`
- `B2-C-06 SessionLogout consumer action 无验证`（若改动触达 session revoke consumer）
- S4a FU-1 (rbacassign same-tx revoke 恢复) / FU-2 (sessionvalidate 503 区分) — **不开 backlog 条目**，PR review findings 默认 in-scope
...
- refresh reuse/宽限失败后 access session 被撤销
- 并发 credential event 不产生 epoch 回退或 session 残留
- **FU-2 验收**：PG session store 注入故障（pool 断开 / query timeout）→ sessionvalidate 返回 503 `ErrAuthServiceUnavailable`，AuthMiddleware 不再统一 401；防枚举文案保持 `invalid or expired authentication token`
...
**收口 backlog**（原有）：
- ACCESSCORE-ACCOUNT-LOCKOUT-AUTO-LOCK-01 🔴 P1（session 状态机一并）— S4b
...
- P3-TD-10 TOCTOU 竞态修复 🟠 P2（session 状态机协议吸收）— S4b
...
- B2-C-06 SessionLogout consumer action 无验证 🟡 P1 — S4b（若触达 session revoke consumer）
...
| S4b | 10h | 5h | authz_epoch + credential event closed loop **+ S4a 遗留 FU-1/FU-2**（rbacassign same-tx revoke 恢复 / sessionvalidate 503 区分） |
...
| **已 ship 累计** | **~134h** | **~68h** | S0/S1/S2/S3+S5/S3F/S4.0/**S4a**/S6/S7/W9 |
| **剩余** | **~21h** | **~10h** | S4b/S4c + B2.B + D4/DX4 并行小 PR |
...
A 路线工时（033 残）47-49h dev / B 路线 v4 ~146h dev —— **约 3 倍**，但收口项从 5 项扩到 ~50 项（C/D/E/F + 033 残），且把"在 cell 内重做框架"问题一次解决，并把 PR#417 5 个 P0/P1 协议缺口压到启动期 fail-fast。

---

## 7. 决策点

**v5 关键路径**（2026-05-13）：S2/S6/S7/S3+S5/S3F/S4.0/**S4a** 已 ship（PR#482）；当前关键路径 = **S4b (authz_epoch closed loop + S4a 遗留 FU-1/FU-2) → S4c (cleanup / L2 e2e + S4a 遗留 FU-3b)**。每个 S4.x 都是 correctness-first，按 PR #445 教训不再单 PR 巨型化；PR #482 review findings 按 in-scope 原则折进下游 PR，不开新 backlog 条目。

B2.B / D4 / DX4 / 路线外独立项可与 S4b/S4c 序列并行；D4 同 PR 吸收 S4a 遗留 FU-3a (login/refresh contract 403 漂移)。
