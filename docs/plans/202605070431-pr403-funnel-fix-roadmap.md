...
- **调用侧 1 条 archtest 保留**：HANDLER-POLICY-REQUIRED-01 扫 `cells/.../cell.go` 的 `<pkg>.NewHandler(svc, nil)` 调用，是 caller-side wiring mistake，handler.tmpl funnel 触不到。保留为 `tools/archtest/handler_policy_required_test.go`（独立文件，~120 行）。
