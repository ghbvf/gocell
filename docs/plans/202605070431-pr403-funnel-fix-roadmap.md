...
#### 步骤 C：funnel 化 PR 系列（按类别分批，登记于本路线图，不入 backlog）

本路线图 §C 是 PR-FUNNEL-NN 的唯一 todo 登记位置。`docs/audit/archtest-inventory.md` §不在本 PR 范围 引用本节编号，但不重复内容。
...
##### PR-FUNNEL-02（待办，~16-24h dev + 4-8h review）

**HTTP-RESPONSE-CONSTRAINTS-CODEGEN-MIGRATION-01** (Cx3, 🟡 可延后)：把 `HANDLER-NO-INLINE-LIMIT-PARSE-01` / `HANDLER-NO-SCHEMA-FOR-NOBODY-01` / `HANDLER-PATH-QUERY-LENGTH-VALIDATION-01` / `HANDLER-VALIDATOR-FAIL-FAST-01` 等 ~5 条 archtest 迁移到 contract.yaml schema 字段 + `handler.tmpl` codegen 拦截（type system 化）。

**修复方向**：(1) `kernel/metadata/parser.go` 扩展 schema 字段（如 `pageSize.maxValue`、`bodyless: true`、`pathParams[].maxLength`）；(2) `tools/codegen/contractgen/{builder.go,generator.go}` + `handler.tmpl` 派生对应 validator 调用；(3) `contracts/http/**/contract.yaml`（45 个）补声明；(4) 删 `tools/archtest/handler_invariants_test.go` 5 条对应 Test 函数。

**关键文件**：`tools/codegen/contractgen/{builder.go,generator.go,handler.tmpl}` + `kernel/metadata/parser.go` + `contracts/http/**/contract.yaml`（45 个）+ `tools/archtest/handler_invariants_test.go`（删 5 个 Test）。

**性质**：改 codegen 工具本身（schema 字段扩展 + 模板修改），与 PR-FUNNEL-01 的"机械文件聚并"性质完全不同，独立 PR。
