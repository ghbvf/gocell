...
#### 步骤 C：funnel 化 PR 系列（按类别分批）

每个 PR 处理一类（编号与 `docs/backlog.md` P2 待办 + `docs/audit/archtest-inventory.md` §不在本 PR 范围 对齐）：

- **PR-FUNNEL-01** (本 PR #408): 同主题聚并 + INVARIANT 锚点 + funnel-first CLAUDE.md 原则 + 路线图/inventory 落地（45 文件 → 11 主题 + 11 锚点修补，已 ship）
- **PR-FUNNEL-02**（backlog 已登记，~16-24h）: codegen 化批次（HANDLER-* / CH-04..06 — 把约束从 archtest 迁到 contract.yaml schema 字段 + codegen 拦截）
- **PR-FUNNEL-03**（backlog 已登记，~12-16h）: governance rules_*.go 60+ 条聚并（15 文件 → ~6 主题文件，机械搬迁）
- **PR-FUNNEL-04**（按需触发）: 冗余 / type system 化（改接口签名让违反不可表达；触发条件：codegen 化覆盖外仍发现 ≥ 3 条可消除 archtest）
