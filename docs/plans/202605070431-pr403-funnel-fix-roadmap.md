...
#### 步骤 C：funnel 化 PR 系列（2-4 天，按类别分批）

每个 PR 处理一类：

- **PR-FUNNEL-01**: codegen 化批次（把 N 条 archtest 替换为 contract.yaml schema 约束 + codegen 拦截）
- **PR-FUNNEL-02**: type system 化批次（改接口签名，删原 archtest）
- **PR-FUNNEL-03**: 冗余删除批次
- **PR-FUNNEL-04**: 保留 archtest 加 INVARIANT 锚点注释
