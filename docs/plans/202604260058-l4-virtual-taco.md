...
7. 4 PR 全 merge 后启 batch2 终点 retrospective 6 角色（累计 diff）✅ **2026-05-01 完成**：见 [`docs/reviews/202605011700-batch2-end-retrospective-six-seat.md`](../reviews/202605011700-batch2-end-retrospective-six-seat.md)（6 席位独立报告 `docs/reviews/batch2-retro/{architect,kernel-guardian,reviewer,devops,doc-engineer,product-manager}.md`）
8. 完成后人工确认：
   - 025 plan Wave 2.5 板块清零（CFG-1 ✅ / CFG-4 → L ✅ / CFG-5 won't-do / CFG-6 → M ✅）— ✅ doc-engineer 席位证实
   - backlog 仅剩 2 条 YAGNI 项（KG-F7 / RA-F5）— ⚠️ **retrospective 新增 5 条 P1 + 8 条 P2**，需先打包 `PR-BATCH2-RETRO-FU` 关闭后才能回到「仅 2 条 YAGNI」状态
   - 025 plan 此后只剩 Wave 3（A15/A16/A17/A36/A37/A38）+ Wave 4（A21/A22/A24/A33）新模块路线图 — 待人工确认

## Retrospective 关键 P1 follow-up（建议 1 PR 打包，~6-8h）

| ID | 主题 | 文件域 |
|---|---|---|
| BR-P1-01 | `runtime/auth/servicetoken.go::errorMiddlewareInternal` `cfg.metrics` nil panic | `runtime/auth/servicetoken.go` |
| BR-P1-02 | `cells/accesscore/.../identitymanage::ChangePassword` `GetByID` 在 tx 外 TOCTOU | `cells/accesscore/slices/identitymanage/service.go` |
| BR-P1-03 | `tools/archtest/*` walker 未 SkipDir `worktrees/`，被 sibling worktree fixture 污染 | `tools/archtest/helpers_test.go` 或同源 walker |
| BR-P1-04 | `TestListenerDXA52Guard` 把 `bak/docs/`、`docs/bak/` 当 active docs，13 处误判 | `tools/archtest/listener_dx_test.go` |
| BR-P1-05 | `runtime/bootstrap/doc.go:10-11` 示例 `PrimaryListener` 用 `cell.AuthNone{}` 误导消费者 | `runtime/bootstrap/doc.go` |

外加 BR-P2-01..08 共 8 条 P2 漂移项（详见 retrospective 文档）；BR-PLAN-01 plan 文本回写（A66 实际 flat + comment groups / M.6 实际 `verify generated` 不是 `--boundary-only` flag）。
