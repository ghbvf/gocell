...
| G-17 | ✅ 触发条件已达（PR-A22 上 029 #13）CELL-RAW-DEPS-ARCHTEST-EXPAND | backlog `CELL-RAW-DEPS-ARCHTEST-EXPAND-01` (PR245-F10，PR-A22 触发) | PR-A5c `OUTBOX-CELL-01` 只 ban `WithPublisher` / `WithOutboxWriter` raw Option；`AccessCore.txRunner` 等 raw dep 仍保留可被新 PR 重新打开口子 | **2026-05-08 触发就绪**：PR-A22 已上 029 关键路径 #13 PR-V1-CELL-IFACE-ISP-SPLIT，G-17 archtest 扩展（`CELL-RAW-DEPS-01` 禁所有 raw-dep Option）已吸收进 #13 同 PR 落地（2h+1h 工时已计入 #13 的 16h+7h 总额），G-17 单 PR 不再独立排期。 | L1 | 2h + 1h（已并入 029 #13）| G-10 |
