...
## Phase K: kernel 层偿债（~13h，3 个 PR 并行）

> kernel 是底座灵魂。依赖方向最底，先修这里上层才能跟进。

### PR-K-VALIDATOR: validator 机器输出 + 定位 API 收敛（9h）
...
### PR-K-METAPERF: metadata parser 性能基准（4h）
...
### PR-K-OBS-CONTRACT: outbox metric 注册契约（2h）
...
### PR-R-BOOT-COGNIT: bootstrap 复杂度拆分（4h）
...
### PR-R-OBS-AUTOWIRE: 默认 HTTP collector 自动接线（3h）
...
### PR-R-ROUTER-METHOD: 公共端点 method-aware 匹配（4h）
...
### PR-R-INTERNAL-LISTENER: internal 信任边界（4-8h，可选升级为 Phase X）
...
### PR-R-AUTH-STRICT: legacy token strict 模式（搭车，1h，产品确认后）
...
| **PR-X3 cursor key rotation 接线**: `NewCursorCodec(current, previous)` 启动接线 + `GOCELL_CURSOR_PREVIOUS_SIGNING_KEY` 双 env 通道 + 轮换兼容回归。对标 K8s `--service-account-key-file`、gorilla/securecookie `CodecsFromPairs` | 4h | `pkg/query/codec.go` + `cmd/core-bundle/main.go` | PR#160 六席位审查 |
...
### PR-A-HARDEN: 生产安全 + PoolStats + broker health（8.5h）
...
| **#31**: RabbitMQ backoff + FailOpen enum 清理 | 2h | `adapters/rabbitmq/` | Wave 2 残留 |
| **POOLSTATS-IFACE-01**: 三个 adapter PoolStats 公共接口（OTel collector 消费） | 1h | `adapters/postgres/pool.go` + `redis/client.go` + `rabbitmq/connection.go` | PR#134 review |
| **POOLSTATS-JSON-01**: PoolStats camelCase json tags | 0.5h | 同上 | PR#134 review |

### PR-A-CI-OTEL: CI 供应链 + OTel 夜间兼容性（4h）
...
| **REPO-SCAN-CLASSIFY-01** (P2, Cx2): `config_repo.go::GetByKey` / `GetVersion` 把所有 Scan 错误映射为 `ErrConfigRepoNotFound`；改用 `errors.Is(err, sql.ErrNoRows)` 判 not found，其他返回 `ErrInternal` 保留 `InternalMessage` | 2h | `cells/config-core/internal/adapters/postgres/config_repo.go` | PR#157 post-merge review |
| **CONTRACT-ERROR-SCHEMA-01** (P2, Cx1): `publish/v1` + `rollback/v1` contract.yaml `responses` 新增 401/403 entries 引用共享错误 schema | 1h | `contracts/http/config/{publish,rollback}/v1/contract.yaml` | PR#157 post-merge review |
...
| 任务 | 工时 | 涉及文件 | 来源 |
|------|------|----------|------|
| **RBAC-REVOKE-POST-01**: `DELETE /internal/v1/access/roles/revoke` 改为 `POST` 避免 DELETE body 代理兼容问题 | 1h | `cells/access-core/slices/rbacassign/handler.go` + `contracts/http/auth/role/revoke/v1/contract.yaml` | PR#143 review 6.2 |
...
| **VALIDATE-EVIDENCE-CI-01** (P2, Cx2): CI 新增独立 `metadata-check` job（`gocell validate` + `check contract-health`），失败阻断 PR；PR template 增"metadata gate"勾选项 | 1h | `.github/workflows/ci.yml` + PR template | PR#155 review F7 |
...
| **SYSTEM-TOPOLOGY-API**: `GET /internal/v1/system/topology` 返回 cell/slice/contract 拓扑 JSON；基于 `kernel/registry` 现有数据构建 | 4h | 新 slice 或 `runtime/bootstrap/` | 历史 Batch 8 |
