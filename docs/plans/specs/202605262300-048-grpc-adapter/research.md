...
| 9 | contract.yaml + codegen | `contracts/http/*/v1/contract.yaml` `kind: http`; `endpoints.http.method/path`; contractgen `ContractGenSpec.Kind ∈ {http, event}` | `kind: grpc`; new fields `grpc.{service, proto}` (service-level per ADR D5 / #1655; `method`/`streamingType` deleted — method set derives from the `.proto` via `contractgen.ReadProtoServiceInfo`); codegen generates multi-method server iface + client invoker + proto-to-DTO mapper | yes (large codegen extension) | yes (`GRPC-CONTRACT-CODEGEN-COMPLETE-01`) |
...
| 12 | archtest | 628+ tests, 16-shard | minimum 6 new invariants (#1/#2/#3/#4/#5/#9 above) + `GRPC-HANDLER-NO-DIRECT-DB-01` + `PROTO-GEN-NO-HANDWRITE-01` + `GRPC-SERVICE-IN-CONTRACT-01` | — | yes (6–8 new archtests) |
