...
- **HTTP/2 with TLS is the standard production deployment**; plaintext mode is gated behind an explicit `AllowInsecure` opt-in (fail-closed: the zero-value config rejects both plaintext and TLS-less operation). The listen address is NOT restricted to loopback when plaintext is enabled — a service-mesh sidecar running plaintext on a non-loopback address with mTLS terminated at the pod boundary is a legitimate production posture. The only fail-closed guarantee is the explicit opt-in switch.
