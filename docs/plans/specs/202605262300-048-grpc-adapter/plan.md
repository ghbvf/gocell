...
> 5. **Adapter-owned chain builder**: `adaptersgrpc.Config` now carries one `Interceptors interceptor.Deps`; `adaptersgrpc.New` always derives both `NewUnaryChain` and `NewStreamChain` from it and binds the same Registrar/Drain instances to the adapter. This closes the "serve grpc but forget `NewStreamChain` → unauthenticated streams" gap for this adapter. The broader #1752 follow-up remains for any future naming/packaging refinement of this wiring surface.
