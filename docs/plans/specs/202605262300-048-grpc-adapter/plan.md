...
> **As-built (PR-10 #1153)** — three refinements to the above:
> 1. **Stream chain is at full parity with unary, incl. auth** (8 interceptors: RequestID→CellAttribution→Tracing→AccessLog→Metrics→Auth→Drain→Recovery). Shipping streaming without `StreamAuth` would expose unauthenticated streams. The per-concern cores are shared between unary and stream variants (one source per concern); the metrics cell-label funnel is the sole exception — inlined per `GRPC-METRICS-LABEL-CELLID-CTXSOURCE-01`'s per-function provenance binding.
> 2. **Framework-side drain (genuine Medium, not Soft).** The original `GRPC-STREAM-DRAIN-01` ("AST scan that a handler respects `ctx.Done()`") is actually Soft (a dead-branch `ctx.Done()` reference satisfies it; charter forbids Soft). Instead the framework *delivers* drain cancellation: a shared `runtime/grpc.DrainSignal` lives in the `runtime/grpc.ServerInterceptors` bundle returned by `interceptor.NewServerInterceptors(deps)`; the adapter consumes that bundle and triggers the same signal at `gracefulStop`. `GRPC-STREAM-DRAIN-01` is now a two-sided Medium guard: (A) `StreamDrain` pinned in the chain + (B) `DrainSignal.Trigger` caller-allowlisted to `adapters/grpc/server.go`. The bufconn integration test proves an in-flight stream is actually cancelled.
> 3. **Retired `GRPC-RECOVERY-PANIC-APPROVED-01` extension** — `StreamRecovery` collapses panic→`codes.Internal` via the shared `recoverGRPCPanic` and never re-panics, so it is outside `PANIC-REGISTERED-01` (same as the unary path); its redacted panic log is auto-covered by `PANIC-LOG-REDACT-01`.
> 4. **Codegen streaming-lift** (`contractgen.ReadProtoServiceInfo` / `parseAllRPCMethods`): the unary-only rejection is removed — service-level granularity registers the whole proto service regardless of per-method stream shape; no `ProtoMethodInfo` stream-flag fields added (no consumer reads `.Methods` post-#1688). iotdevice `WatchCommands` (server-stream) is the first cell consumer.
> 5. **Single wiring funnel**: `adaptersgrpc.Config` carries one `Interceptors runtimegrpc.ServerInterceptors` bundle; the composition root builds it with `interceptor.NewServerInterceptors(deps)`, which (post-#1752) MINTS the shared registrar/drain internally and composes both chains from them, so the adapter and chains can never observe two different instances. This closes the "serve grpc but forget the stream chain → unauthenticated streams" gap AND the "wire two different registrars → silent `_runtime` attribution" gap. #1752 landed the compile-proof same-instance funnel: the chain builders are package-private and `GRPC-WIRING-REGISTRAR-MINT-FUNNEL-01` + `GRPC-WIRING-BUNDLE-CALLER-01` pin minting/assembly to the funnel.
> 6. **File organization**: `WatchCommands` lives in `devicecommandrpc/handler.go` next to `IssueCommand` (not a separate `watch_handler.go` as the file list sketched) — one cell server struct, less handler-layer fragmentation. Stream interceptors are consolidated in `runtime/grpc/interceptor/stream.go` (+ `wrapped_stream.go`) rather than per-concern `stream_*.go` files, since the cores are shared with the unary interceptors.
> 7. **Streaming-mode example scope**: only a **server-stream** consumer (`WatchCommands`) is added as the cell example. client-stream + bidi need no example to be validated — `grpc.StreamServerInterceptor` is direction-agnostic (one chain serves all three), so the bufconn integration test (`server_integration_test.go`) exercises all three modes through the full chain with a synthetic streamer; a per-direction cell handler would be DX gilding, not framework coverage.
