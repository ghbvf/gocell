...
> 2. **Framework-side drain (genuine Medium, not Soft).** The original `GRPC-STREAM-DRAIN-01` ("AST scan that a handler respects `ctx.Done()`") is actually Soft (a dead-branch `ctx.Done()` reference satisfies it; charter forbids Soft). Instead the framework *delivers* drain cancellation: a shared `runtime/grpc.DrainSignal` lives in `adaptersgrpc.Config.Interceptors.Drain`; the adapter derives `StreamDrain` from that deps object and triggers the same signal at `gracefulStop`. `GRPC-STREAM-DRAIN-01` is now a two-sided Medium guard: (A) `StreamDrain` pinned in the chain + (B) `DrainSignal.Trigger` caller-allowlisted to `adapters/grpc/server.go`. The bufconn integration test proves an in-flight stream is actually cancelled.
