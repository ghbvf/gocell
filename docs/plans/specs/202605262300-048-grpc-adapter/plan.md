...
> 2. **Framework-side drain (genuine Medium, not Soft).** The original `GRPC-STREAM-DRAIN-01` ("AST scan that a handler respects `ctx.Done()`") is actually Soft (a dead-branch `ctx.Done()` reference satisfies it; charter forbids Soft). Instead the framework *delivers* drain cancellation: a shared `runtime/grpc.DrainSignal` is wired into both `adaptersgrpc.Config.Drain` (the adapter triggers it at `gracefulStop`) and `interceptor.Deps.Drain` (the `StreamDrain` interceptor binds each in-flight stream's ctx to it). `GRPC-STREAM-DRAIN-01` is now a two-sided Medium guard: (A) `StreamDrain` pinned in the chain + (B) `DrainSignal.Trigger` caller-allowlisted to `adapters/grpc/server.go`. The bufconn integration test proves an in-flight stream is actually cancelled.
...
> 5. **The single-builder Hard upgrade** that would also force both chains + Registrar + Drain to be co-wired (closing the "serve grpc but forget `NewStreamChain` → unauthenticated streams" gap at compile time) stays tracked as **#1752** (cross-cutting; touches unary wiring too).
