...
**AC-12 wall-time 预算（#1430 对账）**：MQTT 集成套件主导项 `TestIntegration_PublisherTrueReconnect`（broker-restart 重连，~60s）；其余 publisher 场景各 <10s。整套并入 adapters shard（`-timeout 15m`、按包并行），不延伸 shard 关键路径出该信封。原 spec「pr-check 增量 ≤60s」作为**孤立独立计量目标不可达**——单个 broker-restart 重连测试本身即 ~60s——故 #1430 将其重构为**机器门控**：integration-test job 现经 `slowgate --threshold=120s`（integration-tier，**保守初值**：~2× 已知最慢集成测试 + GHA 容器 start/stop 开销 headroom，dev 机无 Docker 无法实测，刻意偏向首跑不误红——后续按真实 CI 时长收紧）门控每个 (Package, Test) 的 wall-time，runaway（如重连测试劣化到 120s+）触 CI 红；超阈测试须取 `tools/slowgate/allowlist.txt` justified 条目（`SLOWGATE-ALLOWLIST-01` 守，其 `packages.Load` 用 `FlatNonDefaultTags`，integration-tagged 条目可解析，无需改 archtest）。决策：保留并入、不新建 job、不回归静态 service container（与 `CI-INTEGRATION-DISCOVERY-01` 一致）。精确增量数 + 阈值收紧依据随真实 CI 运行回填（dev 机时长不代表 GHA 2-CPU runner，CI job timing 为权威源）。
