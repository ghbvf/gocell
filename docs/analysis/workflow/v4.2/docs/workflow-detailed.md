...
**操作步骤**:
1. 创建 feature branch: `bash .claude/skills/phase-gate/scripts/bash/create-new-feature.sh "描述" --json --short-name "名称"`
2. 切换到 feature branch: `git checkout {branch}`
...
**产出物**: `specs/{branch}/review-architect.md` + `specs/{branch}/review-roadmap.md` + `specs/{branch}/kernel-constraints.md` + 产品经理审查意见
...
6. **前置条件: Harness 审查（步骤 5）已完成。** 完成后才派发以下 2 个 agent:
   ```
...
**Playwright 配置要求**: QA 自动化编写测试时，playwright.config.ts 必须包含:
```typescript
use: {
  trace: 'on',
...
5.2 总负责人: 审批 batch 方案，派发 Agent(run_in_background=true) x N
    ...
    **开发者 Agent 要求**: 完成任务后自行运行 go build + go test 并在返回结果中报告 PASS/FAIL
...
6.1 派发 6 个命名席位 review Agent(subagent_type=Explore):
    ...
    **Reviewer prompt 强制注入上下文**:
    - `specs/{branch}/kernel-constraints.md`（Harness 约束基准）
    - `git diff main...{branch} --stat`（变更范围概览）
...
7.1 QA Agent: 执行测试（测试脚本已在 S5 编写完成）
    - 运行: npx playwright test（环境已由 DevOps 在 7.0 部署就绪）
    - 收集结果
    - 补充回归脚本（如发现 S5 遗漏的场景）
...
8.0 入口检查（硬阻塞门）:
    ...
    [ ] specs/{branch}/user-signoff.md 存在(仅当 role-roster.md 前端开发者=ON) → 否则回到阶段 7
...
8.2 总负责人派发收尾任务（并行，全部完成后进入 8.3）:
    ...
    - 派发 Kernel Guardian Phase 回顾（与文档工程师并行）:
      输入: kernel-constraints.md + tasks.md + tech-debt.md + qa-report.md + git log
      检查 7 个维度（绿/黄/红）:
        A. 工作流完整性 — 8 阶段是否全执行
    ...
    - 派发产品经理产品评审（与文档工程师、Kernel Guardian并行）:
      输入: product-context.md + product-acceptance-criteria.md + qa-report.md + tech-debt.md + **user-signoff.md（三视角评分）**
      检查 7 个维度（绿/黄/红）:
        A. 验收标准覆盖率 — 分级通过标准:
           P1 AC（核心功能）= 100% PASS
...
| S7 验证 | 使用者 | `user-signoff.md`（三视角评分 + APPROVE/CONDITIONAL/REJECT） |
| S8 PR | 总负责人 | PR 创建 |
...
13. **80% AC 通过率可能放过核心功能 FAIL** → 5 条 AC 中 1 条核心 FAIL 但仍达 80%
    - 修正: AC 分级标准（P1=100% PASS, P2 允许 SKIP, P3 允许 SKIP）

14. **product-context.md 缺少输入来源** → Agent 凭空编造 persona 和成功标准
    - 修正: S0 产品经理 prompt 必须注入 Phase 目标 + PRD + 上一 Phase product-review-report.md

15. **角色清单混合三种不同性质** → 检查产生假红灯或假绿灯
    - 修正: 三层分组（Core Mandatory / Conditional Delivery / Review Bench）

16. **6 视角 Reviewer 是黑箱角色** → 无法确认安全视角存在、谁缺席了
    - 修正: 6 个命名席位显式列出

17. **总负责人过载为单点瓶颈** → 所有门禁执行权集中于一人
    - v4.1 修正: 新增副总负责人分担
    - v4.2 修正: 去除副总负责人，改用 `phase-gate-check.sh` 自动化替代人工门禁监督

18. **QA 编写与执行阶段冲突** → S7 退化为"临时补环境+临时写测试"
    - 修正: S5 编写测试脚本，S7 只执行+报告+补充回归

19. **角色有动作无交接件** → 流程有门无产物，无法追溯
    - 修正: 每个角色每个阶段有明确产出文件，见文档职责矩阵

20. **副总负责人角色价值低于自动化** → 人工复核不如脚本可靠且无法扩展
    - v4.2 修正: 去除副总负责人，所有门禁检查由 phase-gate-check.sh 自动执行

21. **S6 Review 缺少上下文** → Reviewer 无法对照需求和变更范围审查
    - v4.2 修正: Reviewer prompt 强制注入 git diff stat + spec + kernel-constraints + commit hash

22. **S7 使用者验证无结构** → 验证结论主观且不可复现
    - v4.2 修正: 三视角结构化操作（PM/开发者/Vibe Coder）+ 评分量表

23. **Playwright 证据缺失** → QA 结论无法追溯验证
    - v4.2 修正: 强制 trace/video 配置 + evidence/ 目录 + qa-report 引用证据路径

24. **API 实现先于契约定义** → 测试基于实现而非规格，遗漏偏差
    - v4.2 修正: S5 含 API 变更的 batch 强制 contract → test → implement 顺序

25. **CHANGELOG 手动编写效率低** → 文档工程师重复 git log 工作
    - v4.2 修正: 文档工程师先 `git log --oneline main..{branch}` 生成初稿
...
| v4.1 → v4.2 | S6 Review 上下文注入强化（git diff + spec + commit hash） | workflow-detailed-v4.2.md |
| v4.1 → v4.2 | S7.2 三视角使用者验证（PM/开发者/Vibe Coder） | workflow-detailed-v4.2.md |
