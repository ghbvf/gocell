---
name: stage-3-decide
description: "综合裁决: 指令重注入+逐条裁决4方建议+speckit.clarify更新spec+写decisions.md"
...
**入口条件**: S2 出口通过（kernel-constraints.md + review-architect.md + review-roadmap.md 存在）
...
```bash
bash .claude/skills/phase-gate/scripts/bash/phase-gate-check.sh --stage S3 --check exit
```
...
[ ] decisions.md 含 Harness 约束裁决表（每条均标注 accept/reject/defer）
[ ] decisions.md 含延迟项列表
[ ] phase-gate-check.sh --stage S3 --check exit = PASS
```
