...
description: "Speckit Specify: 创建 feature branch + 生成 spec.md + 验证 FR 含文档/DevOps/测试需求"
...
### 步骤 1: 创建 Feature Branch

```bash
bash .claude/skills/phase-gate/scripts/bash/create-new-feature.sh "描述" --json --short-name "{名称}"
```

**重要**:
- 不传 `--number`，脚本自动检测下一个全局可用编号
- 必须包含 `--json` 标志以便解析输出
- 只运行一次，不重复创建

### 步骤 2: 切换到 Feature Branch

```bash
git checkout {branch}
```

### 步骤 3: 执行 Speckit Specify
...
```bash
bash .claude/skills/phase-gate/scripts/bash/phase-gate-check.sh --stage S1 --check exit
```
...
[ ] phase-gate-check.sh --stage S1 --check exit = PASS
```
