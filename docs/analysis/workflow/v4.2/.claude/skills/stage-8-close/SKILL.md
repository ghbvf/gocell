---
name: stage-8-close
description: "PR+收尾+并行双确认+合并: 入口检查+四路并行+Harness7维+产品7维+双PASS合并"
...
总负责人派发以下 4 路任务**并行执行**，全部完成后进入 8.3。
...
B. UI 合规检查 — 空状态处理？错误提示？Loading 状态？导航可达？
   引用 user-signoff.md 三视角评分中的具体发现
...
[ ] 项目 PASS
[ ] PR 已合并
[ ] phase-gate-check.sh --stage S8 --check exit = PASS
```
