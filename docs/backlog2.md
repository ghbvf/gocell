...
| **RMQ-FATAL-SHUTDOWN-SIGNAL** | RMQ Terminal 主动信号 bootstrap shutdown（D 轨道）| P2 | `adapters/rabbitmq/connection.go` + `runtime/bootstrap/phases_shutdown.go` | PR-V1-RMQ-TERMINAL（029 A4）已让 runtime permanent error 转 StateTerminal + `/readyz` 503，但 pod 不退出（按 `docs/ops/readyz.md:12` 设计契约 `/healthz` 是进程级 liveness 永远 200，readinessProbe 503 仅撤 endpoint）。**修复**：bootstrap 增加 `fatalErrCh`；ManagedResource 接口扩 `Terminal()` channel 或 `WithFatalSink(chan<- error)` Option；adapter 进 Terminal 时往 channel 投 error；phase9AwaitShutdownSignal select 增加分支 → 触发 phase10 graceful shutdown → process exit non-zero → k8s restart pod。属架构变更级，需评估对 postgres/redis/vault 的扩展。 | Cx3 |
