...
2. `SCAFFOLD-BUNDLE-MARKER-01` / `NO-CODEGEN-LITERAL-01` 被 PR body 自承 "Cannot be Hard"，把升 Hard 任务延迟到 `SCAFFOLD-BUNDLE-ARCHTEST-HARDEN` backlog——这是 `ai-robust.md` 明确禁止的 Soft 新引入 carry-over，应当场升 Medium 或撤回。
...
**AI-robust 评级**：不涉及 enforcement 机制。
...
**问题**：PR body Reflection 节自承 "Medium × 2 (real-source AST capture; marker 是 hand-written string in text/template，no type-system enforcement available)"，并把升级写入 backlog `SCAFFOLD-BUNDLE-ARCHTEST-HARDEN`。根据 `.claude/rules/gocell/ai-robust.md`「Soft 形态严禁立项；新引入 Soft → 直接 reject，要求改 ≥ Medium」。这是**新引入**的 enforcement，不是既有 Soft 的补丁。
**证据**：ai-robust.md "Review checklist"；PR body 自承认 archtest 评级和 backlog 安排。
...
**问题**：PR body 说该 archtest 是 "scanner AST + concrete-package allowlist"，"Bypass requires (a) 改 allowlist + (b) 再加 os.* call"。ai-robust.md 对 Hard 的定义是「违反不可表达」（codegen funnel / type system / sealed interface）。"两步可绕过且两步都 visible in diff" 是 Medium 的典型形态：违反可表达，只是有 archtest 阻挡。kernel-guardian agent 已在 F5 中独立确认此判断（`HasPrefix("scaffold")` 字符串匹配）。
**证据**：ai-robust.md AI-robust 三档表："archtest by string anchor" = Soft；"archtest type-aware" = Medium。当前形态介于二者之间。
...
**AI-robust 评级**：当前 Medium，目标 Hard（path-level depguard）。
...
**AI-robust 评级**：sealed scope = Hard；field-frozen archtest = Medium（typeseval-based）。
...
| Finding | 优先级 | 阻塞类型 |
|---------|--------|---------|
| F2 | **P0** | Soft archtest 新引入违反 ai-robust.md；merge 后须 follow-up PR 升 Medium |
