...
- **✅ 已由 PR-V1-RMQ-TERMINAL（029 A4）吸收清理**：删 `MaxReconnectAttempts` 字段 + `ErrAdapterAMQPReconnectExhausted` errcode + `WaitConnected` godoc 同步；同 PR runtime reconnect 也重新分类 broker permanent 错误并转 StateTerminal。
