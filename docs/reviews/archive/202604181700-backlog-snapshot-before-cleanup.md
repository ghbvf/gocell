...
| ~~P1-2~~ | ~~**CONFIG-DEMO-FAILOPEN-01**~~ ✅ PR#175：`publishEvent` fail-open 整体删除（`runMode` 字段从 configpublish.Service 移除）；durable 模式 `IsDemo()=false` 路径不可触发；PR#175 `refactor(config-core): remove dead runMode fail-open from configpublish` | — | — | PR#157 post-merge |
| ~~P1-3~~ | ~~**PUBLIC-ENDPOINT-METHOD-MATCH-01**~~ ✅ PR#182: 公共端点声明升级为强制 `"METHOD /path"` 格式；无前缀启动 fail-fast；新建共享解析器 `CompilePublicEndpoints`（单点解析消除双实现漂移）；GET 条目自动别名 HEAD（RFC 7231 §4.3.2，对齐 stdlib ServeMux + chi v5）；`WithPublicEndpointMatcher` AuthOption 接入方法感知 matcher；三处信任边界（auth bypass / tracing new-root / request-id reject）联合 (method+path) 键 | — | — | PR#182 |
...
| ~~A6~~ | ~~**#31 RabbitMQ backoff + FailOpen enum 清理**~~ ✅ PR#180（轻量打磨范围）：`ClaimPolicy.String()` + GoDoc + `backoffJitterDivisor` 命名常量；不改默认参数 | — | — | Wave 2 残留 |
...
| ~~S37~~ | ~~**SUBSCRIPTION-FIRST-CLASS-01**~~ ✅ PR#184：`kernel/outbox.Subscription` 一等对象（Topic + ConsumerGroup + CellID）替代 topic-only `TopicHandlerMiddleware`；ConsumerBase 幂等键从静态 `ConsumerGroup` 字段改为 `sub.ConsumerGroup`，修复跨 Cell 扇出 P0 bug（PR#180 遗留：所有 Cell 共享 `core-bundle:{event-id}` 命名空间，后到 Cell 看到 ClaimDone 静默 ACK 丢失事件）；Subscriber 接口重构为 Setup/Ready/Subscribe/Close；SubscriberInitializer 删除；ConsumerBaseConfig.ConsumerGroup 字段移除；跨 ConsumerGroup 非碰撞覆盖测试 | — | — | PR#184 |
| ~~S38~~ | ~~**COMMIT-ACK-ORDERING-01 + LEASE-LOST-HARD-FENCE-01**~~ ✅ PR#184：① processDelivery 改 Commit→Ack 顺序（先 Commit 再 broker.Ack；Commit 失败 → Nack(requeue=true) 而非 Ack，消除先 Ack 再 Commit 失败无法回滚窗口）；② ConsumerBase runWithRenewal 引入 atomic.Bool latch，leaseRenewalLoop 续租失败置位，handler 返回 Ack 时若 latch=true 强制降级为 Requeue（过期 holder 无法因忽略 ctx.Done() 而成功 Ack）；settleReceipt helper 拆解为 dispatchDisposition/dispatchAck/releaseReceipt 三辅助（认知复杂度 ≤15） | — | — | PR#184 |
...
| ~~S8~~ | ~~**H1-7 RBAC-OUTBOX-MIGRATION**~~ ✅ PR#176：`rbacassign.Service` 角色变更原子写 outbox；`sessionlogout/consumer.go` 新建（ConsumerBase + Claimer 幂等）；`contracts/event/role/assigned+revoked/v1` 新增；`cells/access-core/internal/dto/role_event.go` typed event struct；覆盖测试 `rbacassign/outbox_test.go` + `sessionlogout/consumer_test.go` | — | — | PR#149 review round 2 |
...
| ~~S28~~ | ~~**OUTBOX-ENVELOPE-KERNEL-SHARE-01**~~ ✅ PR#177（S30 搭车）：envelope struct + UnmarshalEnvelope/MarshalEnvelope 提升到 `runtime/outbox/envelope.go`（不是 kernel/outbox —— 因 Store interface 同期落在 runtime 层，envelope 与 Store 共包更内聚）；`runtime/eventbus` + `adapters/rabbitmq` 全部改调 `outbox.UnmarshalEnvelope`；rabbitmq 保留 PascalCase legacy Entry fallback | — | — | PR#174 review F1 跟进 |
...
| ~~S30~~ | ~~**OUTBOX-STORE-ABSTRACTION-01**~~ ✅ PR#177: `runtime/outbox.Store` 7 方法 interface（ClaimPending/MarkPublished/MarkRetry/MarkDead/ReclaimStale/CleanupPublished/CleanupDead），`adapters/postgres.PGOutboxStore` 实现并通过 `runtime/outbox/outboxtest.RunStoreConformanceSuite` 验证；relay 编排从 adapter 搬到 `runtime/outbox/relay.go`（三 goroutine 架构保留）；`outboxtest.FakeStore` 暴露为 public in-memory 实现（cell/slice 单测可用，覆盖 91.8%）；同 PR 搭车 S28 envelope 共享 + X7（AL-01）outbox_relay.go 搬迁。净减 ~2.8kloc adapter 代码 | — | — | PR-OUTBOX-WIRE 方案 α 裁定 + PR#177 落地 |
| S31 | **JWT-ISSUER-STRICT-01** (P2, Cx2, 🟡 可延后): `VerifyIntent` 校验 aud/intent/exp 但缺 issuer 强约束；跨环境（staging/prod）密钥被复用时 staging 颁发的 token 可通过 prod 验证。**修复**: `NewJWTVerifier` 增 `WithExpectedIssuer(string)` option；`real` 模式下 issuer 非空强制；补 wrong-issuer → 401 负向测试。建议搭车 S18 | 1h | `runtime/auth/jwt.go` + `cmd/core-bundle/main.go` + test | 2026-04-18 六席审查 |
| S32 | **CONTROLPLANE-TOKEN-PROD-GATE-01** (P2, Cx2, 🟠 条件延后，real 模式部署前触发): non-real 模式下 `/internal/v1/` 依赖部署隔离而非代码层 token 强制。**修复**: bootstrap 启动时 `GOCELL_CELL_ADAPTER_MODE=real` 下断言内部 listener 或 service-token/mTLS 至少一项；补 CI real-mode smoke；对标 Kratos auth/selector 默认拒绝 | 1h | `cmd/core-bundle/main.go` + `runtime/bootstrap/bootstrap.go` | 2026-04-18 六席审查 |
| S33 | **WALKTHROUGH-CI-SMOKE-01** (P2, Cx1, 🟡 可延后): `examples/sso-bff/walkthrough_test.go` 带 `//go:build integration` 未进入主 CI 门禁，walkthrough 回归可静默通过。**修复**: CI 增 `walkthrough-smoke` job（`go test -tags=integration ./examples/sso-bff/...`），或将核心路径拆出 unit subtests 接入默认 `go test ./...` | 1h | `.github/workflows/ci.yml` + `examples/sso-bff/walkthrough_test.go` | 2026-04-18 六席审查 |
| S34 | **AUTH-ADMIN-FORCE-RESET-01** (P2, Cx2, 🟡 可延后): 新功能。Admin 当前必须知道目标 user 的旧密码才能调 ChangePassword。需要新增 admin-only 端点（如 `POST /api/v1/access/users/{id}/password/reset`）允许无旧密码情况下强制重置 + 生成临时密码 + 设置 `PasswordResetRequired=true`。**前置**：决定凭据传递通道（response body / 凭据文件 / 内部邮件）；与 P1-12b first-login enforcement 配套体验设计 | 4h | `cells/access-core/slices/identitymanage/` + `contracts/http/auth/user/reset/v1/`（新）| PR#183 reviewer round 2 P1-2 (defer rationale: 新 feature，非 bug；本 PR 范围限定于 first-run admin bootstrap) |
| S35 | **AUTH-MIDDLEWARE-EXEMPT-INJECTION-01** (P3, Cx3, 🟡 可延后): `runtime/auth/middleware.go::isPasswordResetExempt` 当前 hardcode 业务路径（`POST /api/v1/access/users/{id}/password`、`DELETE /api/v1/access/sessions/{id}`），违反 runtime/cells 分层独立性。**修法**：对标 Kratos `selector.Server(...).Match(matchFunc)`，把 exempt allowlist 通过 `auth.WithSkipPasswordResetCheck(func(method, path) bool)` Option 由 access-core cell 注入；middleware 实现层零业务路径感知 | 3h | `runtime/auth/middleware.go` + `cells/access-core/cell.go` + `runtime/auth/middleware_test.go` | PR#183 reviewer round 2 P1-7 (defer rationale: Cx3 接口签名重构，需独立 PR + 需评估对 sso-bff/cmd/core-bundle wire 影响) |
| S36 | **WORKER-LAZY-HOIST-01** (P3, Cx2, 🟡 可延后): `lazyBootstrapWorker` / `ssoBFFLazyWorker` / `e2eLazyWorker` 三处实现 Lazy worker 包装器，逻辑完全相同。对标 Watermill `disabledPublisher` Null Object 模式，提到 `runtime/worker.Lazy(get func() Worker) Worker` 框架级抽象，三处替换为 `worker.Lazy(...)` | 2h | `runtime/worker/lazy.go`（新）+ `cmd/core-bundle/main.go` + `examples/sso-bff/main.go` + `cmd/core-bundle/outbox_e2e_integration_test.go` | PR#183 reviewer round 2 P2-3 (defer rationale: 跨 PR scope，runtime/worker 改动需独立评审) |
...
| ~~X6~~ | ~~**28 SOL-B-01 Claimer lease 续租**~~ ✅ PR#180：`Receipt.Extend(ctx, ttl) error` + `ErrLeaseExpired`；ConsumerBase 启动 `LeaseTTL/3` 续租 goroutine（fail-closed：Extend 失败 cancel handler ctx → Requeue）；Redis Lua 脚本 fencing token 校验；goleak 验证无残留 | — | — | Wave 2 |
| ~~X7~~ | ~~**AL-01 outbox_relay.go 轮询调度 → `runtime/outbox/relay.go`**~~ ✅ PR#177（S30 搭车）：`adapters/postgres/outbox_relay.go` + `safe_relay_collector.go` 已删除，relay 循环住在 `runtime/outbox/relay.go` | — | — | 依赖替换分析 |
