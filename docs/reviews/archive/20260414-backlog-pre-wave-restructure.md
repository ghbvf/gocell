...
| PR | 任务合并 | 工时 | 文件 |
|----|----------|------|------|
| ~~Health/Readyz 体系~~ | ~~OPS-3(pg/redis Health checker) + ER-P2-03(Router health) + SEC-READYZ-01(/readyz 与 /healthz 隔离) + CFG-P2-01(watcher readyz checker) + READYZ-ROOT(根 readyz 聚合) + **WATCHER-HEALTH-01**(watcher 初始化失败静默降级)~~ ✅ PR#114 | ✅ done | `runtime/http/health/` + `runtime/config/watcher.go` + `runtime/bootstrap/` |
...
| order+demo+examples 修复 | P4-TD-04(outboxWriter enforce) + P4-TD-12(t.Skip) + **EVT-HDR-RESTORE**(outbox接入后恢复 event headers contract_test 验证) + **WM-6-F8**(demo 全局模式开关, C3) + **P3-DEFER-03**(examples 新 API 示例，前置 ER-ARCH-02 ✅) + **NOOP-RENAME-01**(NoopOutboxWriter→NoopWriter 重命名收口，PR#113 review) + **NIL-PUB-P1**(device-register/service.go nil publisher→panic，与 order-cell 同 bug；device-cell/cell.go warn→fail-fast) | 7.5h | `cells/order-cell/` + `cells/demo/` + `cells/device-cell/` + `examples/` **(WM-6-F8/P3-DEFER-03 从 B8 合入)** |
...
| outbox helper 使用护栏 | **OUTBOX-GUARD-01**: NoopWriter/DiscardPublisher 仍是公共 helper，无"非测试/非示例禁止使用"护栏。需 lint/CI 规则或 build tag 约束 (PR#113 tech-debt TD-1) | 1h |
| discard path 可观测 | **DISCARD-OBS-01**: (1) DiscardPublisher.Publish() 走 `slog.Default()` 而非注入 logger，测试无法捕获 warn、生产日志缺 trace_id/order_id 上下文（PR#113 review F）；(2) discard path 无 metrics/health 指标（PR#113 tech-debt TD-3）。建议统一加 Logger 字段 + counter，需注意保留零值可用语义 | 2h |
| order-cell demo 语义 | **ORDER-DEMO-01**: order-cell 默认 demo 模式 DiscardPublisher 下 HTTP 成功但跳过 order.created 事件。PR#113 已修日志语义，剩余产品行为决策 (PR#113 tech-debt TD-4) | 2h |
| 构造函数 nil fallback 推广 | **NIL-PUB-P2**: 5 个 L2 service 构造函数 publisher nil 传播（configwrite/configpublish/sessionlogin/sessionlogout/auditappend）。Cell Init 保护了正常路径，但导出 NewService 独立调用仍有 panic 风险。统一加 `if publisher == nil { publisher = outbox.DiscardPublisher{} }` | 1h |
...
| 6B | 18 → 剩 11 | ~~25h~~ → ~37h | 11/11 | 6A(RMQ) | 6 项 ✅ PR#101, +B8合入(CUR-HDL/FLAG-RACE/TD-12/CB/WM-6-F8/DEFER-03), +NIL-PUB-P1(device-cell) |
| 6C | 4 | ~5d | 2 轨道 | 6A(Receipt) | P1 功能补全 (BFF+SecureCookie) |
| 7 | 6 | ~16h | 5+tag | 6全完 | **v1.0 RC → v1.0** |
| 8 | 20 → 剩 15 | ~~63.5h~~ → ~45.5h | 15/15 | v1.0 | P2 偿债，12 项前置合入 6A/6B(含 Config增强→6A)，OBS-TRUST/NS/KILLSW ✅ PR#108，+5 新增(META-SIZE/OBS-TABLE/OBS-METRIC/OBS-DX/OBS-DOC from PR#108 review)，+NIL-PUB-P2 |
