...
## 12. 续审跟进（2026-04-25 追加 — 报告本体已 archived，仅追加 forward-pointer）

> 本节是 archived 报告外部追加的跟进指针，**不修改原报告任何 finding / 根因 / 结论**；仅说明四根因簇（§7.1-7.4）经过 Wave 1/2/2.5 落地后的覆盖情况，便于后来人定位"baseline 报告还有哪些残余在飞"。

### 12.1 与 2026-04-25 config 域审查的关系

2026-04-25 对 config 域做了一次全链路静态审查（develop @ `788e03e3` 快照），8 条 finding（6×P1 + 2×P2）。审查结论：**8 条全是 baseline §7 四根因簇在 config 域的残余 instance，不是新问题**。详细映射：

| baseline §7 根因簇 | 2026-04-25 config 域残余 finding | 收口 PR | 状态 |
|---|---|---|---|
| §7.1 Fractured Source Of Truth | event 在 producer Go 包 / contract / consumer cache 三处建模（accesscore 直 import `cells/configcore/events`）；config DTO 加了 sensitive 字段但 response.schema.json 未声明 | PR-CFG-3, PR-CFG-4 | 待开（已登记 backlog + plan Wave 2.5） |
| §7.2 Non-Canonical Project Graph | `gocell validate` 没有 dead-event 规则，allow `flag.changed.v1` 死事件长期存在 | PR-CFG-2 | 待开 |
| §7.3 Verification Is Not Executable Reality | `Relay.HealthCheckers()` 实现了无人调（readyz 假绿）；real 模式 + local-aes 仍能启动（启动 gate 假绿）；`tests/e2e/config_pilot_e2e_test.go` 全部 `t.Skip` 但 CI 绿（验证假绿） | PR-CFG-1, PR-CFG-5, PR-CFG-7 | 待开 |
| §7.4 Runtime Kernel Is A Leaky Stub | ManagedResource 的 HealthCheckers 没有自动聚合到 readyz 治理约束；fail-open silent drop 无 counter | PR-CFG-1, PR-CFG-6 | 待开 |

### 12.2 baseline §8 修复顺序的当前进度

| baseline §8 步骤 | 当前状态 | 证据 |
|---|---|---|
| 1. `gocell validate` 成为唯一硬门禁，verify/check/generate fail closed | ✅ Wave 1 PR-A1 治理打底 + Wave 2 PR-A9/A10 落地 | `kernel/governance/`, `cmd/gocell/app/printers/` |
| 2. metadata parser 产出 validated + canonical graph | ✅ PR#239 (PR-A9) CONTRACT-META-01 + PR#152 系列 parser 重建 | `kernel/metadata/parser.go` |
| 3. 修正 6 个 validate 阻塞项 | ✅ 全部清零（路径已重命名/补齐 schema） | — |
| 4. `generate assembly` 严格服从 entrypoint | ✅ PR-A1/A2 周期已落地 | `kernel/scaffold/` |
| 5. CLI verify 真接 runner + selector 覆盖 journeys/assemblies | 🟡 部分（`gocell verify` 走 runner 但 selector 部分待 backlog `VERIFY-JSON-OUTPUT-01`） | — |
| 6. 硬化 runtime（assembly state machine / BaseCell lifecycle / outbox/idempotency contract） | ✅ Wave 1+2 PR-A5a/A5b/A5c/A6/A11/A12/A12b 一批完成 | — |
| 7. 重写 README + 外部叙事 | ✅ PR-A13 PR#235（DOC-NAMING-GUARD + 文档事实源重写） | — |

**结论**：baseline §8 的 7 步骤主线已基本完成；Wave 2.5 的 7 PR 是把根因簇的"残余 instance + 平台治理硬化"在 config 域内做一次彻底回灌，防止其他 cell 再种。

### 12.3 跨参 reference

- 2026-04-25 config 域 8 条 finding 详情：`docs/backlog.md` 条目 `PR-CFG-1..7`
- Wave 2.5 实施计划：`docs/plans/202604232330-025-architecture-pr-implementation-plan.md` §Wave 2.5
- 第十二轮回灌纪要：同上 plan 头部更新段
