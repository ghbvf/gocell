...
| 启动失败 "credential dir not writable" | 凭据目录无写权限或路径非法 | 检查 `GOCELL_STATE_DIR` 是否为绝对路径且进程 user 有写权限。macOS 沙盒/Containerized 部署默认 `~/Library/Application Support/gocell` 可能不可写，需 `export GOCELL_STATE_DIR=$TMPDIR/gocell` 重新启动 |
| 业务接口 403 + ERR_AUTH_PASSWORD_RESET_REQUIRED | 使用了 bootstrap 密码但未完成改密 | 按流程 Step 5 执行改密，使用响应中的新 accessToken 继续操作 |
| macOS 启动报错写文件失败（不带 `credential dir not writable` 字样） | `$HOME` 未设置导致 `~` 展开失败（极罕见，多见于 launchd / cron 上下文） | 先确认 `echo $HOME` 非空；若为空，显式 `export GOCELL_STATE_DIR=$TMPDIR/gocell` 后重启。常规 shell 启动遇到此项请优先按上一行 `credential dir not writable` 处理 |
