...
并写入受限权限凭据文件，同时启动 24h TTL worker 自动销毁该文件。运维通过读取凭据文件
...
| 平台 | 默认凭据文件 |
| --- | --- |
| Linux | `/run/gocell/initial_admin_password` |
...
权限：Unix 目录 `0700`、文件 `0600`；Windows 使用 protected DACL，仅当前用户、LocalSystem 和本机 Administrators 可访问。

文件格式（由 `cells/accesscore/initialadmin` 的 credential writer 生成）：
...
# `sub` claim with base64url semantics.
...
当前 in-memory repo（开发模式）下，单进程内多次 bootstrap 由 initialadmin lifecycle 内置幂等处理（`userRepo.Create` 返回 `ErrAuthUserDuplicate` → silent skip + recount confirm）。
...
| 文件权限被放宽 | 运维误 `chmod` 或 Windows ACL 被改宽 | cleanup 检测到异常仍会删除文件，并记录 slog.Warn；文件已销毁，如忘记密码走下方重置流程 |
| 启动日志 `hook.start_err name=accesscore.initial-admin-bootstrap` | credential 目录不可写、DB 连接失败等 | 看 `error=` 字段定位根因；auto-discovery 已替代旧 worker sink，无需手工接线 |
| 启动失败 "credential dir not writable" | 凭据目录无写权限或路径非法 | 检查 `GOCELL_STATE_DIR` 是否为绝对路径且进程 user 有写权限 |
...
- **凭据文件受限权限**：Unix 使用 0600；Windows 使用 protected DACL，阻止同节点普通用户读取
