...
| **B2-A-18** | ADAPTER-CONNECT-TIMEOUT-INCONSISTENT | P1 (PG 半 ✅) | `adapters/rabbitmq/connection.go:273` + ~~`adapters/postgres/pool.go:69`~~ ✅ | RMQ DefaultDial 直接调 `amqp.Dial` 无内置超时；~~PG pool 仅 ctx.Ping~~ ✅ 已加 `Config.ConnectTimeout` 默认 5s。**修复**：每个 adapter 暴露 `WithConnectTimeout(d)`，缺省 5s；构造期 fail-fast。**归宿（拆双）**：RMQ 半 → 029 roadmap **B13 PR-V1-RMQ-CONFORMANCE-AND-CLOSURE**（**未关闭**）；PG 半 → ✅ 已合 **PR-V1-PG-STARTUP-HARDEN**（fix/230-pg-startup-harden，B4+B8 合并；`Config.ConnectTimeout` 默认 5s 单层方案写到 `pgconn.Config.ConnectTimeout`，覆盖 DSN connect_timeout；TestNewPool_ConnectTimeout_Blackhole 锁 RFC 5737 黑洞 IP ≤ 2s 返回）。 | Cx2 |
