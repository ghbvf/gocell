...
|---|---|---|---|---|---|---|---|
| ~~B2-PROVISIONER-MUTEX-REVIEW~~ | ✅ **关闭** — Provisioner.mu 已删除 + WithSetupLock 升级为强依赖 wiring option (validateRequiredDeps phase0 fail-fast)。PR #595 ship-review 进一步把 (UserRepo, RoleRepo, SetupLock, TxRunner) 散装 option 整体 unexport，收口到 `accesscore.WithMemBundle(accessmem.Bundle)` / `accesscore.WithPGBundle(accesspg.Bundle)` 单一公开 wire path（Hard 双向 funnel：上游字段私有 + 唯一 NewBundle 工厂；下游 archtest `ACCESSCORE-BUNDLE-FUNNEL-01` 锁 root export）。`ADMINPROVISION-SETUPLOCK-PAIRED-CTOR-HARD-02` 同 PR 一并 ✅。 | refactor | P2/Cx2 | ✅ | — | `cells/accesscore/{mem,postgres}/bundle.go` + `cells/accesscore/bundle_options.go` + `cells/accesscore/cell.go` | backlog2 §13 + PR #482 unblock + PR #595 |
| ~~ADMINPROVISION-SETUPLOCK-PAIRED-CTOR-HARD-02~~ | ✅ **关闭 (PR #595 落地)** — `accessmem.Bundle` / `accesspg.Bundle` typed funnel 已合入: `NewBundle` 是 (UserRepo, RoleRepo, SetupLock, store-paired TxRunner) 唯一构造器, 字段私有, `accesscore.WithMem/PGBundle` 是唯一 root option, 散装 With{User,Role}Repository / WithSetupLock / WithTxManager unexport. caller 在 PG 模式误传 NoopSetupLock 在 type system 上不可表达（NewPGBundle 内部 derive PG setupLock from txMgr）。 | refactor | P3/Cx2 | ✅ | — | `cells/accesscore/{mem,postgres}/bundle.go` + `cells/accesscore/bundle_options.go` + `tools/archtest/accesscore_bundle_funnel_test.go` | PR #595 |
| C-04 | **CELLS-INIT-TEMPLATE-CONVERGE**（含 C-07 emitter health probe helper）— 3 cell Init 切分各异 + internal/ 子包不对称；修复: `kernel/cell` 提供 `BaseCell.RegisterStandard(reg, StandardInit{...})` 模板 + scaffold 预生成 `internal/{ports,domain,dto,events,mem}` 五目录 + 3 cell 改造 + scaffold 升级 + 抽 `cell.RegisterEmitterHealthProbes(reg, emitter)` helper（删 3 cell **3 处** `c.emitter.(cell.HealthProber)` 重复：configcore/cell_init.go:87 · auditcore/cell.go:279 · accesscore/cell_init.go:409；RepoHealthProber 已独立 typed funnel 不计）+ 吸收 C-09 三 cell 文件布局规范化（cell_lifecycle.go/cell_helpers.go 惯例 + scaffold 模板 + archtest）| refactor | P1/Cx2 | 🟡 | K-06 落地后 | `kernel/cell/` + `cells/{accesscore,auditcore,configcore}/` + scaffold 模板 | 030 §3 C-04 + C-07 |
| ~~C-06~~ | ✅ **关闭 (PR #602)** — 走 option (b): `docs/architecture/consistency.md` + `glossary.md` 明确 L0 cell 为未来扩展点、当前无实例；schema 字段保留供未来 L0 实例使用 | doc | P3/Cx1 | ✅ | — | `docs/architecture/consistency.md` + `docs/architecture/glossary.md` | 030 §3 C-06 + PR #602 |
| C-09 | **CELL-SPLIT-LAYOUT-NORMALIZE** — 三 cell 文件布局已分化无统一范式（**2026-05-19 代码核实 re-scope**：原"`cell_routes.go` 名不副实 / `RegisterSubscriptions` 错位"前提**已失效**——`cell_routes.go` 经 C-05 #447 删除、`RegisterSubscriptions` 经 K#05/K#06 codegen 迁移消除、configcore 已有 `cell_lifecycle.go`）。残留真问题：(a) auditcore 单 `cell.go` 17 func 全混未拆；(b) accesscore lifecycle 注册埋在 `cell_init.go:404 registerHealthAndLifecycle` 未迁 `cell_lifecycle.go`；(c) pure helper（configcore `deriveModes`/`ensureCursorCodec`、accesscore `validateRequiredDeps`）混在 `cell_init.go` 未归 `cell_helpers.go`；修复: 引入 `cell_lifecycle.go`+`cell_helpers.go` 命名惯例 + 三 cell 反向迁移 + scaffold 模板预生成骨架（codegen funnel）+ archtest 固化（无 enforcement 即 Soft，按 ai-robust.md 严禁单独立项，**须并入 C-04 同 PR**）| refactor | P2/Cx2 | 🟡 | 并入 C-04（文件域 100% 重叠；K-07 依赖已解除） | `cells/{accesscore,auditcore,configcore}/` + `tools/codegen/cellgen/templates/{scaffold-cell,cell}.tmpl` + `tools/archtest/` | 030 §3 C-09（2026-05-19 re-scope，原 cell_routes 措辞作废）|
...
| ID | 描述 | Type | P/Cx | Flag | Trigger | Files | Source |
|---|---|---|---|---|---|---|---|
| P1-8 | **DEVICE-LIST-API** — 现状: `cells/devicecell/slices/devicelist/` 缺；修复: 新建 slice + `GET /api/v1/devices` 分页 + contract + contract_test | feat | P1/— | ✅ | — | `examples/iotdevice/cells/devicecell/slices/devicelist/` + `examples/iotdevice/contracts/http/device/list/v1/` | backend_issues.md #1 — 2026-05-19 核实落地（slice + contract + 4 个 test 全在）|
| B2-T-04 | **Contract userId 风格混用** — 现状: payload schema 字段命名混用 userId/UserID；修复: 统一 camelCase | refactor | P2/Cx2 | ✅ | — | `contracts/event/user/created/v1/payload.schema.json:6` | backlog2 §8 B2-T-04 — 2026-05-19 核实统一为 camelCase |
| F-03 | **PKG-CTXKEYS-NO-CELL-MODEL ARCHTEST**（2026-05-19 re-scope，原 PKG-CONTRACTS-BOUNDARY-DOC 主前提失效）— `pkg/contracts/` 路径已删，Hard archtest `CONTRACTTEST-BOUNDARY-01` 守"legacy pkg/contracts must stay deleted"，doc 重建不可行。残留: `pkg/ctxkeys/doc.go` 已写边界但 `tools/archtest/` 内无 `PKG-CTXKEYS-NO-CELL-MODEL` 守护，cell-model identifier 误入 pkg/ctxkeys 无静态拦截；修复: 落 archtest 锁 pkg/ctxkeys 不出现 cell-model 类型 | doc | P3/Cx1 | 🟠 | 出现首例 cell-model identifier 误入 pkg/ctxkeys | `tools/archtest/` | 030 §3 F-03 → 2026-05-19 re-scope |
...
| ID | 描述 | Type | P/Cx | Flag | Trigger | Files | Source |
|---|---|---|---|---|---|---|---|
| A26-R3 | **SETUP-PATH-NAMESPACE-POLICY-01** — 现状: 顶级 `/api/v1/setup/` 与 per-Cell 入口规则未明文；修复: 在 api-versioning.md 写明 | doc | P3/Cx1 | 🟡 | — | `.claude/rules/gocell/api-versioning.md` | PR#247 round-2 N-01 |
| HTTPUTIL-WRITEERRORBODY-DOUBLE-MARSHAL | **错误响应双重 JSON marshal** — 现状: writeErrorBody marshal+unmarshal+encode 三次；修复: errcode.MarshalJSON 原生支持 envelope 注入 | bug | P3/Cx2 | 🟡 | HTTP 错误成 hot path | `pkg/httputil/response.go` + `pkg/errcode/errcode.go` | PR #391 review round-2 |
...
| LISTENER-API-SPEC-01 | **Listener API spec 化** — 现状: listener 选项散在代码；修复: contracts 化声明 | arch-opt | P3/Cx3 | 🟡 | — | `contracts/http/` | PR#237 |
| ROUTE-ERROR-POLICY-01 | **Route error policy 统一** — 现状: 3+ route family 错误处理不一；修复: 定义共享 policy | arch-opt | P2/Cx3 | 🟠 | 3+ route 家族出现 | `runtime/http/` | systems review |
| T4 | **CB-RESILIENCE-PACKAGE-01** — 现状: Allower / CircuitBreakerRetryAfter 在 `runtime/http/middleware`；修复: 迁到 `runtime/resilience/circuitbreaker/` 独立包 (also: cap-x-cross) | refactor | P3/Cx2 | 🟠 | 出现第 2 个非 HTTP CB 消费方 | `runtime/http/middleware/` + `runtime/resilience/circuitbreaker/` (新) | T4 |
| WM-32 | **mTLS 中间件** — 现状: 缺；修复: 加 TLS 构建器 + HTTP 证书提取钩子（折中：大规模环境 mTLS 卸载在 K8s/Service Mesh 解决，框架仅提供构建器） | feat | P3/Cx2 | 🟡 | V1.1 启动 | `runtime/http/middleware/` | backlog_later §7 WM-32（4/6 票）|
| ~~B2-T-08~~ | ✅ **关闭 (PR #598)** — `contracts/http/config/publish/v1/contract.yaml` + handler/handler_test 补 404 失败码声明 + 单测验证 | bug | P2/Cx1 | ✅ | — | `contracts/http/config/publish/v1/contract.yaml` + `cells/configcore/slices/configpublish/handler.go` | backlog2 §8 B2-T-08 + PR #598 |
...
| READYZ-PROBE-FAILURE-METRIC-01 | **Readyz probe 失败 Prometheus counter** — 现状: 四通道模型落地后 wire 简化，operator 唯一诊断信号是 slog；缺 `readyz_probe_failures_total{name=...}` 细粒度告警，无法在不解析 slog 的情况下触发 PagerDuty 等告警规则；修复: `runtime/http/health` 暴露 prometheus counter，与 `_ready` 命名约束对齐 | feat | P3/Cx2 | 🟡 | 监控有需求时 | `runtime/http/health/` | ADR 202605171200 §3 / PR #552 review C8 |
| FOUR-CHANNEL-HANDLER-EXTENSION-01 | **ADR 202605171200 §6 funnel matrix 表新增**（2026-05-19 re-scope：§5 扩展协议 3 步已写完，仅 §6 funnel matrix 表残留）— 当第二个 handler 引入 ops-diagnostics 通道 d（典型场景: recovery middleware panic dump / outbox last_error sanitize / auditquery payload redaction）时，需在 ADR §6 funnel matrix 追加条目，并对齐 typed redacted 包装类型 + archtest funnel 形态 | doc | P3/Cx1 | 🟢 | 新增 handler 引入时 | `docs/architecture/202605171200-adr-readyz-verbose-four-channel-redaction.md` §5 §6 | PR #552 review C9 |
...
| PR-A8-RESIDUAL | **Vault K8s auth E2E** — 现状: Vault K8s auth 实现已落，缺真 K8s e2e；修复: 跑 testcontainers k8s 验证 | arch-opt | P3/Cx2 | 🟡 | — | `adapters/vault/` | PR#305 |
...
| PR267-FU-AUTHTEST-INTERNAL | **Auth test 内部化** — 现状: testHelpers 暴露过多；修复: internal package | arch-opt | P3/Cx1 | 🟡 | — | `cells/accesscore/` | PR#267 |
| PR267-FU-ROLE-PREFIX-ADR | **Role prefix ADR** — 现状: role 命名前缀约定无 ADR；修复: 写 ADR | doc | P3/Cx1 | 🟡 | — | `docs/architecture/` | PR#267 |
| X3 | **WM-36 SecureCookie key rotation** — 现状: 无密钥轮转；修复: 接入 rotation worker | feat | P3/Cx2 | 🟡 | — | `runtime/auth/` | WM-35 后续 |
| X5 | **P3-TD-11 accesscore domain 拆分** — 现状: domain 包过大（admin/role/user 已拆分，Session entity 仍寄居 runtime/auth/session）；修复: User/Session/Role 拆分 | refactor | P3/Cx2 | 🟡 | X1 落地后 | `cells/accesscore/internal/domain/` | 历史 Batch 8 |
...
| T5 | **AUTH-SIGNER-01** — 现状: SigningKeyProvider 返回 `*rsa.PrivateKey`；修复: 改 `crypto.Signer` 支持 HSM/KMS/EC | arch-opt | P3/Cx3 | 🟡 | caller 需 HSM/KMS | `runtime/auth/` | T5 |
| C-AC7 | **JWT jti claim 支持** — 现状: Issue() 已加 jti（runtime/auth/jwt.go:386 + claims_test.go:31 parsed），残留为 blacklist 存储；修复: jti 黑名单存储 | feat | P3/Cx2 | 🟡 | 出现单 token 撤销需求 | `runtime/auth/` | backlog_later §6 C-AC7 |
| AUTHZ-MUTATION-FUNNEL-UPSTREAM-HARD-01 | **funnel 上游 caller-set 由 Medium 升 Hard** — 现状: `AUTHZ-MUTATION-APPLY-FUNNEL-01` 下游 Hard（domain 字段私有化 compile-time 不可绕过）已闭合，但上游 caller-set 是文件级 archtest allowlist（Medium-by-necessity）：creation-time 调用无法用 sealed interface / codegen 在 compile 期表达"仅限构造时"语义，且 identitymanage.Delete / changePasswordInTx / rbacassign.Revoke 因 co-tx aggregate 语义必须直调 `invalidator.Apply`（经 authzmutate 是聚合语义错误，非疏漏）。#494 residual 对标 ent(`tx.Client()`=capability=Medium)/go-kratos(context-tx=Low) 实证：Go 在 tx-bound side-effect funnel 的类型天花板就是"下游 Hard + 上游 Medium"，任何 TxHandle/marker 是伪 Hard。修复方向（若未来出现可达 Hard 路径）: codegen 注入构造点 + sealed tx-scoped handle 让"非构造期 / funnel 外调用"包外不可表达。**charter §Funnel双向锁 mandated 显式登记项**，D1 archtest godoc 已点名本条。 | arch-opt | P2/Cx3 | 🟠 | 出现 codegen-injected 构造点可表达"仅构造期"语义时立项 | `cells/accesscore/internal/{authzmutate,credentialinvalidate}/` + `tools/archtest/domain_authz_mutation_funnel_invariants_test.go` | ADR-credential §A10; ai-robust.md §Funnel双向锁评级; #494 residual review |
| CREDENTIAL-AUTHORITY-FUNNEL-SCOPE-AUTO-DERIVE-01 | **funnel scope 自动派生** — 现状：`tools/archtest/credential_authority_assert_funnel_test.go` 的 `assertCallerAllowlist` + `sliceFunnelScopes` 是手维护的 3 个 slice 前缀；新增第 4 slice 必须手动更新 archtest allowlist 否则 funnel 失效。修复方向：从 `cells/accesscore/cell.yaml` + 各 `slice.yaml` 元数据派生 owner package 集合（参见 `kernel/governance` 现有元数据派生模式）。**触发条件**：accesscore 新增第 4 slice 时立项。 | arch-opt | P2/Cx2 | 🟠 | accesscore 新增第 4 slice | `tools/archtest/credential_authority_assert_funnel_test.go` + `tools/archtest/session_revoked_field_access_test.go` + cell.yaml 派生器 | PR #542 reviewer Cx3 #4 |
...
| SESSIONREFRESH-STALE-EPOCH-REJECT-HARDEN-01 | **SESSIONREFRESH-STALE-EPOCH-REJECT archtest Hard 升级** — 现状: `SESSIONREFRESH-STALE-EPOCH-REJECT-01` 所有 prong 使用 go/parser AST 名字/字符串锚点（Sel.Name / Ident.Name / BasicLit）而非 typeseval 类型身份解析，AI-robust 评级为 Medium（PR #494 review F1）；修复: 将 `cascadeRevoke` 和 `handleReuseDetected` callee 识别从 Sel.Name 字符串锚点替换为 `typeseval.ResolveMethodCall` 类型身份解析，同时对 rejectIfStaleEpoch 调用点的 control-flow 形态做类型级锁定，升为 Hard | arch-opt | P2/Cx2 | 🟠 | 下一次 archtest hardening pass | `tools/archtest/sessionrefresh_stale_epoch_reject_test.go` | PR #494 review F1 |
| P4-TD-03 | **IssueTestToken HS256 dead code** — 现状: 测试 helper 仍保留 HS256 路径，JWTVerifier 全拒；修复: 删 dead code 防误用 | refactor | Cx1 | ✅ | — | `cells/accesscore/slices/sessionvalidate/helpers_test.go:46` + `runtime/auth/jwt.go:257` | tech-debt-registry P4-TD-03 — 2026-05-19 STALE-CLOSE：IssueTestToken 已强制 RS256，HS256 仅在 negative test 出现 |
| SECURECOOKIE-AEAD-NEG-01 | **SecureCookie AEAD 负向测试** — 现状: AEAD 失败路径无测试；修复: 截断/伪造/边界长度/解密失败类型断言 (`errors.Is(err, ErrAEADAuthFailed)`) | test | P2/Cx2 | 🟡 | v1.0 GA 前 | `pkg/securecookie/securecookie_test.go` | backlog1 §2.5 |
| S1-CO-02-WIRING-OPTION-STICKY-DOCTRINE | **runtime-api.md sentinel sticky 通用契约明示** — 现状: 多处 wiring option（router.WithRateLimiter/WithCircuitBreaker/WithAuthMiddleware + session.WithFingerprint/WithOrdering）已实现 sentinel 粘滞失败行为，但 `.claude/rules/gocell/runtime-api.md §Option 范式分层` 未明示此为通用契约；session 包内已加 sticky test 锁定 Medium AI-robust；修复: 章程层面明示 + 可选 archtest 跨 option 检测注释一致性 | doc+test | Cx2 | ✅ | — | `.claude/rules/gocell/runtime-api.md:268-274` + `runtime/auth/session/protocol_test.go:329,348` | PR#439 reviewer P1 follow-up — 2026-05-19 主体已落地（§Option 范式分层 + sticky test）；archtest 跨 option 检测留作触发型 follow-up |
...
| ID | 描述 | Type | P/Cx | Flag | Trigger | Files | Source |
|---|---|---|---|---|---|---|---|
| T3 | **DEVICE-ENQUEUE-RBAC**（2026-05-19 路径 stale：devicecell 已迁 examples/iotdevice/）— 现状: `examples/iotdevice/cells/devicecell/slices/devicecommand/handler.go:25-31` Enqueue adapter 仅透传 `req.ID`，无设备粒度 RBAC，对比 status endpoint 已用 `auth.SelfOr("id", ...)` ownership 模式；修复: 加设备粒度策略 | feat | P3/Cx2 | 🟠 | 多租户 operator | `examples/iotdevice/cells/devicecell/` | T3 |
| T11 | **ADMIN-ROLE-DEDUP** — 现状: admin role 字符串散在多处；修复: 抽 const 单源 | arch-opt | — | ✅ | — | `runtime/auth/roles.go:3-4` | T11 — 2026-05-19 核实 RoleAdmin const 已抽，生产代码无裸字面量 |
| B2-T-07-FU-2 | **BUILTIN-SERVICE-ROLES 删除 FU** — 现状: scope 派生 builtin role 还在 hard-code；修复: 完全派生（A5 follow-up） | arch-opt | Cx3 | ✅ | — | `runtime/auth/principal.go:53-78` + `tools/archtest/no_deleted_auth_symbols_test.go` | backlog2 §8 A5 follow-up — 2026-05-19 核实 service principal 改 CallerCellID + archtest Hard 锁 3 符号删除 |
...
| ID | 描述 | Type | P/Cx | Flag | Trigger | Files | Source |
|---|---|---|---|---|---|---|---|
| ~~PR341-FU-OUTBOXTEST-CLOSE-BUDGET-COVERAGE~~ | ✅ **关闭 (PR #596)** — `kernel/outbox/outboxtest/conformance.go` 全部 route Close 走 `closeWithBudget`；helper + doc 同步说明边界（测 Close 自身语义场景仍可裸调，但 route 路径强制走 budget） | test | P3/Cx1 | ✅ | — | `kernel/outbox/outboxtest/conformance.go` + `helpers.go` + `doc.go` | PR #341 round-1 + PR #596 |
| RBACASSIGN-L2-PG-ATOMICITY-01 | **RbacAssign L2 PG 原子性测试** — 现状: rbacassign 仅有 mem-based L2 路径测试（RecordingWriter），无 PG-level outbox 原子性验证；修复: testcontainer 驱动 PG outbox writer，故意 fail writer → 验证 domain 写成功 + outbox 失败 → tx rollback（与 AUDITAPPEND-L2-FAILURE-PROOF-01 同模式）。**2026-05-19 核实：X1 PG accesscore 仓储已落地（role_repo.go + user_repo.go），可直接套用 audit 范本**。 | test | P1/Cx2 | 🟡 | — | `adapters/postgres/` (新) + `cells/accesscore/slices/rbacassign/` | PR #514 reviewer F9 |
...
| KERNEL-RECONCILE-01 | **kernel/reconcile L3 收敛循环** — 现状: 缺 Reconciler 模式；修复: 新建 reconcile 包 | feat | P3/Cx3 | 🟡 | L3 业务出现 | `kernel/reconcile/` (新) | backlog_later §2 |
| WM-18 | **延迟消息原语** — 现状: 缺 TTL；修复: RMQ x-delayed-message 插件绑定 + 测试桩支持（运维成本拉升，等 Outbox 稳定后探索） | feat | P3/Cx2 | 🟡 | V1.1 启动 + Outbox 彻底稳定 | `adapters/rabbitmq/` + outbox | backlog_later §7 WM-18（3/6 票）|
| ~~R-02~~ | ✅ **关闭 (PR #599)** — `runtime/eventbus/eventbus.go` broadcast/roundRobin drop 路径升 Error 级 + entry_id / aggregate_id / event_type 三字段（与 R-01 counter 对应）| bug | P2/Cx1 | ✅ | — | `runtime/eventbus/eventbus.go` | 030 §2 R-02 + PR #599 |
...
| ID | 描述 | Type | P/Cx | Flag | Trigger | Files | Source |
|---|---|---|---|---|---|---|---|
| PR-CFG-A-DEFER-2 | **ConfigCore L2 divergence** — 现状: L2 与 L1 表项 schema 偏差；修复: 收口 | arch-opt | Cx1 | ✅ | — | `cells/configcore/` | PR#268 — 2026-05-19 STALE-CLOSE：config_entries vs feature_flags 两表 schema 差异是按 concept 设计（L1 flagwrite / L2 configpublish 有意分级），非 bug |
...
| PR320-FU-CONFIGCORE-CI-NOOP | **ConfigCore CI noop test** — 现状: noop publisher CI 路径未覆盖；修复: 加测 | test | P3/Cx1 | ✅ | — | `cells/configcore/slices/configpublish/service_test.go:37-43` | PR#320 — 2026-05-19 核实：newTestService 默认 NoopEmitter 隐式覆盖 happy-path |
| B2-A-33 | **Redis sentinel env & logvalue 缺** — 现状: sentinel 模式 env 配置不完整 + log value 缺；修复: 补 env 列表 + logvalue 透传 | bug | P3/Cx2 | 🟠 | sentinel 部署需求出现 | `cmd/corebundle/redis.go:18-22` + `adapters/redis/client.go:90-104` | backlog2 §5.3 B2-A-33 |
| PR238-FU8 | **CONFIGREPO-UPDATE-ROLLBACK-OP-LABEL-TEST-01** — 现状: `doUpdate` 通过 `op` 参数向 `scanConfigOrMapError` 传 `"Update"` 或 `"UpdateForRollback"`，`InternalMessage` 携带该 op，但 `TestConfigRepository_UpdateForRollback_NotFound` / `TestConfigRepository_UpdateForRollback` 均未断言 InternalMessage 含 `"UpdateForRollback"`，若有人把 op 硬编码回 `"Update"`，CI 不会 FAIL；修复: 相关 NotFound 测试追加 `assert.Contains(t, ec.InternalMessage, "UpdateForRollback")` | test | P3/Cx1 | ✅ | — | `cells/configcore/internal/adapters/postgres/config_repo_test.go:363-367,407-408` | PR#238 L4 round-2 reviewer T-R4 + PR#553 ship — 2026-05-19 核实双向 NotContains 锁已就位 |
| CONFIGREPO-OP-LABEL-TYPED-ENUM-HARD-01 | **op label typed enum (Hard 升级 PR238-FU8)** — 现状: PR#553 抽 `opUpdate / opUpdateForRollback` unexported const + `Update_NotFound` 双向 NotContains 锁定到 AI-robust Medium；改字面量需改 const 单点。修复方向: doUpdate 接受 typed `updateOp` private type（2 valid values），InternalMessage 由 enum.String() 派生，硬编码 string 编译失败（违反不可表达）→ Hard | arch-opt | P2/Cx2 | 🟠 | 同模式 op-string-label-in-error-internal-message 在 ≥ 2 个其他仓储路径出现 | `cells/configcore/internal/adapters/postgres/config_repo.go` | PR#553 plan §AI-robust evaluation |
...
| ID | 描述 | Type | P/Cx | Flag | Trigger | Files | Source |
|---|---|---|---|---|---|---|---|
| PR-V1-PG-STARTUP-HARDEN-FU-RACE-COVERAGE | **TEST-RACE-COVERAGE-ADAPTERS-INTEGRATION-01** — 现状: PG concurrent Up CI 不带 -race；修复: test-race.yml 加 adapters/postgres 路径（评估） | test | P2/Cx3 | ✅ | — | `.github/workflows/test-race.yml:50-105` | PR-V1-PG-STARTUP-HARDEN F5 — 2026-05-19 核实 race-pg-integration job 已落 + CI-RACE-LANE-SUBSET-01 archtest 守 discovery |
| X1 | **PG-DOMAIN-REPO** — 现状: 5 个 Repository 仅内存；修复: User/Session/Role/Device/Command PG 实现 + 4 migration DDL；联动 RBAC-ASSIGN-LEVEL-UPGRADE ✅ closed by S4c-T1 (rbacassign 统一 L2 + RBACASSIGN-L2-STATIC-01 archtest 锁定字面量)/ SEED-ROLE-IFACE ✅ closed by S4c-T1 (adminprovision 重构隐性闭环 + SEED-ROLE-IFACE-01 Hard archtest 锁定生产代码零 `*mem.RoleRepository` 引用)/ AUTH-CACHE 激活 ✅ closed by S4c-T5 (`adapters/redis.CachingSessionStore` 默认关闭，env `GOCELL_SESSION_CACHE_TTL` 启用；触发型 backlog AUTH-CACHE-SUBJECT-REVERSE-INDEX-01 守 user.AuthzEpoch 进 cache 升级路径) | feat | P3/— | ✅ | — | `adapters/postgres/*` + `cells/accesscore/internal/adapters/postgres/*` | PR#155 review F4 — 2026-05-19 核实 5 个 Repo PG 实现全部落地（user/role/session/command + devicecell PG 在 PR#575 ship） |
| S14a | **AWS KMS provider** — 现状: 仅 Vault；修复: 加 KMS adapter | feat | P3/Cx3 | 🟠 | 云平台部署需求 | `adapters/kms/` (新) | S14a |
...
| ID | 描述 | Type | P/Cx | Flag | Trigger | Files | Source |
|---|---|---|---|---|---|---|---|
| V-A8-DEFERRED | **CMD-CORE-INTERNAL-GUARD-PUBLIC-01** — 现状: cmd/corebundle/main.go 28 行，archtest 锁 ≤30；修复: 触发后评估提升为公开类型 | debt | P3/Cx2 | 🟠 | runtime/bootstrap 子包出现 / 多消费方 | `runtime/bootstrap/` + `cmd/corebundle/` | PR-A64a deferred |
| PR252-F1 | **QueueRegistrar bootstrap 集成** — 现状: 当前仅 InMemQueue；修复: 下一个 durable command adapter 落地时加入。**2026-05-19 核实 trigger 已达成**：`adapters/postgres/command_queue.go:27` PGCommandQueue 已落 + `runtime/command/bootstrap_phase.go:15` DiscoverQueueRegistrars 已提供，但 cmd/corebundle/ 无消费点 | arch-opt | P2/Cx3 | 🟡 | — | `runtime/command/` + `cmd/corebundle/` | PR#252 |
| PR252-F2 | **Sweeper 生产治理** — 现状: 单 replica 假设；修复: multi-replica command consumer 时落 | arch-opt | P3/Cx4 | 🟠 | multi-replica command consumer | `runtime/command/` | PR#252 |
| PR333-BOOTSTRAP-OPTION-CROSS-CONCERN | **Bootstrap option 跨 concern 拆分** — 现状: option 概念混杂；修复: 按 concern 拆 | arch-opt | Cx2 | ✅ | — | `runtime/bootstrap/options_*.go` | PR#333 — 2026-05-19 核实 PR-A66 (#333) merge `7c9816558`，5 个 options_*.go 按 concern 拆分到位 |
| PR448-BUDGET-ISOLATION-PARENT-CHAIN-GUARD | **PHASE10-TEARCTX-PARENT-CHAIN-GUARD-01** — 现状: TestPhase10_BudgetIsolation_LIFOTeardownGetsFreshCtx 只断言 ctx 未 done，无法检测未来若有人把 tearCtx 改成 `context.WithTimeout(drainCtx, ...)` 形成继承链导致 budget 隐性泄漏；修复: 加一断言用 `context.WithCancel(Background)` 包装 tearCtx parent，drain 期间主动 cancel 并验证 tearCtx 不传播 cancel | test | P3/Cx1 | 🟡 | — | `runtime/bootstrap/shutdown_ordering_test.go` | PR#448 reviewer F4 |
| COREBUNDLE-MAINTEST-FAIL-FAST-01 | **corebundle main_test fail-fast** — 现状: bind 错误被白名单吞掉；修复: 用 `net.Listen("tcp", "127.0.0.1:0")` 注入 + 断言关键装配里程碑 | test | P3/Cx2 | 🟡 | — | `cmd/corebundle/main_test.go` | backlog1 §2.7 |
...
| B2-X-04 | **HEALTH-LISTENER-DEFAULT-LOOPBACK** — 现状: `cmd/corebundle/shared_deps.go:128-135` health listener 默认 `127.0.0.1:9091`，real mode 仅靠注释 + `GOCELL_HTTP_HEALTH_LOCAL_ONLY=1` opt-in 提醒，未显式绑定 PodIP/Service 时 K8s kubelet/Prometheus probe 不可达且无 fail-fast；修复: real mode 缺显式 bind 时 fail-fast，或默认 Pod-reachable `:9091` + archtest 锁 internal healthz only via mTLS | bug | P1/Cx2 | ✅ | — | `cmd/corebundle/shared_deps_validate.go:177-189` + `shared_deps_test.go:152-158` | backlog2 §3 B2-X-04 — 2026-05-19 核实 validateHealthReachability 已实现 real-mode loopback fail-fast |
| B2-X-03 | **PG invalid index warn continue** — 现状: PG invalid index 仅 warn 继续启动；修复: 改 fail-fast 防隐藏数据完整性问题 | bug | P2/Cx2 | ✅ | — | `cmd/corebundle/bundle_configcore_storage.go:98-104` + `adapters/postgres/schema_guard.go:1007` | backlog2 §7 B2-X-03 — 2026-05-19 核实 VerifyNoInvalidIndexes fail-fast 已替换 warn-continue |
