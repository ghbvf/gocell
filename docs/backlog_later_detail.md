...
| **PR-A12-ACK-ATOMIC** | P2 | adapters/postgres command store 实现时需在单事务内完成 Pending→Sent→Delivered→Succeeded 链式 advance。当前 devicecommand.Service.Ack 由三次独立 StateAdvancer 调用组成（非原子），demo 模式下 InMemQueue 互斥锁掩盖了这一问题；postgres adapter 必须以 BEGIN/COMMIT 包裹整个链式 advance 消除中间状态可见性。详见 `kernel/command/ports.go` StateAdvancer godoc 说明。 |
| **PR-A12-SWEEPER-WIRE** | P2 | `kernel/command.Sweeper` 当前只支持单设备扫描（`Sweeper.DeviceID` 必填）；多设备 / 全局 sweep 需要扩 `kernel/command.Reader` 接口增加 `PendingAll(ctx) ([]Entry, error)` 方法，并放宽 `Sweeper.Start` 对 `DeviceID == ""` 的拒绝，改为"空串 = 全局扫描"语义。本 PR 同步遗留了 `examples/iotdevice/cells/devicecell/cell.go` 的 `TODO(PR-A12-SWEEPER-WIRE)` 接线占位——待 Reader.PendingAll 落地后把 Sweeper 作为 `LifecycleContributor` 注册到 cell init。建议与 `adapters/postgres/command_store.go` 同批实现（两者共用 PendingAll 接口）。 |
| ~~**kernel/command**~~ ✅ | ~~P1~~ | ✅ 已落地 PR-A12（2026-04-24，分支 `refactor/522-pr-a12-kernel-command`）：`kernel/command/` 新包 + `Queue` 门面（Enqueue/Dequeue/Ack/ExtendLease/Cancel/ListPending）+ `Sweeper` 过期驱动器 + `commandtest.InMemQueue` 测试后端；`examples/iotdevice/cells/devicecell/slices/devicecommand/` 彻底迁移至新底座；T3 DEVICE-ENQUEUE-RBAC 预埋点 `EnqueueOptions.Authz` 就绪。残余 `PR-A12-SWEEPER-WIRE`（InMemQueue 多设备扫描）作为 backlog 单独登记。 |
