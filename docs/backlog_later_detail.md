...
* **AL-02 (DistLock):** 🟢 in flight (PR-A20, refactor/531)。重设计为 Lock-as-Context API（`Acquire` 返回 `(context.Context, func(), error)`）+ 单共享 manager goroutine + Driver 三原语 (SetNX/Renew/Release)；`adapters/redis/distlock.go` 重写为 RedisDriver。
