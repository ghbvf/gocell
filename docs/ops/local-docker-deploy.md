...
`GOCELL_AUDIT_ADMIN_PASSWORD` is **not** auto-generated by the script. Operators who
want the super-admin cross-tenant audit read capability must add it manually to
`.env.local` (e.g. `GOCELL_AUDIT_ADMIN_PASSWORD=$(openssl rand -hex 16)`) before
running `make local-up` on a fresh data directory. Omitting it is the default and
leaves `make local-up` unaffected.
