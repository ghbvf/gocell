...
`cmd/corebundle` uses Redis as the shared coordination backend for real multi-pod deployments. When `GOCELL_ADAPTER_MODE=real` and `GOCELL_SINGLE_POD` is not set, **either** `GOCELL_REDIS_ADDR` (standalone / sentinel) **or** `GOCELL_REDIS_CLUSTER_ADDRS` (Redis Cluster) is required at startup. The same Redis client backs both service-token nonce replay protection and outbox idempotency claiming, so missing Redis fails fast before any listener binds.
...
| `GOCELL_REDIS_PASSWORD` | Redis password (applies to all modes) | — | No | Passed directly to the Redis client. Leave unset for deployments without Redis AUTH. URL-embedded credentials in cluster URLs override this on a per-URL basis. |
