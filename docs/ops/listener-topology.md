...
**Network-exposure note**: the gRPC listener serves service-to-service token introspection
(`grpc.auth.session.verify.v1`) and should be cluster-internal — analogous to the internal
listener. Do NOT expose port `9095` via a public LoadBalancer or Ingress. Restrict it in your
NetworkPolicy to caller pods only (see [Kubernetes NetworkPolicy](#kubernetes-networkpolicy)).
...
containerPorts:
  ...
  - name: health
    ...
    protocol: TCP
  - name: grpc
    containerPort: 9095       # gRPC (always-on, service-to-service only — do NOT expose via public LB)
    protocol: TCP
...
> **HTTP idempotency scope is assembly-wide, not per-listener — but only for authenticated *user* requests.** Unlike auth (per-listener — each listener carries its own auth chain), the framework HTTP idempotency replay domain spans **all listeners and all pods** that share one Redis backend: a mutating request is deduplicated/replayed by the `(tenant, subject, method, path, Idempotency-Key)` key with no per-listener/cell/pod dimension. **Applicability caveat:** only requests carrying a `PrincipalUser` with a non-empty subject are tracked — `PrincipalService` (the 4-part service token used on `/internal/v1/*`), anonymous, and unauthenticated requests **bypass idempotency entirely** (no Claim, no replay; see `runtime/http/idempotency.extractIdentity`). So "all listeners share the replay domain" means the *store and keyspace* are shared, not that every listener's traffic is cached: an `/internal/v1/*` service-token call is never deduplicated even though it shares the store. See `docs/ops/redis-cluster-deployment.md` §"Full-assembly HTTP idempotency" and ADR `202606051000-1449`. (Routing one *logical command* to a single dedup slot *across cells* is a distinct, deferred concern — #1610.)
...
services:
  gocell:
    ...
    environment:
      ...
      # OR set GOCELL_GRPC_ALLOW_INSECURE=true to run plaintext behind a
      # TLS-terminating sidecar — otherwise startup fails fast.
      GOCELL_GRPC_ALLOW_INSECURE: "true"   # sidecar/demo: sidecar terminates TLS
    healthcheck:
...
spec:
  ...
  ingress:
    ...
    - ports:
        - protocol: TCP
          port: 9091
    # gRPC listener — service-to-service token introspection only.
...
5. **Roll-out**: deploy the new binary into a canary pod first. The canary serves `/api/v1/*` on the same external endpoint, so Ingress / Service stays valid throughout the swap; the only externally observable change is that `/healthz` on `:8080` stops returning 200 — a JWT-protected primary answers the now-unregistered path with `401` (auth challenge before route dispatch), an unauthenticated primary with `404`. Match probes on "not 200", not a specific code, and land the probe swap in step 3 before traffic shifts.
