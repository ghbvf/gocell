...
`gocell generate contract` 使用与 `generate cell` 完全相同的 shared parser（`parseCodegenFlags`），默认行为一致：

| 调用形式 | 行为 |
|---|---|
| `gocell generate contract` | 等价 `--all`（处理所有 K#04 opt-in contract） |
| `gocell generate contract <contractID>` | 仅处理该 contract；positional id 优先于 --all |
| `gocell generate contract --dry-run` | 默认 --all + dry-run |
| `gocell generate contract --verify` | 默认 --all + verify drift |

## verify codegen-contract

| 调用形式 | 行为 |
|---|---|
| `gocell verify codegen-contract` | 等价 `--local=true`（in-place verify against current working tree） |
| `gocell verify codegen-contract --local=false` | sandbox mode（git worktree 隔离 + go run） |
